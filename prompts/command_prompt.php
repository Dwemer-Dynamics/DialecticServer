...
/*
$COMMAND_PROMPT_FUNCTIONS = "
Use tool calling to control {$GLOBALS["DIALECTIC_NAME"]}'s actions.
...
if (!function_exists('dialecticGetDialogueTargetVerb')) {
    function dialecticGetDialogueTargetVerb()
    {
        $mode = strtoupper((string)($GLOBALS["DIALECTIC_EXECUTION_MODE"] ?? ""));
        if ($mode === "WHISPER") {
...
$dialogueTargetVerb = dialecticGetDialogueTargetVerb();
$DIALOGUE_TARGET="({$dialogueTargetVerb} {$GLOBALS["DIALECTIC_NAME"]})";
$MEMORY_OFFERING="";
