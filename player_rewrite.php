...
if (PHP_SAPI !== "cli") {
    ...
    echo json_encode([
        'schema' => 'dialectic.player_rewrite.response.v1',
        'ok' => false,
        'error' => 'player_rewrite.php is an internal CLI worker',
...
function dialecticPlayerRewritePromptFromManager(string $promptKey, string $fallback, array $replacements): string
{
    try {
        $db = $GLOBALS["db"] ?? null;
        if (!$db) {
...
$sourceSpeech = trim((string)($argv[1] ?? ''));
...
if (! isset($GLOBALS["DIALECTIC_CORE_CURRENT_CONNECTOR_DATA"])) {
    ...
    error_log("Using {$GLOBALS["CURRENT_CONNECTOR"]} <{$sourceSpeech}>");
    ...
    if (!empty($GLOBALS["PROMPT_HEAD"])) {
        $promptHead = strtr($GLOBALS["PROMPT_HEAD"], ["#DIALECTIC_NAME#" => $GLOBALS["PLAYER_NAME"]]);
    }
    ...
    if ($sourceSpeech === "") {
        ...
        if ($removePlayerAutochatAsterisks) {
            $instruction = dialecticPlayerRewritePromptFromManager(
            ...
            $outputInstruction = dialecticPlayerRewritePromptFromManager(
