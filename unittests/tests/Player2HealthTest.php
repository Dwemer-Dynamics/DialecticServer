...
final class Player2HealthTest extends TestCase
{
    public function testNormalizesSupportedConnectorUrls(): void
    {
        self::assertSame(
            ...
            dialecticPlayer2HealthNormalizeUrl('http://127.0.0.1:4315/v1/chat/completions')
        );
        self::assertSame(
            'https://player2.example:443/v1/health',
            dialecticPlayer2HealthNormalizeUrl('https://player2.example:443/docs')
        );
    }
    ...
    public function testRequiresFreshGameActivityAndCurrentArmedSession(): void
    {
        ...
        self::assertTrue(dialecticPlayer2HealthShouldPing($state, 1000));
        self::assertFalse(dialecticPlayer2HealthShouldPing($state, 959));

        $state['last_activity'] = 700;
        self::assertFalse(dialecticPlayer2HealthShouldPing($state, 1000));
