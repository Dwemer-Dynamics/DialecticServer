...
function deleteCachedCartesiaVoiceId($voiceName, ?array $config = null): void {
    $db = ensureCartesiaDb();
    if (!$db) {
        return;
    }
...
function getCartesiaCachedVoicesMap(?array $config = null): array {
    ...
    $config = is_array($config) ? $config : getCartesiaActiveConfig();
    $cloned = [];
    $prefix = getCartesiaVoiceCachePrefix($config);
...
function cloneVoiceToCartesia($voiceName, $voiceSamplePath, ?array $config = null) {
    ...
    $data .= "Voice generated by DIALECTIC for NPC: {$voiceName}" . $eol;
