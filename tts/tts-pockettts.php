...
function pockettts_is_audio_cpp() {
	$endpoint = pockettts_scalar_setting('endpoint', '');
...
$GLOBALS["TTS_IN_USE"]=function($textString, $mood , $stringforhash) {
		...
		$isAudioCppPocketTts = pockettts_is_audio_cpp();
		$url = normalize_endpoint_url($GLOBALS["TTS"]["POCKETTTS"]["endpoint"]);
		$url .= $isAudioCppPocketTts ? "/v1/audio/speech" : "/tts_to_audio";
