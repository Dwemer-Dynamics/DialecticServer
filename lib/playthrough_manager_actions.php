...
function dpt_query($conn, string $sql, array $params = [])
{
    ...
    if (!$result) {
        ...
        throw new RuntimeException('Snapshot operation failed. Your previous data has been kept.');
    }
    return $result;
}
...
function dpt_capture($conn, string $name, string $notes, ?array $existing = null, bool $active = false): void
{
    ...
    if (!str_starts_with($schema, 'dialectic_profile_')) throw new RuntimeException('Invalid snapshot schema.');
    if (!$existing && pts_schema_exists($conn, $schema)) throw new RuntimeException('A snapshot with this name already exists.');
...
{
    if (!in_array($action, ['setup', 'create', 'switch', 'delete'], true)) throw new RuntimeException('Unknown snapshot action.');
    if (!pts_metadata_schema_ready($conn)) throw new RuntimeException('Run the DIALECTIC database update before managing snapshots.');
    ...
    try {
        ...
        if (pg_fetch_result(dpt_query($conn, "SELECT pg_try_advisory_xact_lock(hashtext('dialectic_storage_manager'))"), 0, 0) !== 't') {
            throw new RuntimeException('Another snapshot action is running. Try again shortly.');
        }
        if ($action === 'setup' || $action === 'create') {
            ...
            if ($count === 0) dpt_capture($conn, 'default', 'Initial recovery snapshot', null, true);
            if ($action === 'create') {
                $name = trim((string)($input['name'] ?? ''));
                if ($name === '' || strlen($name) > 160) throw new RuntimeException('Enter a snapshot name of 1–160 characters.');
            ...
            $message = $action === 'setup' ? 'Initial recovery snapshot saved.' : 'Snapshot saved.';
        } else {
            $id = filter_var($input['profile_id'] ?? null, FILTER_VALIDATE_INT);
            if (!$id || $id < 1) throw new RuntimeException('Choose a valid snapshot.');
            $target = pg_fetch_assoc(dpt_query($conn, 'SELECT * FROM dialectic_meta.playthrough_profiles WHERE id=$1 FOR UPDATE', [$id]));
            if (!$target) throw new RuntimeException('That snapshot no longer exists.');
            if (in_array($target['is_active'], [true, 't', '1'], true)) throw new RuntimeException('This snapshot is already loaded.');
            if ($target['storage_type'] !== 'schema' || !str_starts_with((string)$target['schema_name'], 'dialectic_profile_')) {
                throw new RuntimeException('This snapshot has an unsupported storage format.');
            }
            if ($action === 'delete') {
                if (strtolower($target['name']) === 'default') throw new RuntimeException('The initial recovery snapshot cannot be deleted.');
                $drop = pts_drop_schema($conn, $target['schema_name']);
                if (empty($drop['success'])) throw new RuntimeException('Could not delete the snapshot. Nothing was removed.');
                dpt_query($conn, 'DELETE FROM dialectic_meta.playthrough_profiles WHERE id=$1', [$id]);
                $message = 'Snapshot deleted.';
            } else {
                if (!pts_schema_exists($conn, $target['schema_name'])) throw new RuntimeException('The saved snapshot schema is missing.');
                ...
                if (!$current) throw new RuntimeException('No loaded snapshot is recorded. Save a recovery snapshot before restoring.');
                ...
                $message = 'Snapshot restored. Restart the DIALECTIC server and Fallout, then load the matching game save.';
            }
        }
