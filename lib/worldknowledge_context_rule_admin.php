...
function worldknowledge_context_rule_values($value): array
{
    $values = preg_split('/\s*,\s*/u', trim((string)$value)) ?: [];
    return array_values(array_filter(array_map('trim', $values), static fn($entry) => $entry !== ''));
}

function worldknowledge_context_rule_conditions_from_post(array $post): array
{
    $conditions = [];
    foreach ([
        'npc',
        ...
        $values = worldknowledge_context_rule_values($post['condition_' . $field] ?? '');
        if (!empty($values)) {
...
function worldknowledge_context_rule_condition_text(array $conditions, string $field): string
{
...
function worldknowledge_context_rule_handle_post($conn, string $schema, array $post, string $method): string
{
    if ($method !== 'POST') {
        return '';
    }
    ...
    if (isset($post['delete_context_rule'])) {
        ...
        $result = pg_query_params(
            $conn,
            "DELETE FROM {$schema}.worldknowledge_context_rule WHERE id = $1",
            [$ruleId]
        );
    ...
    $conditionsJson = json_encode(
        worldknowledge_context_rule_conditions_from_post($post),
    ...
    if ($ruleId > 0) {
        $result = pg_query_params(
            $conn,
            "UPDATE {$schema}.worldknowledge_context_rule
    ...
    $result = pg_query_params(
        $conn,
        "INSERT INTO {$schema}.worldknowledge_context_rule
