...
function ptr_ensure_schema($conn): void {
    ptr_query($conn, 'CREATE SCHEMA IF NOT EXISTS dialectic_meta');
    ptr_query($conn, 'CREATE TABLE IF NOT EXISTS dialectic_meta.settings (key TEXT PRIMARY KEY, value TEXT)');
    if (ptr_exists($conn, 'dialectic_meta.playthrough_profiles')) {
        ptr_query($conn, "ALTER TABLE dialectic_meta.playthrough_profiles ADD COLUMN IF NOT EXISTS retention_kind TEXT NOT NULL DEFAULT 'unclassified'");
        ptr_query($conn, 'ALTER TABLE dialectic_meta.playthrough_profiles ADD COLUMN IF NOT EXISTS retention_pinned BOOLEAN NOT NULL DEFAULT false');
    }
}

function ptr_read($conn, string $key, $fallback) {
    if (!ptr_exists($conn, 'dialectic_meta.settings')) return $fallback;
    $row = pg_fetch_assoc(ptr_query($conn, 'SELECT value FROM dialectic_meta.settings WHERE key=$1', [$key]));
    return $row ? (json_decode($row['value'], true) ?? $fallback) : $fallback;
}

function ptr_write($conn, string $key, $value): void {
    ptr_query($conn, 'INSERT INTO dialectic_meta.settings (key,value) VALUES ($1,$2) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value', [$key, json_encode($value, JSON_THROW_ON_ERROR)]);
}
...
function ptr_lock($conn): bool {
    return pg_fetch_result(ptr_query($conn, "SELECT pg_try_advisory_lock(hashtext('dialectic_meta_playthrough_retention'))"), 0, 0) === 't';
}

function ptr_unlock($conn): void {
    @pg_query($conn, "SELECT pg_advisory_unlock(hashtext('dialectic_meta_playthrough_retention'))");
}

function ptr_profiles($conn): array {
    if (!ptr_exists($conn, 'dialectic_meta.playthrough_profiles')) return [];
    ...
    $rows = pg_fetch_all(ptr_query($conn, "SELECT id, name, is_active, created_at, size_bytes,
        ...
        FROM dialectic_meta.playthrough_profiles p ORDER BY created_at DESC, id DESC")) ?: [];
    foreach ($rows as &$row) {
        $row['id'] = (int)$row['id'];
...
function ptr_preview($conn, array $settings, ?string $category = null): array {
    ...
    if ($settings['playthroughs_enabled'] && $settings['playthrough_keep'] > 0) {
        ...
        foreach (ptr_profiles($conn) as $profile) {
            ...
            if ($profile['storage_type'] !== 'schema' || !str_starts_with((string)$profile['schema_name'], 'dialectic_profile_')) continue;
            $plan['playthroughs'][] = ['id' => $profile['id'], 'name' => $profile['name'], 'bytes' => $profile['bytes']];
        }
...
function ptr_delete_playthrough($conn, int $id): void {
    $row = pg_fetch_assoc(ptr_query($conn, 'SELECT *, to_jsonb(p)->>\'retention_pinned\' AS pinned FROM dialectic_meta.playthrough_profiles p WHERE id=$1 FOR UPDATE', [$id]));
    ...
    if (($row['storage_type'] ?? 'dump') === 'schema') {
        ...
        if (!preg_match('/^dialectic_profile_[a-z0-9_]+$/D', $schema)) throw new RuntimeException('That playthrough has an unexpected name and was left alone.');
    ...
    ptr_query($conn, 'DELETE FROM dialectic_meta.playthrough_profiles WHERE id=$1', [$id]);
}
...
function ptr_execute($conn, array $plan): array {
    ...
    try {
        ...
        if (ptr_exists($conn, 'dialectic_meta.playthrough_profiles')) ptr_query($conn, 'LOCK TABLE dialectic_meta.playthrough_profiles IN SHARE ROW EXCLUSIVE MODE');
...
function ptr_tick($conn): void {
    if (!ptr_exists($conn, 'dialectic_meta.settings')) return;
