...
class AutomaticBackup {
    ...
    public function __construct() {
        ...
        // Ensure dialectic_meta.settings exists and load retention count if available
        try {
            $db = $this->getDatabaseConnection();
            if ($db) {
                $rows = $db->fetchAll("SELECT value FROM dialectic_meta.settings WHERE key = 'AUTOMATIC_BACKUP_MAX_COUNT'");
    ...
    public function isEnabled() {
        ...
        try {
            $db = $this->getDatabaseConnection();
            if ($db) {
                $rows = $db->fetchAll("SELECT value FROM dialectic_meta.settings WHERE key = 'AUTOMATIC_DATABASE_BACKUPS'");
                $row = is_array($rows) && count($rows) > 0 ? $rows[0] : null;
                if ($row && isset($row['value'])) {
    ...
    public function createBackup() {
        ...
        try {
            ...
            $pgpassLine = "{$host}:{$port}:{$dbname}:{$username}:{$password}";
            $pgpassResult = shell_exec('printf %s ' . escapeshellarg($pgpassLine) . ' > /tmp/.pgpass; echo $?');
            ...
            $command = "HOME=/tmp pg_dump -d " . escapeshellarg($dbname) . " -U " . escapeshellarg($username) . " -h " . escapeshellarg($host) . " -p " . escapeshellarg($port) . " -T dialectic_meta.settings > " . escapeshellarg($filepath) . " 2>&1";
    ...
    public function shouldCreateBackup() {
        ...
        try {
            ...
            $db = $this->getDatabaseConnection();
            if ($db) {
                $row = $db->fetchOne("SELECT value FROM dialectic_meta.settings WHERE key='AUTOMATIC_BACKUP_LAST_TIMESTAMP'");
    ...
    public function updateBackupCheckDate() {
        try {
            ...
            $db->upsertRowOnConflict('dialectic_meta.settings', ['key'=>'AUTOMATIC_BACKUP_LAST_TIMESTAMP','value'=>$timestamp], 'key');
