...
CREATE OR REPLACE FUNCTION dialectic_meta.sync_schema_sequences(target_schema text)
RETURNS integer AS $$
DECLARE
    obj RECORD;
...
CREATE OR REPLACE FUNCTION dialectic_meta.clone_selected_schema(source_schema text, dest_schema text, selected_tables text[])
RETURNS void AS $$
DECLARE
    obj RECORD;
    seq_val BIGINT;
...
BEGIN
    ...
    PERFORM dialectic_meta.sync_schema_sequences(dest_schema);
...
CREATE OR REPLACE FUNCTION dialectic_meta.clone_schema(source_schema text, dest_schema text)
RETURNS void AS $$
BEGIN
    PERFORM dialectic_meta.clone_selected_schema(source_schema, dest_schema, NULL);
END;
$$ LANGUAGE plpgsql;
-- Helper function to drop a schema and all its contents safely
CREATE OR REPLACE FUNCTION dialectic_meta.drop_schema_safe(schema_name text)
RETURNS boolean AS $$
BEGIN
    -- Prevent dropping critical schemas
    IF schema_name IN ('public', 'pg_catalog', 'information_schema', 'pg_toast', 'dialectic_meta') THEN
...
CREATE OR REPLACE FUNCTION dialectic_meta.get_schema_size(schema_name text)
RETURNS bigint AS $$
DECLARE
    total_size bigint := 0;
    obj RECORD;
