...
CREATE OR REPLACE FUNCTION dialectic_meta.prepare_playthrough(source_schema text, selected_tables text[])
RETURNS text AS $$
DECLARE
    stage_schema text := 'dialectic_profile_upgrade_' || pg_backend_pid() || '_' || txid_current();
    source_names text[];
    live_names text[];
    manifest_text text;
    manifest jsonb;
...
BEGIN
    IF source_schema !~ '^dialectic_profile_[a-z0-9_]+$' OR source_schema = stage_schema THEN
    ...
    PERFORM dialectic_meta.validate_save_manifest(source_schema, all_source_names);
    ...
    PERFORM dialectic_meta.clone_selected_schema(source_schema, stage_schema, source_names);
    ...
    FOREACH table_name IN ARRAY live_names LOOP
        IF NOT (table_name=ANY(source_names)) THEN
            ...
            required_version := CASE WHEN table_name=version_key THEN 20260829002::bigint ELSE NULL END;
            ...
            IF source_policy=2 AND table_name=ANY(ARRAY['worldknowledge','worldknowledge_audit','worldknowledge_catalogs','worldknowledge_context_rule']) THEN
...
CREATE OR REPLACE FUNCTION dialectic_meta.validate_playthrough(source_schema text, selected_tables text[])
RETURNS void AS $$
BEGIN
    BEGIN
        PERFORM dialectic_meta.restore_playthrough(source_schema,selected_tables);
...
CREATE OR REPLACE FUNCTION dialectic_meta.restore_playthrough_upgraded(source_schema text, selected_tables text[])
RETURNS void AS $$
DECLARE stage_schema text; missing jsonb; empty_tables jsonb;
BEGIN
    stage_schema := dialectic_meta.prepare_playthrough(source_schema,selected_tables);
    ...
    END IF;
    PERFORM dialectic_meta.restore_playthrough(stage_schema,selected_tables);
