...
class sql
{
    ...
    public function __construct()
    {
        ...
        if (isset($_SERVER['HTTP_X_DIALECTIC_PLAYTHROUGH'])
    ...
    private function readConnectionSetting(string $envName, string $globalName, string $default): string
    {
        ...
        if (isset($GLOBALS[$globalName]) && trim(strval($GLOBALS[$globalName])) !== '') {
            return trim(strval($GLOBALS[$globalName]));
        }
    ...
    {
        $host = $this->readConnectionSetting('DIALECTIC_DB_HOST', 'DIALECTIC_DB_HOST', '127.0.0.1');
        $dbname = $this->readConnectionSetting('DIALECTIC_DB_NAME', 'DIALECTIC_DB_NAME', 'dialectic');
        $user = $this->readConnectionSetting('DIALECTIC_DB_USER', 'DIALECTIC_DB_USER', 'dwemer');
        $password = $this->readConnectionSetting('DIALECTIC_DB_PASSWORD', 'DIALECTIC_DB_PASSWORD', 'dwemer');
        $connectTimeout = $this->readConnectionSetting('DIALECTIC_DB_CONNECT_TIMEOUT', 'DIALECTIC_DB_CONNECT_TIMEOUT', '90');
        ...
        return implode(' ', [
            ...
            'connect_timeout=' . $this->escapeConnectionValue($connectTimeout),
        ]);
    }
