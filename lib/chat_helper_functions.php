...
function callConfiguredTts($textString, $mood, $stringforhash)
{
    ...
    $ttsFile = __DIR__ . "/../tts/tts-" . $ttsFunction . ".php";
...
{
    if (function_exists('dialectic_tts_cache_seed')) {
        return dialectic_tts_cache_seed(dirname(__DIR__), $speaker, $text);
    }
...
function canRetryNpcTtsWithFallback(): bool
{
    ...
    $currentName = trim(strval($GLOBALS["DIALECTIC_NAME"] ?? ''));
...
function ensureNpcTtsVoiceResolvedForCurrentSpeaker(): void
{
    $currentNpcData = $GLOBALS["DIALECTIC_CORE_CURRENT_NPC_DATA"] ?? null;
    $currentName = trim(strval($GLOBALS["DIALECTIC_NAME"] ?? ''));
    if (!is_array($currentNpcData) || $currentName === '' || strcasecmp($currentName, 'The Narrator') === 0) {
        return;
    }
    if (strcasecmp(trim(strval($currentNpcData['npc_name'] ?? '')), $currentName) !== 0) {
        return;
    ...
    $voiceKey = md5($currentName . '|' . trim(strval($currentNpcData['voiceid'] ?? '')) . '|' . trim(strval($GLOBALS["TTSFUNCTION"] ?? '')));
    if (($GLOBALS['DIALECTIC_NPC_TTS_VOICE_RESOLVED_FOR'] ?? '') === $voiceKey) {
        return;
    }
    ...
    Logger::phaseStart("npc_tts_voice_lazy_resolve", [
        "npc" => $currentName,
        "voiceid" => $currentNpcData['voiceid'] ?? '',
        "connector" => $GLOBALS["TTSFUNCTION"] ?? "",
    ]);

    $npcMaster = new NpcMaster();
    $npcMaster->setOldGlobalsFromCurrentNpcData($currentNpcData, true);
    $GLOBALS['DIALECTIC_NPC_TTS_VOICE_RESOLVED_FOR'] = $voiceKey;

    Logger::phaseEnd("npc_tts_voice_lazy_resolve", [
        "npc" => $currentName,
        "voice" => $GLOBALS["TTS_NPC_RESOLVED_VOICE"] ?? "",
        "sample" => !empty($GLOBALS["TTS_NPC_RESOLVED_VOICE_SAMPLE"] ?? '') ? "yes" : "no",
    ], "info");
}

function callNpcTtsWithFallback($textString, $mood, $stringforhash)
{
    ensureNpcTtsVoiceResolvedForCurrentSpeaker();
    ...
    Logger::warn("[TTS FALLBACK] Retrying NPC TTS for {$GLOBALS["DIALECTIC_NAME"]} with fallback voice '{$fallbackVoice}' after initial synthesis failure.");
...
{
    // Never speak unresolved NVSE string-variable names from contaminated context.
    $rawResponse = preg_replace('/\bs(?:Subtitle|SpeakerName|TargetName)\b/i', '', strval($rawResponse));
    ...
    $rawResponse = str_replace(
        ["\0", "\u{2010}", "\u{2011}", " \u{2014} ",  "\u{2014}",  "\u{2018}", "\u{2019}", "\u{201A}", "\u{201B}", "\u{3002}\u{3002}\u{3002}", "\u{2026}",   "{", "}" ],  //, " U.S.A. ", " U.S. " ],
        [''  , "-", "-", " - ", " - ", "'", "'", "'", "'", "...", "...", "",  ""  ], //,  " USA ",    " US "    ],
        $rawResponse);
    ...
    $dialecticNameShort = $GLOBALS["DIALECTIC_NAME"];
    $matches = [];
        // Some LLMs will omit part of the name in brackets (e.g., Ranger [NCR Patrol] becomes Ranger in the response).
    // This avoids reading off the abbreviated name in the TTS.
    if (preg_match('/^(.+?) \[(.+)\]$/', $GLOBALS["DIALECTIC_NAME"], $matches)) {
        $dialecticNameShort = $matches[1];
    }
    
    if (stripos($toSplit, "{$GLOBALS["DIALECTIC_NAME"]}:") !== false || preg_match("/{$dialecticNameShort}\s*:/", $toSplit, $matches)) {
        $rawResponseSplited = explode(":", $toSplit);
        array_shift($rawResponseSplited);
        $toSplit = implode(":", $rawResponseSplited);
    }

    //$toSplit = preg_replace("/{$GLOBALS["DIALECTIC_NAME"]}\s*:\s*/", '', $toSplit);
    ...
    $sentenceXX = str_replace(
        array("\u{FF11}", "\u{FF12}", "\u{FF13}", "\u{FF14}", "\u{FF15}", "\u{FF16}", "\u{FF17}", "\u{FF18}", "\u{FF19}", "\u{FF10}"),
...
function findFastSentencePosition($s_string,$min_sentence_size=0) {
    ...
    if (preg_match_all($splitSentenceRegex, $s_string, $matches, PREG_OFFSET_CAPTURE)) {
        foreach ($matches[1] as $match) {
            ...
            // (e.g. Japanese sentence punctuation, which is multi-byte in UTF-8) are not split mid-character.
...
//    $paragraph = strtr($paragraph, array(" \n\n" => ".", " \n" => ".", "\n\n" => ".", '\n' => ".", "\n" => "."));
    ...
    $paragraphNcr = preg_replace('/([\x{3002}\x{FF01}\x{FF1F}])(?=\S)/u', '$1 ', $paragraphNcr);
...
function split_sentences_stream($paragraph)
{
    ...
    $paragraph = preg_replace('/([\x{3002}\x{FF01}\x{FF1F}])(?=\S)/u', '$1 ', $paragraph);
...
function stripOutputSpeakerPrefix($text, $speakerName = null) {
    $speakerName = $speakerName ?? ($GLOBALS["DIALECTIC_NAME"] ?? "");
    if ($speakerName === '') {
        return (string)$text;
    }
...
function formatNpcSubtitleText($text) {
    ...
    return cleanupDisplayText($subtitleText, $GLOBALS["DIALECTIC_NAME"] ?? null);
}

function formatInlineNarrationDialogueSubtitleText($text) {
    $subtitleText = shouldStripNpcOutputAsterisks() ? formatNpcSpeechText($text) : (string)$text;
    $subtitleText = cleanupDisplayText($subtitleText, $GLOBALS["DIALECTIC_NAME"] ?? null);
...
function formatTextOnlyInlineNarrationSubtitleText($text) {
    return cleanupDisplayText($text, $GLOBALS["DIALECTIC_NAME"] ?? null);
}
...
function saveCurrentVoiceSettings() {
    return [
        ...
        'has_current_tts_connector_id' => array_key_exists('DIALECTIC_CORE_CURRENT_TTS_CONNECTOR_ID', $GLOBALS),
        'current_tts_connector_id' => $GLOBALS['DIALECTIC_CORE_CURRENT_TTS_CONNECTOR_ID'] ?? null,
...
function applyVoiceIdToTtsGlobals(string $voiceid): void
{
    ...
    $voiceReference = dialectic_resolve_tts_voice_reference($voiceid, [
        'actor_name' => $GLOBALS["DIALECTIC_NAME"] ?? '',
    ]);
    $GLOBALS['PATCH_OVERRIDE_VOICE'] = $voiceReference;
    unset($GLOBALS['PATCH_OVERRIDE_VOICE_ID']);

    $GLOBALS['TTS']['XTTSFASTAPI']['voiceid']  = $voiceReference;
...
function restoreVoiceSettings($savedSettings) {
    ...
    if (!empty($savedSettings['has_current_tts_connector_id'])) {
        $GLOBALS['DIALECTIC_CORE_CURRENT_TTS_CONNECTOR_ID'] = $savedSettings['current_tts_connector_id'];
    } else {
...
function unmoodSentence($sentence) {
    ...
    if (!$isPlayerSpeech && $processAsterisks === true ) {
        error_log("[unmoodSentence] NPC output asterisk filtering is active! $sentence <" . ($GLOBALS['strip_emotes_from_output'] ?? 'N/A') . "> <" . ($GLOBALS['REMOVE_ASTERISKS_FROM_NPC_OUTPUT'] ?? 'N/A') . ">" );
...
function dialectic_generate_deferred_tts(): void
{
    if (empty($GLOBALS["DIALECTIC_DEFERRED_TTS"]) || !is_array($GLOBALS["DIALECTIC_DEFERRED_TTS"])) {
        return;
    }

    $pending = $GLOBALS["DIALECTIC_DEFERRED_TTS"];
    $GLOBALS["DIALECTIC_DEFERRED_TTS"] = [];
    ...
    foreach ($pending as $entry) {
        ...
        $phaseName = "npc_tts_deferred:" . substr(md5($speaker . "|" . $cacheKey), 0, 8);
        Logger::phaseStart($phaseName, [
            "speaker" => $speaker,
            "connector" => $entry["tts_function"] ?? ($GLOBALS["TTSFUNCTION"] ?? ""),
            "cache_key" => $cacheKey,
            "chars" => strlen($text),
        ]);
...
function dialectic_npc_tts_php_binary(): string
{
    $knownCliCandidates = DIRECTORY_SEPARATOR === '\\'
        ? [
            defined('PHP_BINDIR') ? rtrim((string)PHP_BINDIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'php.exe' : '',
            defined('PHP_BINDIR') ? rtrim((string)PHP_BINDIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'php' : '',
            'php',
        ]
        : [
            '/usr/bin/php',
    ...
    $candidates = PHP_SAPI === 'cli'
        ? array_merge([defined('PHP_BINARY') ? (string)PHP_BINARY : ''], $knownCliCandidates)
        : $knownCliCandidates;
...
function dialectic_spawn_deferred_npc_tts_worker(array $entry): bool
{
    ...
    if (!is_file($worker)) {
        Logger::warn('[TTS] NPC deferred worker missing' . Logger::formatContext([
            'worker' => $worker,
        ]));
        return false;
    }
    ...
    $jobsDir = $root . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'npc_tts_jobs';
    if (!is_dir($jobsDir)) {
    ...
    if (file_exists($lockPath) && (time() - (int)@filemtime($lockPath)) < 60) {
        Logger::debug('[TTS] NPC deferred worker already active' . Logger::formatContext([
    ...
    if (@file_put_contents($lockPath, (string)time(), LOCK_EX) === false) {
        Logger::warn('[TTS] Could not write NPC deferred worker lock' . Logger::formatContext([
            'lock_path' => $lockPath,
        ]));
        return false;
    }
    ...
    if (@file_put_contents($jobPath, json_encode($entry, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), LOCK_EX) === false) {
        @unlink($lockPath);
        Logger::warn('[TTS] Could not write NPC deferred worker job' . Logger::formatContext([
            'job_path' => $jobPath,
        ]));
        return false;
    }

    $php = dialectic_npc_tts_php_binary();
    ...
    if (is_resource($handle)) {
        @pclose($handle);
        Logger::info('[TTS] Spawned NPC deferred TTS worker' . Logger::formatContext([
            'speaker' => $entry['speaker'] ?? '',
            'cache_key' => $cacheKey,
    ...
    @unlink($lockPath);
    Logger::warn('[TTS] Failed to spawn NPC deferred TTS worker' . Logger::formatContext([
        'speaker' => $entry['speaker'] ?? '',
        'cache_key' => $cacheKey,
    ]));
    return false;
...
function returnLines($lines,$writeOutput=true)
{
    ...
    foreach ($lines as $n => $sentence) {
        ...
        if ($preserveAsterisksInContext) {
            $responseForContext = cleanupDisplayText($sentenceForSubtitles, $GLOBALS["DIALECTIC_NAME"] ?? null);
        }
        ...
        if (isset($GLOBALS["FEATURES"]["MISC"]["TTS_RANDOM_PITCH"])&&($GLOBALS["FEATURES"]["MISC"]["TTS_RANDOM_PITCH"])) {
            $random_per_character=sprintf('%u', crc32($GLOBALS["DIALECTIC_NAME"])); // Unsigned integer
        ...
        if ($responseTextUnmooded || $hasNarrationBlocks || $hasTextOnlyNarration) {
            ...
            // Check if we need to split narration to The Narrator
                ...
                $savedDialecticName = $GLOBALS["DIALECTIC_NAME"];
        ...
        if ($shouldEmitNpcLine) {
            Logger::info("Speech sent for {$GLOBALS["DIALECTIC_NAME"]}, generator {$GLOBALS["TTSFUNCTION"]}, size: ".strlen($responseText). "  '".substr($responseText,0,10)."'");
        } else {
        ...
        $outBuffer = array(
            ...
            'actor' => $GLOBALS["DIALECTIC_NAME"],
            'action' => "dialectic_dialogue_response",
            'tag' => (isset($tag) ? $tag : "")
        );
        ...
        if ($writeOutput && $shouldEmitNpcLine) {
            ...
            if (true) {
                ...
                // DIALECTIC_ANIMATIONS is now always enabled (controlled via MCM in-game)
                ...
                $strictRechatListener = dialecticGetStrictRechatListenerName();
                if ($strictRechatListener !== "") {
                    $GLOBALS["SCRIPTLINE_LISTENER"] = $strictRechatListener;
                }

                $strictDirectedPlayerListener = dialecticGetStrictDirectedPlayerListenerName();
                ...
                $responseStreamingRequested = function_exists('dialectic_response_stream_requested')
                    ? dialectic_response_stream_requested()
                    : !empty($GLOBALS["DIALECTIC_RESPONSE_STREAMING"]);
                $jsonStreamingEnabled = function_exists('dialectic_json_streaming_enabled') && dialectic_json_streaming_enabled();
                $generateNpcTtsBeforeEmit = function_exists('dialectic_should_generate_npc_tts_before_emit')
                    ? dialectic_should_generate_npc_tts_before_emit()
                    : !$responseStreamingRequested;

                if ($generateNpcTtsBeforeEmit && $pendingNpcTtsText !== "" && $pendingNpcTtsCacheKey !== "") {
                    $phaseName = "npc_tts_before_emit:" . substr(md5(($GLOBALS["DIALECTIC_NAME"] ?? "") . "|" . $pendingNpcTtsCacheKey), 0, 8);
                    Logger::phaseStart($phaseName, [
                        "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                        "connector" => $GLOBALS["TTSFUNCTION"] ?? "",
                        "cache_key" => $pendingNpcTtsCacheKey,
                        "chars" => strlen($pendingNpcTtsText),
                    ]);
                    ...
                    if ($ttsOutput) {
                        $GLOBALS["TRACK"]["FILES_GENERATED"][] = $ttsOutput;
                        $npcTtsGeneratedBeforeEmit = true;
                        Logger::info("[TTS] Generated before JSON emit for {$GLOBALS["DIALECTIC_NAME"]}: {$ttsOutput}");
                        Logger::phaseEnd($phaseName, [
                            "status" => "ok",
                            "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                            "output" => $ttsOutput,
                        ...
                        Logger::warn("[TTS] No audio generated before JSON emit for {$GLOBALS["DIALECTIC_NAME"]}");
                        Logger::phaseEnd($phaseName, [
                            "status" => "failed",
                            "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                            "cache_key" => $pendingNpcTtsCacheKey,
                ...
                } elseif (!$generateNpcTtsBeforeEmit && $pendingNpcTtsText !== "" && $pendingNpcTtsCacheKey !== "") {
                    Logger::info("[TTS] Streaming response: emitting line before NPC TTS generation for {$GLOBALS["DIALECTIC_NAME"]}: {$pendingNpcTtsCacheKey}" . Logger::formatContext([
                        "stream_requested" => $responseStreamingRequested ? 1 : 0,
                        "json_streaming" => $jsonStreamingEnabled ? 1 : 0,
                        "response_format" => $GLOBALS["DIALECTIC_RESPONSE_FORMAT"] ?? "",
                    ]));
                }

                dialectic_buffer_speech_response_line(
                    (string)$outBuffer["actor"],
                    $responseForSubtitles,
                    (string)$GLOBALS["SCRIPTLINE_EXPRESSION"],
                    (string)$GLOBALS["SCRIPTLINE_LISTENER_ATOMIC"],
                    (string)$GLOBALS["SCRIPTLINE_ANIMATION"],
                ...
                $GLOBALS["DEBUG_DATA"]["OUTPUT_LOG"] = json_encode([
                    "speaker" => (string)$outBuffer["actor"],
                    "action" => "say",
                    "text" => $responseForSubtitles,
                    "metadata" => [
                        "expression" => (string)$GLOBALS["SCRIPTLINE_EXPRESSION"],
                        "listener" => (string)$GLOBALS["SCRIPTLINE_LISTENER_ATOMIC"],
                        "animation" => (string)$GLOBALS["SCRIPTLINE_ANIMATION"],
                        "phonetic" => $responseTextPhonetic,
                        "volume" => $volumeBoost,
                        "rechat_target" => (string)$GLOBALS["SCRIPTLINE_RECHAT_TARGET"],
                ...
                if (class_exists('Logger')) {
                    ...
                    Logger::debug("[plugin-output] queued speech line" . Logger::formatContext([
                        "speaker" => (string)$outBuffer["actor"],
                        "listener" => (string)$GLOBALS["SCRIPTLINE_LISTENER_ATOMIC"],
                        "utterance_id" => $currentUtteranceId,
                        "chars" => strlen($responseForSubtitles),
                        "rechat_target" => (string)$GLOBALS["SCRIPTLINE_RECHAT_TARGET"],
                    ]));
                }
                file_put_contents($outputToPluginLog,$GLOBALS["DEBUG_DATA"]["OUTPUT_LOG"], FILE_APPEND | LOCK_EX);
                
            }
            else
                dialectic_buffer_response_line((string)$outBuffer["actor"], (string)$outBuffer["action"], $responseForSubtitles);
            
            if (ob_get_level()) @ob_flush();
            @flush();
        }

        $deferNpcTts = function_exists('dialectic_json_response_enabled')
            && dialectic_json_response_enabled()
            && strcasecmp((string)($GLOBALS["DIALECTIC_NAME"] ?? ""), "Player") !== 0;

        if ($npcTtsGeneratedBeforeEmit) {
            Logger::info("[TTS] Skipping deferred/immediate post-emit TTS because audio was prepared before JSON emit for {$GLOBALS["DIALECTIC_NAME"]}: {$pendingNpcTtsCacheKey}");
        } elseif ($pendingNpcTtsText !== "" && $pendingNpcTtsCacheKey !== "" && $deferNpcTts) {
            $streamingMode = function_exists('dialectic_response_stream_requested')
                ? dialectic_response_stream_requested()
            ...
            if (!$spawnedWorker) {
                if (!isset($GLOBALS["DIALECTIC_DEFERRED_TTS"]) || !is_array($GLOBALS["DIALECTIC_DEFERRED_TTS"])) {
                    $GLOBALS["DIALECTIC_DEFERRED_TTS"] = [];
                }
                $GLOBALS["DIALECTIC_DEFERRED_TTS"][] = $deferredEntry;
            }
            Logger::info("[TTS] Deferred JSON-mode TTS for {$GLOBALS["DIALECTIC_NAME"]}: {$pendingNpcTtsCacheKey} mode=" . ($streamingMode ? "streaming" : "buffered"));
        } elseif ($pendingNpcTtsText !== "" && $pendingNpcTtsCacheKey !== "") {
            $phaseName = "npc_tts_immediate:" . substr(md5(($GLOBALS["DIALECTIC_NAME"] ?? "") . "|" . $pendingNpcTtsCacheKey), 0, 8);
            Logger::phaseStart($phaseName, [
                "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                "connector" => $GLOBALS["TTSFUNCTION"] ?? "",
                "cache_key" => $pendingNpcTtsCacheKey,
            ...
            if ($ttsOutput) {
                $GLOBALS["TRACK"]["FILES_GENERATED"][] = $ttsOutput;
                Logger::info("[TTS] Generated after response emit for {$GLOBALS["DIALECTIC_NAME"]}: {$ttsOutput}");
                Logger::phaseEnd($phaseName, [
                    "status" => "ok",
                    "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                    "output" => $ttsOutput,
                ...
                Logger::warn("[TTS] No audio generated after response emit for {$GLOBALS["DIALECTIC_NAME"]}");
                Logger::phaseEnd($phaseName, [
                    "status" => "failed",
                    "speaker" => $GLOBALS["DIALECTIC_NAME"] ?? "",
                    "cache_key" => $pendingNpcTtsCacheKey,
        ...
        if (!isset($GLOBALS["PATCH_DONT_STORE_SPEECH_ON_DB"])) {
            $escapeLogValue = static function ($value) use ($db) {
                $value = (string)$value;
            ...
            $db->insert(
                ...
                array(
                    'localts' => time(),
                    'prompt' => nl2br($escapeLogValue(json_encode($GLOBALS["DEBUG_DATA"], JSON_PRETTY_PRINT))),
                    'response' => $escapeLogValue($responseForContext),
                    'url' => nl2br($escapeLogValue("$receivedData [AI secs] $elapsedTimeAI  [TTS secs] $elapsedTimeTTS"))
...
function lastKeyWordsContext($n, $npcname='')
{
    ...
    foreach ($uppercaseWords as $n=>$e) {
        ...
        } else if (stripos($e, $GLOBALS["DIALECTIC_NAME"])!==false) {
...
function offerMemory($gameRequest)
{
    ...
    if (!empty($memory)) {
        ...
        if($hoursAgo > getGametsLimitFor($GLOBALS["DIALECTIC_NAME"])) {
            $daysAgo = floor(($gameRequest[2]-$memories[0]["gamets_truncated"]) * 0.0000001);
            $fallout_date = gamets2str_format_date($memories[0]["gamets_truncated"], 'Y-m-d');
            $s_prefix = "{$daysAgo} days ago, on {$fallout_date} ... ";
        } else {
...
function dialecticNormalizePeopleValue($value)
{
    $names = [];

    $appendName = static function ($candidate) use (&$names) {
    ...
    $appendValue = function ($candidate) use (&$appendValue, $appendName) {
        if (is_string($candidate)) {
            foreach (parsePeoplePipeList($candidate) as $name) {
                $appendName($name);
            }
            return;
        }
...
function dialecticDecodeAudienceSnapshotField($rawField)
{
    ...
    $payload = json_decode($rawField, true);
    if (!is_array($payload)) {
        return "";
    }

    if (!empty($payload["people"])) {
        $people = dialecticNormalizePeopleValue($payload["people"]);
        if ($people !== "") {
            return $people;
        }
    }

    if (!empty($payload["companions"])) {
        return dialecticNormalizePeopleValue($payload["companions"]);
    }

    return "";
}

function dialecticSetCurrentTurnPeopleSnapshot($peoplePipe)
{
    ...
    $GLOBALS["DIALECTIC_TURN_PEOPLE_SNAPSHOT"] = $normalized;
    return $normalized;
}

function dialecticGetCurrentTurnPeopleSnapshot()
{
    if (!isset($GLOBALS["DIALECTIC_TURN_PEOPLE_SNAPSHOT"])) {
        return "";
    }

    return normalizePeoplePipeList(parsePeoplePipeList($GLOBALS["DIALECTIC_TURN_PEOPLE_SNAPSHOT"]));
}
...
function dialecticActorStatusSuffixPattern()
{
...
function stripActorStateSuffix($name)
{
    ...
    $name = trim($name, "|");
    $name = preg_replace(dialecticActorStatusSuffixPattern(), '', $name);
...
function buildDialogueTargetSuffix($listenerName, $isSpeakingLoudly = false)
{
    ...
    if (isShoutExecutionMode()) {
        return "(shouting to {$listenerName})";
    }
...
function dialecticGetRechatMode()
{
...
{
    return dialecticGetRechatMode() !== "tight";
}

function dialecticBuildRechatModeSessionKey(array $members)
{
    $members = dialecticNormalizeRechatActorList($members);
    sort($members, SORT_NATURAL | SORT_FLAG_CASE);

    if (empty($members)) {
        $members[] = trim((string)($GLOBALS["DIALECTIC_NAME"] ?? "rechat"));
    }

    return md5(implode("|", $members) . "_" . floor(time() / 120));
}

function dialecticResolveEffectiveRechatMode($configuredMode, array $members)
{
    ...
    $sessionKey = dialecticBuildRechatModeSessionKey($members);
    $modeFile = sys_get_temp_dir() . "/dialectic_rechat_mode_" . $sessionKey . ".json";
...
function dialecticGetStrictRechatListenerName()
{
    ...
    if (
        ...
        !in_array(($GLOBALS["gameRequest"][0] ?? ""), ["rechat", "continue"], true)
    ) {
        return "";
    }
...
function dialecticGetStrictDirectedPlayerListenerName()
{
    if (empty($GLOBALS["ENFORCE_STRICT_RECHAT_RESPONSE"])) {
        return "";
    }
    ...
    if (!in_array(($GLOBALS["gameRequest"][0] ?? ""), ["inputtext", "inputtext_s"], true)) {
        return "";
    }
...
function dialecticNormalizeRechatActorList(array $names)
{
    $normalized = [];
...
function dialecticExtractPeopleListFromPipeString($peoplePipe)
{
    $peoplePipe = trim((string)$peoplePipe);
    if ($peoplePipe === "") {
        return [];
    ...
    return dialecticNormalizeRechatActorList($names);
}

function dialecticRechatCollectStructuredClassifiers($value, &$parts)
{
    static $classifierKeys = [
        "race" => true,
        "species" => true,
        "category" => true,
        "type" => true,
    ...
    if (is_string($value)) {
        ...
        if (is_array($decoded)) {
            dialecticRechatCollectStructuredClassifiers($decoded, $parts);
        }
        return;
    }
    ...
    foreach ($value as $key => $fieldValue) {
        $fieldKey = strtolower((string)$key);
        if (isset($classifierKeys[$fieldKey])) {
            if (is_scalar($fieldValue)) {
                $fieldText = trim((string)$fieldValue);
                if ($fieldText !== "") {
                    $parts[] = $fieldText;
                }
            } elseif (is_array($fieldValue)) {
                dialecticRechatCollectStructuredClassifiers($fieldValue, $parts);
            }
            continue;
        }
...
function dialecticRechatClassifierContains($text, $term)
{
...
function dialecticRechatActorBlockReason($actorName, $npcData = null)
{
    $actorLabel = strtolower(normalizeDialogueListenerName($actorName));
    if (in_array($actorLabel, ["rex", "dogmeat"], true)) {
        return "creature/animal category is not allowed for rechat: " . $actorLabel;
    }

    $parts = [];
    if (is_array($npcData)) {
        foreach (["race", "voiceid", "base", "refid"] as $key) {
            if (!empty($npcData[$key]) && is_scalar($npcData[$key])) {
                $parts[] = (string)$npcData[$key];
            }
        }
        foreach (["metadata", "extended_data", "appearance"] as $key) {
            if (!empty($npcData[$key])) {
                dialecticRechatCollectStructuredClassifiers($npcData[$key], $parts);
            }
        }
    }

    $text = strtolower(implode(" ", array_filter($parts, static function ($part) {
    ...
    foreach ($blockedTerms as $term) {
        if (dialecticRechatClassifierContains($text, $term)) {
            return "creature/animal category is not allowed for rechat: " . $term;
        }
    }

    if (dialecticRechatClassifierContains($text, "creature") &&
        !dialecticRechatClassifierContains($text, "robot") &&
        !dialecticRechatClassifierContains($text, "protectron") &&
        !dialecticRechatClassifierContains($text, "securitron") &&
        !dialecticRechatClassifierContains($text, "mister handy") &&
        !dialecticRechatClassifierContains($text, "mr handy") &&
        !dialecticRechatClassifierContains($text, "mister gutsy") &&
        !dialecticRechatClassifierContains($text, "mr gutsy") &&
        !dialecticRechatClassifierContains($text, "eyebot") &&
...
function dialecticParseServerSideRechatPayload($rawData)
{
    ...
    foreach (["speaker_formid", "listener_formid", "target_formid"] as $formIdKey) {
        if (!empty($decoded[$formIdKey])) {
            $payload[$formIdKey] = trim((string)$decoded[$formIdKey]);
        }
    }

    $appendActorNames = function ($value, array &$names) use (&$appendActorNames) {
        if (is_string($value)) {
            foreach (parsePeoplePipeList($value) as $name) {
                $name = normalizeDialogueListenerName($name);
                if ($name !== "") {
                    $names[] = $name;
                }
        ...
        foreach ($value as $item) {
            if (is_string($item)) {
                $name = normalizeDialogueListenerName($item);
                if ($name !== "") {
                    $names[] = $name;
                }
                ...
                foreach (["name", "actor_name", "display_name"] as $key) {
                    if (!empty($item[$key])) {
                        $name = normalizeDialogueListenerName($item[$key]);
                        if ($name !== "") {
                            $names[] = $name;
                        }
                        break;
                ...
                $appendActorNames($item, $names);
            }
        }
    };

    $audienceNames = [];
    if (!empty($decoded["audience"])) {
        $appendActorNames($decoded["audience"], $audienceNames);
    }
    if (!empty($decoded["audience_snapshot"]) && is_array($decoded["audience_snapshot"])) {
        if (!empty($decoded["audience_snapshot"]["people"])) {
            $appendActorNames($decoded["audience_snapshot"]["people"], $audienceNames);
        }
        if (!empty($decoded["audience_snapshot"]["companions"])) {
            $appendActorNames($decoded["audience_snapshot"]["companions"], $audienceNames);
        }
    }

    $chainMemberNames = [];
    if (!empty($decoded["chain_members"])) {
        $appendActorNames($decoded["chain_members"], $chainMemberNames);
    }

    $payload["audience"] = dialecticNormalizeRechatActorList($audienceNames);
    $payload["chain_members"] = dialecticNormalizeRechatActorList($chainMemberNames);

    return $payload;
}

function dialecticLatestNearbyActorStatusMapForRechat(): array
{
    $db = $GLOBALS["db"] ?? null;
    ...
    $rows = $db->fetchAll("SELECT party, localts, ts FROM eventlog WHERE type='nearby_actors' AND COALESCE(party, '') <> '' ORDER BY gamets DESC, ts DESC LIMIT 1 OFFSET 0");
    if (!is_array($rows) || empty($rows)) {
        return [];
    }

    $eventLocalTs = intval($rows[0]["localts"] ?? 0);
    if ($eventLocalTs <= 0) {
        $eventLocalTs = intval($rows[0]["ts"] ?? 0);
    }
    $ageSeconds = $eventLocalTs > 0 ? max(0, time() - $eventLocalTs) : PHP_INT_MAX;
    $fresh = $ageSeconds <= 12;

    $payload = json_decode($rows[0]["party"] ?? "", true);
    if (!is_array($payload) || !is_array($payload["actors"] ?? null)) {
        return [
            "__snapshot_present" => true,
            "__snapshot_fresh" => false,
            "__snapshot_age_seconds" => $ageSeconds,
        ];
    }

    $map = [
        "__snapshot_present" => true,
        "__snapshot_fresh" => $fresh,
        "__snapshot_age_seconds" => $ageSeconds,
    ];
    foreach ($payload["actors"] as $actor) {
        ...
        $name = normalizeDialogueListenerName($actor["name"] ?? "");
        ...
        $map[strtolower($name)] = [
            ...
            "can_hear_player" => filter_var($actor["can_hear_player"] ?? false, FILTER_VALIDATE_BOOLEAN),
        ];
    }
...
function dialecticRechatNearbyActorBlockReason(
    string $candidate,
    array $nearbyStatusMap,
    bool $requirePresence = true,
    bool $blockOnStale = true
): string
{
    if (empty($nearbyStatusMap)) {
        return "";
    }

    if (isset($nearbyStatusMap["__snapshot_fresh"]) && !$nearbyStatusMap["__snapshot_fresh"]) {
        if (!$blockOnStale) {
            return "";
        }
        $age = intval($nearbyStatusMap["__snapshot_age_seconds"] ?? 0);
        return "latest nearby actor scene is stale ({$age}s old)";
    }

    $key = strtolower(normalizeDialogueListenerName($candidate));
    if ($key === "" || !isset($nearbyStatusMap[$key])) {
        if (!$requirePresence) {
            return "";
        }
        return "not present in latest nearby actor scene";
    }

    $state = $nearbyStatusMap[$key];
    if (empty($state["eligible"])) {
        return "latest nearby actor snapshot marks actor ineligible";
    }
    ...
    if (!empty($state["can_hear_player"]) ||
        strpos($status, "can hear you") === 0 ||
        $reason === "line_of_sight_clear" ||
        $reason === "path_fallback_clear" ||
        $reason === "distance_fallback_clear" ||
        $reason === "immediate_proximity" ||
        $reason === "open_door_muffled" ||
        $reason === "line_of_sight_blocked_muffled") {
        return "";
    }
    ...
    foreach ($blockedTokens as $token) {
        if ($token !== "" && strpos($combined, $token) !== false) {
            return "latest nearby actor status is {$combined}";
        }
    }
...
function dialecticResolveServerSideRechatTarget(array $payload)
{
    $speakerName = normalizeDialogueListenerName($payload["speaker"] ?? ($GLOBALS["DIALECTIC_NAME"] ?? ""));
    ...
    $configuredRechatMode = dialecticGetRechatMode();

    $peoplePipe = "";
    $payloadAudience = dialecticNormalizeRechatActorList($payload["audience"] ?? []);
    $hasAuthoritativePayloadAudience = !empty($payloadAudience);
    if ($hasAuthoritativePayloadAudience) {
        $audience = $payloadAudience;
        $peoplePipe = normalizePeoplePipeList($audience);
        Logger::info("[RECHAT_SELECT] Using authoritative payload audience: " . $peoplePipe);
    } else {
        ...
        $audience = dialecticExtractPeopleListFromPipeString($peoplePipe);
    }

    $rechatMode = dialecticResolveEffectiveRechatMode($configuredRechatMode, array_merge(
    ...
    $candidates = dialecticNormalizeRechatActorList($candidates);
    $npcMaster = new NpcMaster();
    $selected = "";
    ...
    foreach ($candidates as $candidate) {
        ...
        $candidateNpcData = $npcMaster->getByName($candidate);
        if (!$candidateNpcData) {
            continue;
        }
        ...
        $nearbyBlockReason = dialecticRechatNearbyActorBlockReason(
            $candidate,
            $nearbyStatusMap,
            !$hasAuthoritativePayloadAudience,
            !$hasAuthoritativePayloadAudience
        );
        if ($nearbyBlockReason !== "") {
            Logger::info("[RECHAT_SELECT] Skipping {$candidate}: {$nearbyBlockReason}");
            continue;
        }
    ...
    Logger::info("[RECHAT_SELECT] speaker={$speakerName} listener={$listenerHint} target={$rechatTargetHint} " .
        "mode={$rechatMode} audience=" . implode(",", $audience) .
...
function dialecticBuildServerSideRechatSessionKey(array $resolvedTarget)
{
    ...
    if (empty($members)) {
        $members[] = trim((string)($resolvedTarget["speaker"] ?? $GLOBALS["DIALECTIC_NAME"] ?? "rechat"));
    }

    return md5("members_" . implode("|", $members));
}

function dialecticSwitchActiveNpcProfile($npcName)
{
    ...
    if (function_exists('dialecticRuntimeSetActiveProfile')) {
        dialecticRuntimeSetActiveProfile(md5($npcName));
    } else {
...
function dialecticGetVisibleChatDeliveryStates()
{
    return ['emitted', 'spoken'];
}

function dialecticGetInFlightChatDeliveryStates()
{
    return ['pending', 'emitted'];
}

function dialecticBuildChatDeliveryStateSql($column = 'delivery_state', array $states = null, $defaultState = 'spoken')
{
    ...
    if ($states === null) {
        $states = dialecticGetVisibleChatDeliveryStates();
    }
    ...
    $defaultState = trim((string)$defaultState);
    if ($defaultState === '') {
        $defaultState = 'spoken';
    }
    ...
    return "COALESCE({$column}, '{$defaultState}') IN (" . implode(', ', $quotedStates) . ")";
}
...
function isNarratorPrivateActionName($actionName)
{
    ...
    if (function_exists('dialecticFindActionCatalogRowByNameOrCode')) {
        $row = dialecticFindActionCatalogRowByNameOrCode($actionName, false);
        if (is_array($row)) {
...
function extractNarratorPrivateActionTargetName($eventType, $eventData)
{
    ...
    if ($eventType === "funcret") {
        $payload = json_decode($eventData, true);
        if (is_array($payload) && isNarratorPrivateActionName($payload["action"] ?? ($payload["command"] ?? ""))) {
            return trim((string)($payload["target"] ?? ($payload["arg"] ?? "")));
        }
        return "";
    }
...
function shouldScopeNarratorPrivateActionEvent($eventType, $eventData, $listenerName = "")
{
    ...
    if ($eventType === "funcret") {
        $payload = json_decode($eventData, true);
        return is_array($payload) && isNarratorPrivateActionName($payload["action"] ?? ($payload["command"] ?? ""));
    }
...
function lookupConversationPeopleSourceOfTruth($speakerName, $targetName, $maxAgeSeconds = 300)
{
    ...
    $rows = $db->fetchAll(
        "SELECT rowid, type, data, people
         FROM eventlog
         WHERE localts > {$cutoff}
           AND type IN ('chat','prechat','inputtext','inputtext_s','narrator_inputtext')
    ...
    foreach ($rows as $row) {
        ...
        if (in_array($rowType, ["inputtext", "inputtext_s", "narrator_inputtext"], true)) {
...
function dialecticGenerateUtteranceId()
{
    try {
...
function logEvent($dataArray,$forcePeople='')
{
    ...
    if (!isset($dataArray)) { // function called without parameter values
        ...
        if (!$hasForcedPeople) {
            $turnPeopleSnapshot = dialecticGetCurrentTurnPeopleSnapshot();
...
function selectRandomInArray($arraySource)
{
    ...
    if ($n>0) {
        if ($n==1) {
            $s_res = strtr($arraySource[0],["#DIALECTIC_NPC1#"=>$GLOBALS["DIALECTIC_NAME"]]);
        } else {
            $s_res = strtr($arraySource[rand(0, $n-1)],["#DIALECTIC_NPC1#"=>$GLOBALS["DIALECTIC_NAME"]]);
        }
