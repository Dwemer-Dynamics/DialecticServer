...
function initializeLogTrimming($maxLines = 10000) {
    try {
        ...
        $enabledRow = @$db->fetchOne("SELECT value FROM dialectic_meta.settings WHERE key='AUTO_TRIM_LOGS_ENABLED'");
        $enabled = true;
        if (is_array($enabledRow) && isset($enabledRow['value'])) {
            $enabled = in_array(strtolower(trim((string)$enabledRow['value'])), ['true','1','yes','on']);
        } else {
            // persist default true
            @$db->insert('dialectic_meta.settings', ['key'=>'AUTO_TRIM_LOGS_ENABLED','value'=>'true']);
            $enabled = true;
        }
        ...
        $lastRunRow = @$db->fetchOne("SELECT value FROM dialectic_meta.settings WHERE key='AUTO_TRIM_LOGS_LAST_RUN'");
        $now = time();
        ...
        @$db->insert('dialectic_meta.settings', ['key'=>'AUTO_TRIM_LOGS_LAST_RUN','value'=> (string)$now]);
        // If already exists, update
        @$db->update('dialectic_meta.settings', "value='".$db->escape((string)$now)."'", "key='AUTO_TRIM_LOGS_LAST_RUN'");
