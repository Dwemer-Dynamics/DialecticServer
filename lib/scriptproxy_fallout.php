...
/**
 * Fallout command builder for Dialectic role commands.
 * Generates JSON payloads consumed by the Dialectic xNVSE plugin.
 * 
 * Usage:
 *   $falloutCmd = new FalloutCommandBuilder();
 *   $json = $falloutCmd->Actor->ForceActorValue("0x00000014", "Health", 100.0);
 *   falloutCmd->send($json); // via SSE-Net, HTTP, etc.
 */

class FalloutCommandBuilder
{
    ...
    public function getLoadOrderESP(): string{
        $hexFormId = $GLOBALS["db"]->fetchOne("select value from conf_opts where id='dialectic_rolemaster_faction'");
        if (!$hexFormId ) {
            throw new Exception("Configuration 'dialectic_rolemaster_faction' not set.");
        }
    ...
    public function send($cmd,$localts = null) {
        ...
        $GLOBALS["db"]->insert(
            ...
            [
                ...
                'actor'   => "rolemaster",
                'text'    => "",
                'action'  => dialecticEncodeCommandAction('ScriptProxy', ['payload' => $strJson]),
                'tag'     => '',
            ]
        );
    }
...
// Use 
// $refId = "0x00000014"; // ? 20
// $cmd = $fallout->Actor->ForceActorValue($refId, "Health", 100.0);
