...
function processNearbyDiary($gameRequest, $eventType) {
    ...
    dialectic_buffer_command_response_line(
        "The Narrator",
        "DebugNotification",
        ["message" => "Checking for nearby NPCs within 400 units..."]
    );
}
...
function generateNearbyDiary($npcName, $gameRequest, $eventType) {
    ...
    $defaultDiaryConnector = function_exists('dialecticResolveDiaryConnectorName') ? dialecticResolveDiaryConnectorName() : null;
    ...
    try {
        $NPC_CONF = [
            "DIALECTIC_NAME" => $npcName,
            "PLAYER_NAME" => $GLOBALS["PLAYER_NAME"],
            "DIALECTIC_PERS" => "An NPC in the world of Fallout.",
            "PROMPT_HEAD" => isset($GLOBALS["PROMPT_HEAD"]) ? $GLOBALS["PROMPT_HEAD"] : "You are an NPC in the world of Fallout.",
        ...
        if ($npcName === "The Narrator") {
            ...
            $profileData = $narrator->getNarratorData();
        } else {
            require_once(__DIR__ . "/core/npc_master.class.php");
            $npcMaster = new NpcMaster();
            $profileData = $npcMaster->getByName($npcName);
        }

        if (is_array($profileData) && !empty($profileData)) {
            $NPC_CONF["DIALECTIC_PERS"] = $profileData["core"] ?? $profileData["npc_static_bio"] ?? $NPC_CONF["DIALECTIC_PERS"];
            $NPC_CONF["PROMPT_HEAD"] = $profileData["prompt_head"] ?? $NPC_CONF["PROMPT_HEAD"];
            $NPC_CONF["DIALECTIC_BACKGROUND"] = $profileData["npc_static_bio"] ?? "";
            $NPC_CONF["DIALECTIC_PERSONALITY"] = $profileData["personality"] ?? "";
            $NPC_CONF["DIALECTIC_APPEARANCE"] = $profileData["appearance"] ?? "";
            $NPC_CONF["DIALECTIC_OCCUPATION"] = $profileData["occupation"] ?? "";
            $NPC_CONF["DIALECTIC_SKILLS"] = $profileData["skills"] ?? "";
            $NPC_CONF["DIALECTIC_SPEECHSTYLE"] = $profileData["speechstyle"] ?? "";
            $NPC_CONF["DIALECTIC_GOALS"] = $profileData["goals"] ?? "";
            Logger::info("DIARY_NEARBY: Loaded database profile for $npcName");
        } else {
        ...
        $resolvedDiaryConnector = function_exists('dialecticResolveDiaryConnectorName')
            ? dialecticResolveDiaryConnectorName($NPC_CONF["CONNECTORS_DIARY"] ?? $defaultDiaryConnector)
        ...
        $contextDataHistoric = DataLastDataExpandedFor("{$NPC_CONF["DIALECTIC_NAME"]}", $lastNDataForContext * -1, $sqlfilter);
        $historyData = "";
        ...
        $diaryPrompt = strtr($GLOBALS["DIARY_PROMPT"], ['#DIALECTIC_NAME#'=>$npcName,'#PLAYER_NAME#'=>$NPC_CONF["PLAYER_NAME"]]);
        ...
        // Prefer connector's configured max_tokens; fallback to memory max token config; then a sane default
        $maxTokens = null;
...
function generatePlayerDiary($gameRequest, $eventType)
{
    ...
    $systemParts = [
        "You are writing {$playerName}'s private diary in the world of Fallout.",
    ...
    $sqlfilter = " and type<>'prechat' and type<>'itemfound' and type<>'infoaction' ";
    ...
    try {
        ...
        if (!empty($currentConnectorData)) {
            ...
            unset($GLOBALS["DIALECTIC_CORE_CURRENT_CONNECTOR_DATA"]["stop"]);
            ...
            $diaryConnector = function_exists('dialecticResolveDiaryConnectorName') ? dialecticResolveDiaryConnectorName() : null;
            if ($diaryConnector === null) {
                Logger::warn("PLAYER_DIARY: No diary connector configured");
                return false;
            }

            Logger::warn("PLAYER_DIARY: Falling back to configured diary connector '{$diaryConnector}'");
    ...
    if (!empty(trim($buffer))) {
        ...
        dialectic_buffer_command_response_line($playerName, "DebugNotification", [
            "message" => "Diary Entry Written for " . $playerName,
        ]);
...
function generateFollowerDiary($followerName, $gameRequest, $eventType) {
    ...
    // Determine max tokens: prefer DB connector setting; fallback to driver config; then memory config; then default
    $maxTokens = null;
    if (isset($currentConnectorData["max_tokens"]) && $currentConnectorData["max_tokens"] !== null && $currentConnectorData["max_tokens"] !== '') {
        $maxTokens = (int)$currentConnectorData["max_tokens"];
    } elseif (isset($GLOBALS["CONNECTOR"][$currentConnectorData["driver"]]["max_tokens"])) {
    ...
    if (!empty(trim($buffer))) {
        ...
        dialectic_buffer_command_response_line($followerName, "DebugNotification", [
            "message" => "Diary Entry Written for " . $followerName,
        ]);
...
function updateDynamicProfileField($npcName, $field, $historyData) {
    // Map field names to their corresponding DIALECTIC prompts
    $fieldMapping = [
