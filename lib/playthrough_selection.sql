...
CREATE OR REPLACE FUNCTION dialectic_meta.is_global_setting(table_name text, setting_id text)
RETURNS boolean AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION dialectic_meta.validate_save_manifest(source_schema text, actual_tables text[])
RETURNS void AS $$
...
BEGIN
    ...
    manifest := raw::jsonb;
    IF manifest->>'format' NOT IN ('dialectic_selected_tables_v1','dialectic_selected_tables_v2')
    ...
    END IF;
    IF manifest->>'format'='dialectic_selected_tables_v2' AND
...
CREATE OR REPLACE FUNCTION dialectic_meta.playthrough_identity(source_schema text)
RETURNS jsonb AS $$
...
CREATE OR REPLACE FUNCTION dialectic_meta.capture_playthrough(dest_schema text, selected_tables text[])
RETURNS void AS $$
DECLARE
    names text[];
    lock_list text;
    ...
    table_name text;
BEGIN
    IF dest_schema !~ '^dialectic_profile_[a-z0-9_]+$' THEN
    ...
    IF NOT dialectic_meta.drop_schema_safe(dest_schema) THEN
        RAISE EXCEPTION 'Cannot replace playthrough schema';
    END IF;
    PERFORM dialectic_meta.clone_selected_schema('public', dest_schema, names);
    IF EXISTS (
    ...
    FOREACH table_name IN ARRAY ARRAY['conf_opts','general_settings'] LOOP
        IF table_name=ANY(names) THEN
            EXECUTE format('DELETE FROM %I.%I WHERE dialectic_meta.is_global_setting($1,id)',dest_schema,table_name) USING table_name;
        END IF;
    END LOOP;
    EXECUTE format('COMMENT ON SCHEMA %I IS %L', dest_schema,
        jsonb_build_object('format','dialectic_selected_tables_v2','table_policy_version',3,
            'tables',names,'migrations',versions,'upgrade_version',2,
            'player_identity',dialectic_meta.playthrough_identity(dest_schema))::text);
END;
$$ LANGUAGE plpgsql SET lock_timeout = '10s';

-- Restore rows in place, keeping excluded tables, views, triggers and table identities.
CREATE OR REPLACE FUNCTION dialectic_meta.restore_playthrough(source_schema text, selected_tables text[])
RETURNS void AS $$
DECLARE
    names text[];
    source_names text[];
...
BEGIN
    IF source_schema !~ '^dialectic_profile_[a-z0-9_]+$' THEN
        RAISE EXCEPTION 'Invalid playthrough schema';
    END IF;
    ...
    END IF;
    PERFORM dialectic_meta.validate_save_manifest(source_schema,
    ...
    FOREACH lock_list IN ARRAY names LOOP
        ...
        row_filter := CASE WHEN lock_list IN ('conf_opts','general_settings')
            THEN format(' WHERE NOT dialectic_meta.is_global_setting(%L,id)',lock_list) ELSE '' END;
...
CREATE OR REPLACE FUNCTION dialectic_meta.sync_playthrough_comments(selected_tables text[])
RETURNS void AS $$
DECLARE item record;
BEGIN
    FOR item IN
