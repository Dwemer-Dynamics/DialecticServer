...
function pas_select(array $rows, string $character, string $name, int $gamets, bool $allowLegacy = true, array $links = []): ?array {
    ...
    if ($character === '') {
        // Old Fallout saves may lack our co-save ID; honor an existing unique name association.
...
function pas_invalidate($conn): void {
    ...
    $session['message'] = 'Reload your Fallout save to connect its character.';
    unset($session['token']);
...
function pas_guard($conn, bool $required = false): void {
    ...
    $token = $_SERVER['HTTP_X_DIALECTIC_PLAYTHROUGH'] ?? '';
    ...
    echo json_encode(['ok'=>false,'message'=>'DIALECTIC is waiting for the loaded character. Open Playthrough Saves.']);
    exit;
}
...
function pas_http_guard(bool $required = false): void {
    if (PHP_SAPI === 'cli' || (!$required && !isset($_SERVER['HTTP_X_DIALECTIC_PLAYTHROUGH']))) return;
    ptr_runtime_enter();
    $conn = ptp_connect();
...
function pas_manual($conn, array $input): array {
    ...
    try {
        ...
        return array_replace($result, ['message'=>$ready ? 'Character associated. Reload the Fallout save to resume DIALECTIC.' : 'Character associated. Restart the mod server before reloading Fallout.']);
...
function pas_handshake($conn, array $input): array {
    ...
    if ($name === '' || !preg_match('//u', $name) || mb_strlen($name) > 80 || preg_match('/[\x00-\x1f\x7f]/u', $name)
        ...
        return ['ok'=>false,'status'=>'pending','message'=>'Waiting for your Fallout character name. Reload the save after naming your character.'];
    }
    $runtime = ptr_runtime_begin_switch(30.0, $conn);
    try {
        ...
        if (in_array($input['client_id'], $retired, true)) throw new RuntimeException('This Fallout session was replaced.');
        ...
        $session += ['player_name'=>$name, 'status'=>'pending', 'message'=>'Choose the loaded character’s Playthrough Save, then reload the Fallout save.'];
        ...
        if (!$state['active_id']) {
            $live = json_decode(pg_fetch_result(pth_query($conn, "SELECT dialectic_meta.playthrough_identity('public')"),0,0),true);
        ...
        if (!$target) {
            ...
            if ($target['id'] === 0) {
                $label = $name; $suffix = 2;
                while (pg_num_rows(pth_query($conn, "SELECT id FROM dialectic_meta.playthrough_profiles WHERE lower(name)=lower($1)", [$label]))) {
                    $label = $name . ' (' . $suffix++ . ')';
                }
                $target['id'] = (int)pth_capture($conn, $label)['id'];
                pth_query($conn, "UPDATE dialectic_meta.playthrough_profiles SET is_active=true,retention_pinned=true WHERE id=$1", [$target['id']]);
                ptr_write($conn, 'PLAYTHROUGH_HOME_REVISION', bin2hex(random_bytes(16)));
            }
        ...
        if (!$ready) {
            ...
            $session['message'] = 'Playthrough restored. Restart the mod server before reloading Fallout.';
            ptr_write($conn, 'PLAYTHROUGH_SESSION', $session);
        }
