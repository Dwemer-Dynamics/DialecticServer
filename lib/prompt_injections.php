...
function dialecticNormalizePromptInjectionKey(string $value): string
{
...
function dialecticRegisterPromptInjection(string $slot, string $id, $content, int $priority = 100): bool
{
    $slot = dialecticNormalizePromptInjectionKey($slot);
    $id = dialecticNormalizePromptInjectionKey($id);
    if ($slot === '' || $id === '') {
        return false;
    }
...
function dialecticPromptInjectionContentToText($content, string $slot, array $context = []): string
{
...
function dialecticRenderPromptInjections(string $slot, array $context = []): string
{
    $slot = dialecticNormalizePromptInjectionKey($slot);
    ...
    foreach ($entries as $entry) {
        $content = dialecticPromptInjectionContentToText($entry['content'] ?? '', $slot, $context);
        if ($content !== '') {
            $parts[] = $content;
        }
    }
...
function dialecticRegisterActorProfileEnricher(string $id, callable $callback, int $priority = 100): bool
{
    $id = dialecticNormalizePromptInjectionKey($id);
    if ($id === '') {
        return false;
...
function dialecticBuildActorProfileEnrichmentText(string $actorName, string $actorType, array $context = []): string
{
    ...
    foreach ($enrichers as $key => $enricher) {
        if (is_array($enricher) && isset($enricher['callback'])) {
            ...
            $entries[] = [
                'id' => is_string($key) ? $key : 'entry_' . count($entries),
                'callback' => $enricher,
                'priority' => 100,
            ];
        }
