...
if ($daemon && function_exists('pcntl_fork')) {
    ...
    @file_put_contents($logFile, date('[Y-m-d H:i:s]') . " TRACE: Daemon mode without pcntl, PID=" . getmypid() . "\n", FILE_APPEND);
}
...
// Bootstrap the Dialectic environment
@file_put_contents($logFile, date('[Y-m-d H:i:s]') . " TRACE: Before bootstrap\n", FILE_APPEND);
$GLOBALS['ENGINE_PATH'] = $enginePath;

try {
    @file_put_contents($logFile, date('[Y-m-d H:i:s]') . " TRACE: Loading runtime bootstrap\n", FILE_APPEND);
