...
$_relEntryNpcName = $GLOBALS["DIALECTIC_NAME"] ?? 'NULL';
Logger::debug("[REL-ENTRY] postrequest.php loaded for DIALECTIC_NAME='{$_relEntryNpcName}'");
...
// Priority: Dialectic profile data > currentNpcData > DIALECTIC_NAME fallback
$npcName = null;
$npcNameSource = 'none';

if (isset($GLOBALS["DIALECTIC_CORE_CURRENT_NPC_DATA"]["npc_name"])) {
    $npcName = $GLOBALS["DIALECTIC_CORE_CURRENT_NPC_DATA"]["npc_name"];
    $npcNameSource = 'DIALECTIC_CORE_CURRENT_NPC_DATA';
} elseif (isset($currentNpcData["npc_name"])) {
    $npcName = $currentNpcData["npc_name"];
    $npcNameSource = 'currentNpcData';
} elseif (!empty($GLOBALS["DIALECTIC_NAME"])) {
    $npcName = $GLOBALS["DIALECTIC_NAME"];
    $npcNameSource = 'DIALECTIC_NAME';
}
...
// frequency gate before listener resolution, database reads, and context assembly.
...
// Get NPC ID
$npcId = null;
if (!empty($GLOBALS["DIALECTIC_CORE_CURRENT_NPC_DATA"]["id"])) {
    $npcId = intval($GLOBALS["DIALECTIC_CORE_CURRENT_NPC_DATA"]["id"]);
} elseif (isset($currentNpcData["id"]) && intval($currentNpcData["id"]) > 0) {
    $npcId = intval($currentNpcData["id"]);
} elseif (!empty($GLOBALS["DIALECTIC_ID"])) {
    $npcId = intval($GLOBALS["DIALECTIC_ID"]);
} else {
    // Look up by name
    $escapedName = $GLOBALS["db"]->escape($npcName);
    $npcRow = $GLOBALS["db"]->fetchOne(
...
if ($useRelLLM && $npcId) {
    ...
    // inputtext = player spoke, other request types = NPC-initiated (Board Chat, etc.)
    $playerInputTypes = ["inputtext", "inputtext_s"];
    ...
    if (!empty($GLOBALS["CACHE_PEOPLE"])) {
        $context['nearby_npcs'] = _relSplitPeopleList($GLOBALS["CACHE_PEOPLE"]);
    }
...
if (!$useRelLLM && !empty($GLOBALS["talkedSoFar"])) {
    dialecticRelationshipProcessInlineCompletedResponse(
        $GLOBALS["talkedSoFar"],
        (string)$npcName
    );
}
