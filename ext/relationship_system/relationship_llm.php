...
class RelationshipLLM {
    ...
    private function getAnalysisPrompt($playerName) {
        $fallback = <<<'PROMPT'
You are a relationship analyzer for Fallout New Vegas NPCs. Analyze relationship descriptions and output JSON.
...
PROMPT;
    ...
    private function parseResponse($response) {
        ...
        foreach ($parsed['relationships'] as $target => $data) {
            ...
            if (in_array($targetLower, ['player', 'the player', 'Courier', 'the Courier', '#player_name#'])) {
                $target = 'Player';
            }
...
PROMPT;
    ...
    private function applyChanges($npcId, $changes, $currentRels) {
        ...
        // Factions/groups are allowed (NCR, Caesar's Legion, Followers, etc.)
        $blockedTitles = [
            'Courier', 'the Courier',
            'legate', 'the legate',
            'hero', 'the hero',
        ...
        foreach ($changes as $target => $change) {
            ...
            if ($targetLower === strtolower($playerName) ||
                in_array($targetLower, ['player', 'the player', 'Courier', 'the Courier', '#player_name#'])) {
                $target = 'Player';
            }
