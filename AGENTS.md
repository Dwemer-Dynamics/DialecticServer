...
### DialecticServer specifics

- DIALECTIC stores the listed `worldknowledge` tables and `quests` with the playthrough. Biography templates, connector/profile setup and the listed reusable libraries remain global. Preserve product-specific integrations and unmanaged tables; do not import CHIM or STOBE table names by analogy.
