...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_individual'])) {
 ...
 DO UPDATE SET
 core = EXCLUDED.core,
 worldknowledge_tags = EXCLUDED.worldknowledge_tags,
...
//
//
// CSV UPLOAD - Safe NPC biography import
//
//
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_csv'])) {
 ...
 // Process CSV through the UI-owned import path
 $processedCount = 0;
 $errorCount = 0;
 ...
 // Order: npc_name, core, worldknowledge_tags, npc_static_bio, personality,
 ...
 $worldknowledge_tags = $getValue(2) ?? '';
 ...
 $toastMessage = " No NPC records were imported";
 }
 ...
 if (!empty($errors)) {
 $message .= '<details style="margin-top:10px;"><summary style="cursor:pointer; color:#ff6464;"> '
...
?><link rel="stylesheet" href="<?php echo $webRoot; ?>/ui/css/main.css"><style>
 ...
 .table-container th:nth-child(5), /* Knowledge Tags */
...
</style><?php endif; ?><main><div id="toast" class="toast-notification"><span class="message"></span></div><div class="page-header"><h1>NPC Biography Management
 </h1><p class="page-subtitle">Create custom character profiles for AI NPCs during roleplay</p></div><div class="indent5"><div class="content-section"><h1>Batch Upload</h1><form action="" method="post" enctype="multipart/form-data"><h3><strong>Please use underscores instead of spaces.</strong></h3><h4>Example: Doc Mitchell becomes doc_mitchell</h4><div><label for="csv_file">Select .csv file to upload:</label><br><input type="file" name="csv_file" id="csv_file" accept=".csv" required></div><div class="button-group"><input type="submit" name="submit_csv" value="Upload CSV" class="action-button upload-csv"><a href="?action=download_example" class="action-button download-csv">Download Example CSV</a><a href="?action=export_custom_npcs" class="action-button" style="background: rgba(255, 182, 65, 0.8);">Export Custom NPCs</a></div><p><strong>Relationships column:</strong> Use a JSON object seed such as <code>{"Player":{"aff":25,"type":"professional"}}</code>. Prose no longer seeds the relationship affinity system.</p><p>You can verify that NPC data has been uploaded successfully by going to
 <b>Server Actions -> Database Manager -> configured DIALECTIC database -> public -> bio_templates_custom</b>.</p><p>All uploaded biographies will be saved into the <code>bio_templates_custom</code> table. This overwrites any entries in the regular table.</p><p>Also you can check the merged view at
 <b>Server Actions -> Database Manager -> configured DIALECTIC database -> public -> Views (Top bar) -> combined_bio_templates</b>.</p><p><strong>Export Custom NPCs:</strong> Download all your custom NPC entries as a CSV file for backup or sharing purposes. The exported file will include all custom entries with their extended profiles and voice overrides.</p></form><form action="" method="post"><input
 type="submit"
 name="truncate_npc"
 value="Factory Reset NPC Override Table"
 class="btn-danger"
 onclick="return confirm('Are you sure you want to DELETE ALL ENTRIES in bio_templates_custom? This action is IRREVERSIBLE!');"
 ></form><p>This will just delete any custom NPC entires you have uploaded.</p><p>You can download a backup of the full character database in the
 ...
 // Knowledge Tags (npc_misc) column
 $worldknowledgeTagsValue = $row['worldknowledge_tags'] ?? '';
 echo ' <td style="font-size: 1.5em; line-height: 1.4;">';
 if (!empty(trim($worldknowledgeTagsValue))) {
 // Split by commas and display as badges/tags
 $tags = array_map('trim', explode(',', $worldknowledgeTagsValue));
 foreach ($tags as $tag) {
 if (!empty($tag)) {
 echo '<span style="display: inline-block; background: rgba(255, 182, 65, 0.2); color: rgb(255, 182, 65); padding: 3px 8px; margin: 2px; border-radius: 4px; font-size: 0.85em; font-weight: 500;">' . htmlspecialchars($tag) . '</span>';
 }
 }
 } else {
