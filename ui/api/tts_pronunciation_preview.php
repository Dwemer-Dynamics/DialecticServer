...
function dialecticTtsPronunciationPreviewRespond(array $payload, int $status = 200): void
{
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    dialecticTtsPronunciationPreviewRespond(['ok' => false, 'error' => 'Use POST to generate a preview.'], 405);
}
...
$recentRequests = array_values(array_filter(
    is_array($_SESSION['dialectic_tts_pronunciation_previews'] ?? null)
        ? $_SESSION['dialectic_tts_pronunciation_previews']
        : [],
    static fn($timestamp): bool => is_numeric($timestamp) && floatval($timestamp) > $now - 60
));
if (count($recentRequests) >= 30) {
    dialecticTtsPronunciationPreviewRespond(['ok' => false, 'error' => 'Too many previews. Wait a moment and try again.'], 429);
}
$recentRequests[] = $now;
$_SESSION['dialectic_tts_pronunciation_previews'] = $recentRequests;
session_write_close();
...
if ($connectorId <= 0 || $voice === '' || $text === '' || $textLength > 240) {
    dialecticTtsPronunciationPreviewRespond(['ok' => false, 'error' => 'Choose a connector and voice, and preview 240 characters or fewer.'], 400);
}

$previewOptions = dialecticTtsPronunciationPreviewOptions($enginePath);
...
if (!in_array($connectorId, $availableConnectorIds, true)
    ...
    dialecticTtsPronunciationPreviewRespond(['ok' => false, 'error' => 'That connector or installed voice is no longer available.'], 400);
}

$ttsConnector = new TTSConnector();
$connector = $ttsConnector->getById($connectorId);
if (!is_array($connector)) {
    dialecticTtsPronunciationPreviewRespond(['ok' => false, 'error' => 'The selected TTS connector could not be loaded.'], 404);
}
...
try {
    ...
    dialecticTtsPronunciationPreviewRespond([
        'ok' => true,
