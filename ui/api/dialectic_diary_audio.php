...
try {
    ...
    $GLOBALS['DIALECTIC_CORE_CURRENT_NPC_DATA'] = $npcData;
    if ($isNarrator) {
    ...
    $cacheSeed = 'dialectic-diary-audio|' . hash('sha256', implode('|', [
        strval($entryId),
        $content,
        $author,
        $voiceId,
    ...
    $lockPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'dialectic-diary-audio-' . md5($cacheSeed) . '.lock';
