...
$configuration = [];
foreach (['conf.sample.php', 'conf.php'] as $configurationFile) {
    $configurationPath = $enginePath . 'conf' . DIRECTORY_SEPARATOR . $configurationFile;
...
dialecticRuntimeImportConfigVariables($configuration);
...
try {
    ...
    Logger::error('[PLUGIN-PACKAGE] ' . $error->getMessage());
