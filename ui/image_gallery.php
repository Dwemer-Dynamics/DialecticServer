...
?><link rel="stylesheet" href="<?php echo $webRoot; ?>/ui/css/main.css"><style>
 ...
 .gallery-header h1 { margin:0; font-family:'Gothic821', serif; word-spacing:8px; color:rgb(255, 182, 65); font-size:2.2em; text-align:center; text-shadow: 2px 2px 4px rgba(0,0,0,0.5); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['upload_image'])) {
 ...
 $loc = 'image_gallery.php' . (count($qs)?('?'.http_build_query($qs)):'');
 header('Location: ' . $loc);
 exit;
 } else {
...
?><div class="container-fluid"><div class="gallery-header"><h1> Image Gallery</h1><div class="gallery-meta"><?php echo number_format(count($images)); ?> image(s)</div><form method="post" enctype="multipart/form-data" class="upload-inline" style="display:flex; gap:6px; align-items:center; justify-content:center; flex-wrap:wrap; margin-top:6px;"><input type="hidden" name="embed" value="<?= $isEmbed? '1':'' ?>"><input type="hidden" name="picker" value="<?= $isPicker? '1':'' ?>"><input type="file" name="upload_image" accept="image/jpeg,image/png" style="background:#2a2a2a; color:#e9efff; border:1px solid #4a4a4a; border-radius:6px; padding:6px 8px;" /><button type="submit" class="btn">Upload</button><?php if (!empty($uploadError)): ?><span style="color:#ff6b6b; font-size:12px;"><?= htmlspecialchars($uploadError) ?></span><?php endif; ?></form></div><?php if (empty($images)): ?><div class="empty">No images found. Upload pictures here to use them in profile art fields.</div><?php else: ?><div class="grid"><?php foreach ($images as $img): $n = $img['name']; $u = $img['url']; ?><div class="card" data-url="<?php echo htmlspecialchars($u); ?>"><?php if ($img['type']=="mp4") {?><video class="vthumb thumb" controls src="<?php echo htmlspecialchars($u); ?>" alt="<?php echo htmlspecialchars($n); ?>" loading="lazy" ></video><?php } else {?><img class="thumb" src="<?php echo htmlspecialchars($u); ?>" alt="<?php echo htmlspecialchars($n); ?>" loading="lazy" /><?php } ?><div class="info"><div class="name" title="<?php echo htmlspecialchars($img['rel']); ?>"><?php echo htmlspecialchars($n); ?></div><div class="actions"><?php if (!$isPicker): ?><a class="btn" href="<?php echo htmlspecialchars($u); ?>" download>Download</a><?php else: ?><button class="btn pick" type="button" data-pick="<?php echo htmlspecialchars($u); ?>">Use</button><?php endif; ?></div></div></div><?php endforeach; ?></div><?php endif; ?></div><div id="lightbox" class="lightbox"><div class="inner"><div class="tools"><a id="lb_open" href="#" target="_blank" rel="noopener">Open</a><a id="lb_dl" href="#" download>Download</a><div class="dropdown"><button class="btn dropdown-toggle" id="lb_reimagine_toggle" type="button">Reimagine</button><div class="dropdown-content" id="lb_reimagine_menu"><a id="lb_reimage1" href="#" title="Will send image to GPTImage to create a reimagined version, needs an OpenAI API key">GPT Reimagine ($)</a><a id="lb_reimage2" href="#" title="Will send image to Replicate to create a reimagined version, needs a Replicate API key">Replicate Reimagine ($)</a><a id="lb_reimage3" href="#" title="Will send image to OpenRouter to create a reimagined version, needs a OpenRouter API key">OR-Gemini Reimagine ($)</a><a id="lb_reimage4" href="#" title="Will send image to Replicate to create a short video (7secs), needs a Replicate API key">Replicate Animate ($)</a></div></div><button id="lb_close" type="button">Close</button><button id="lb_del" type="button">Delete</button></div><img id="lb_img" src="" alt="preview"><video id="lb_video" src="" alt="preview" controls></video></div></div><script>
(function(){
