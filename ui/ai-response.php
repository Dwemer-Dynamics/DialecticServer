...
function dialecticAiResponseH($value)
{
    return htmlspecialchars((string)$value, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
}

function dialecticAiResponseFormatLocalTs($value)
{
    $ts = intval($value ?? 0);
    if ($ts <= 0) {
        return "";
...
function dialecticAiResponseSafeFetchAll($db, $query)
{
    try {
...
function dialecticAiResponseSafeDeleteLog($db)
{
    try {
        $db->delete("log", "true");
        return true;
...
function dialecticAiResponseNormalizeMarkup($value)
{
    if ($value === null) {
        return "";
    }
...
function dialecticAiResponseNormalizePromptMarkup($value)
{
    if ($value === null) {
        return "";
    }
...
function dialecticAiResponseNormalizeDisplayedText($value)
{
    $value = dialecticAiResponseNormalizeMarkup($value);
    if ($value === "") {
        return "";
    }
...
function dialecticAiResponseDecodeStoredPayload($value)
{
    $clean = dialecticAiResponseNormalizeMarkup($value);
    if ($clean === "") {
        return null;
    }
...
function dialecticAiResponseLooksStructured($value)
{
    $value = ltrim(dialecticAiResponseNormalizeMarkup($value));
    if ($value === "") {
        return true;
    }
...
function dialecticAiResponseExtractSubtitleFromJsonLog($value)
{
    $value = dialecticAiResponseNormalizeMarkup($value);
    if ($value === "") {
        return "";
    ...
    if (isset($payload['lines']) && is_array($payload['lines'])) {
        foreach ($payload['lines'] as $line) {
            ...
            $text = dialecticAiResponseNormalizeDisplayedText((string)($line['subtitle'] ?? $line['text'] ?? ''));
            if ($text !== '') {
                return $text;
            }
        }
    ...
    if (($payload['action'] ?? '') === 'say') {
        return dialecticAiResponseNormalizeDisplayedText((string)($payload['subtitle'] ?? $payload['text'] ?? ''));
    }

    return "";
}

function dialecticAiResponseValueAtPath($payload, array $path)
{
    $cursor = $payload;
...
function dialecticAiResponseExtractTextFromPayload($payload)
{
    if (!is_array($payload)) {
        return "";
    }
    ...
    if (isset($payload['response']) && is_array($payload['response'])) {
        ...
        foreach ($payload['response'] as $responseEntry) {
            if (is_array($responseEntry) && isset($responseEntry['processed'])) {
                $processed = dialecticAiResponseNormalizeDisplayedText($responseEntry['processed']);
                if ($processed !== "") {
                    $parts[] = str_replace('|', "\n", $processed);
                }
            }
    ...
    if (isset($payload['OUTPUT_LOG'])) {
        $subtitle = dialecticAiResponseExtractSubtitleFromJsonLog($payload['OUTPUT_LOG']);
        if ($subtitle !== "") {
            return $subtitle;
        }
    }
    ...
    foreach ($candidatePaths as $path) {
        $candidate = dialecticAiResponseValueAtPath($payload, $path);
        if (is_string($candidate)) {
            $text = dialecticAiResponseNormalizeDisplayedText($candidate);
            if ($text !== "" && !dialecticAiResponseLooksStructured($text)) {
                return $text;
            }
        }
    }
...
function dialecticAiResponseExtractBestText($rawResponseValue, $rawPromptValue = "")
{
    $responseText = dialecticAiResponseNormalizeDisplayedText($rawResponseValue);
    if ($responseText !== "" && !dialecticAiResponseLooksStructured($responseText)) {
        return $responseText;
    }

    $subtitleFromResponse = dialecticAiResponseExtractSubtitleFromJsonLog($rawResponseValue);
    if ($subtitleFromResponse !== "") {
        return $subtitleFromResponse;
    }

    $decodedResponse = dialecticAiResponseDecodeStoredPayload($rawResponseValue);
    if (is_array($decodedResponse)) {
        $decodedText = dialecticAiResponseExtractTextFromPayload($decodedResponse);
        if ($decodedText !== "") {
            return $decodedText;
        }
    }

    $decodedPrompt = dialecticAiResponseDecodeStoredPayload($rawPromptValue);
    if (is_array($decodedPrompt)) {
        $decodedPromptText = dialecticAiResponseExtractTextFromPayload($decodedPrompt);
        if ($decodedPromptText !== "") {
            return $decodedPromptText;
        }
    }

    $subtitleFromPrompt = dialecticAiResponseExtractSubtitleFromJsonLog($rawPromptValue);
...
function dialecticAiResponseDecodePromptExportString($value)
{
    return str_replace(["\\\\", "\\'"], ["\\", "'"], $value);
}

function dialecticAiResponseReadPhpExportQuotedString($text, $startPos)
{
    $length = strlen($text);
...
function dialecticAiResponseExtractPhpExportScalarField($text, $fieldName)
{
    $needle = "'" . $fieldName . "' => '";
    ...
    $parsed = dialecticAiResponseReadPhpExportQuotedString($text, $fieldPos + strlen($needle));
    if (!is_array($parsed) || !isset($parsed["value"])) {
        return "";
    }

    return dialecticAiResponseDecodePromptExportString((string)$parsed["value"]);
}

function dialecticAiResponseExtractPromptPayloadFromDecodedArray(array $decoded)
{
    $promptPayload = null;
...
function dialecticAiResponseExtractPromptPayloadFromPhpArrayExport($rawPrompt)
{
    ...
    while (($rolePos = strpos($rawPrompt, $roleNeedle, $cursor)) !== false) {
        $roleParsed = dialecticAiResponseReadPhpExportQuotedString($rawPrompt, $rolePos + strlen($roleNeedle));
        ...
        $contentParsed = dialecticAiResponseReadPhpExportQuotedString($rawPrompt, $contentPos + strlen($contentNeedle));
        if (!is_array($contentParsed) || !isset($contentParsed["value"], $contentParsed["next"])) {
            break;
        }

        $messages[] = [
            "role" => dialecticAiResponseDecodePromptExportString((string)$roleParsed["value"]),
            "content" => dialecticAiResponseDecodePromptExportString((string)$contentParsed["value"]),
        ];
    ...
    $responseConnector = [];
    $connectorType = dialecticAiResponseExtractPhpExportScalarField($rawPrompt, "connector_type");
    if ($connectorType !== "") {
    ...
    return [
        "messages" => $messages,
        "model" => dialecticAiResponseExtractPhpExportScalarField($rawPrompt, "model"),
        "response_connector" => $responseConnector,
    ];
}

function dialecticAiResponseExtractPromptModalData($rawPromptValue)
{
    $rawPrompt = dialecticAiResponseNormalizePromptMarkup($rawPromptValue);
    if ($rawPrompt === "") {
        return null;
    }
    ...
    if (is_array($decoded)) {
        return dialecticAiResponseExtractPromptPayloadFromDecodedArray($decoded);
    }

    return dialecticAiResponseExtractPromptPayloadFromPhpArrayExport($rawPrompt);
}

function dialecticAiResponseExtractWorldKnowledgeTopics($rawPromptValue)
{
    $text = dialecticAiResponseNormalizeMarkup($rawPromptValue);
    if ($text === "") {
        return [];
    }
...
function dialecticAiResponseExtractWorldKnowledgeTopicAndLevel($rawPromptValue)
{
    $result = [null, 'none'];
    $text = dialecticAiResponseNormalizeMarkup($rawPromptValue);
    if ($text === "") {
    ...
    if (preg_match('/#World Knowledge\s*\((?=[^)]*advanced knowledge)[^)]*\):\s*([^"\<\r\n]+)/i', $text, $matches)) {
    ...
    if (preg_match('/#World Knowledge\s*\((?=[^)]*basic knowledge)[^)]*\):\s*([^"\<\r\n]+)/i', $text, $matches)) {
        $topic = trim(strip_tags($matches[1]));
        if ($topic !== "") {
            return [$topic, 'basic'];
        }
...
function dialecticAiResponseFormatWorldKnowledge($rawPromptValue)
{
    [$worldknowledgeTopic, $worldknowledgeLevel] = dialecticAiResponseExtractWorldKnowledgeTopicAndLevel($rawPromptValue);
    $topics = dialecticAiResponseExtractWorldKnowledgeTopics($rawPromptValue);
    $labels = [];
    $seen = [];

    if ($worldknowledgeTopic !== null && trim((string)$worldknowledgeTopic) !== "") {
        $label = trim((string)$worldknowledgeTopic) . " (" . trim((string)$worldknowledgeLevel) . ")";
        $labels[] = $label;
        $seen[strtolower(trim((string)$worldknowledgeTopic))] = true;
    }
...
function dialecticAiResponseFormatPromptModalMessageBodyHtml($content)
{
    if (is_array($content)) {
    ...
    return '<div class="prompt-modal-message-body">'
        . dialecticAiResponseH($content)
        . '</div>';
}

function dialecticAiResponseFormatPromptModalHtml($rawPromptValue)
{
    $rawPrompt = dialecticAiResponseNormalizePromptMarkup($rawPromptValue);
    if ($rawPrompt === "") {
    ...
    $formattedPrompt = "";
    $promptModalData = dialecticAiResponseExtractPromptModalData($rawPromptValue);
    ...
    if (is_array($promptModalData)) {
        ...
        if ($responseConnectorLabel !== "") {
            $metaBits[] = '<span class="prompt-meta prompt-meta-label">' . dialecticAiResponseH($responseConnectorLabel) . '</span>';
        }
        if ($responseConnectorDriver !== "") {
            $metaBits[] = '<span class="prompt-meta prompt-meta-driver">' . dialecticAiResponseH($responseConnectorDriver) . '</span>';
        }
        if ($model !== "unknown") {
            $metaBits[] = '<span class="prompt-meta prompt-meta-model">' . dialecticAiResponseH($model) . '</span>';
        }
        if (!empty($metaBits)) {
        ...
        foreach ($messages as $msgIndex => $msg) {
            ...
            $formattedPrompt .= '<div class="prompt-modal-message prompt-role-' . dialecticAiResponseH($roleClass) . '">';
            $formattedPrompt .= '<div class="prompt-modal-message-header">';
            $formattedPrompt .= '<span class="prompt-role">' . dialecticAiResponseH(strtoupper($role)) . '</span>';
            $formattedPrompt .= '<span class="prompt-index">#' . intval($msgIndex) . '</span>';
            $formattedPrompt .= '</div>';
            $formattedPrompt .= dialecticAiResponseFormatPromptModalMessageBodyHtml($msg["content"]);
            $formattedPrompt .= '</div>';
        }
    ...
    if ($formattedPrompt === "") {
        $formattedPrompt = '<pre class="prompt-raw">' . dialecticAiResponseH($rawPrompt) . '</pre>';
    }

    return $formattedPrompt;
}

function dialecticAiResponseTimeColor($seconds)
{
    if ($seconds <= 2.0) {
        return "#88cc88";
    }
...
function dialecticAiResponseFormatUrlWithTimings($url, $response)
{
    ...
    if (strpos((string)$response, "Array") === 0) {
        ...
        return dialecticAiResponseH((string)$stripped);
    }

    $pattern = '/\[AI secs\]\s+([\d.]+)\s+\[TTS secs\]\s+([\d.]+)/';
    if (preg_match($pattern, $safeUrl, $matches) !== 1) {
        return nl2br(dialecticAiResponseH($safeUrl));
    }
    ...
    $baseHtml = $baseText !== "" ? nl2br(dialecticAiResponseH($baseText)) . "<br>" : "";

    return $baseHtml
        . "[LLM] <span style='color:" . dialecticAiResponseH(dialecticAiResponseTimeColor($aiTime)) . "'>" . dialecticAiResponseH(number_format($aiTime, 2)) . "</span>"
        . " [TTS] <span style='color:" . dialecticAiResponseH(dialecticAiResponseTimeColor($ttsOnly)) . "'>" . dialecticAiResponseH(number_format($ttsOnly, 2)) . "</span>"
        . " [Total] <span style='color:" . dialecticAiResponseH(dialecticAiResponseTimeColor($totalTts)) . "'>" . dialecticAiResponseH(number_format($totalTts, 2)) . "</span>";
}

function dialecticAiResponseBuildUrl($page, $limit, array $extraParams = [])
{
    $params = array_merge([
...
if (isset($_GET["cleanlog"]) && $_GET["cleanlog"]) {
    dialecticAiResponseSafeDeleteLog($db);
    header("Location: ai-response.php");
    exit;
}

$totalRow = dialecticAiResponseSafeFetchAll($db, "SELECT COUNT(*) AS total FROM log");
$totalRecords = intval($totalRow[0]["total"] ?? 0);
$totalPages = max(1, (int)ceil($totalRecords / $limit));

if (isset($_GET["export"]) && ($_GET["export"] === "1" || $_GET["export"] === "log")) {
    $allRows = dialecticAiResponseSafeFetchAll(
        $db,
        "SELECT A.*, ROWID
         FROM log a
    ...
    header("Content-Disposition: attachment; filename=\"dialectic_ai_responses.csv\"");
    $out = fopen("php://output", "w");
    if ($out !== false) {
        fputcsv($out, array_merge(["rowid", "time_utc", "ai_response", "worldknowledge_topic", "prompt", "http_request"], array_keys($extraColumns)));
        foreach ($allRows as $row) {
            $csvRow = [
                intval($row["ROWID"] ?? $row["rowid"] ?? 0),
                dialecticAiResponseFormatLocalTs($row["localts"] ?? 0),
                dialecticAiResponseExtractBestText($row["response"] ?? "", $row["prompt"] ?? ""),
                dialecticAiResponseFormatWorldKnowledge($row["prompt"] ?? ""),
                strval($row["prompt"] ?? ""),
                strval($row["url"] ?? ""),
            ];
...
$rows = dialecticAiResponseSafeFetchAll(
    $db,
    "SELECT A.*, ROWID
     FROM log a
     ORDER BY localts DESC, rowid DESC
...
<div id="contentModal" class="modal">
    <div class="modal-content">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            <h2 style="margin: 0; color: rgb(255, 182, 65); font-family: 'Gothic821', sans-serif;">&#x1F4DC; Prompt Viewer</h2>
            <div>
    ...
    <div class="tab-container">
        ...
        <div id="responselog-tab" class="tab-content">
            <div style="background: #2a2a2a; border-left: 4px solid rgb(255, 182, 65); padding: 12px 15px; border-radius: 5px; margin: 15px 0; font-size: 0.9em;">
                <span style="color: rgb(255, 182, 65); font-weight: bold;">AI Responses:</span>
                <span style="color: #f8f9fa;">Complete log of AI-generated responses including the full context payload sent to the LLM. Use this to debug model behavior, prompt composition, WorldKnowledge topics, and timing.</span>
            </div>
            ...
            <div class="pagination-row" style="margin-bottom: 10px;">
                ...
                <?php if ($page > 1): ?>
                    <a class="btn-base btn-primary" href="<?php echo dialecticAiResponseH(dialecticAiResponseBuildUrl($page - 1, $limit)); ?>">Previous</a>
                <?php endif; ?>
                <?php if ($page < $totalPages): ?>
                    <a class="btn-base btn-primary" href="<?php echo dialecticAiResponseH(dialecticAiResponseBuildUrl($page + 1, $limit)); ?>">Next</a>
                <?php endif; ?>
                <div style="margin-left:auto; display:flex; gap:10px; flex-wrap:wrap;">
                    ...
                    <button onclick="window.open('<?php echo dialecticAiResponseH(dialecticAiResponseBuildUrl($page, $limit, ['export' => 1])); ?>', '_blank')" class="btn-base btn-primary" style="padding: 8px 12px; font-size: 0.9em;">Export Response Log</button>
                </div>
            </div>
            ...
            <div class="table-container">
                <table class="ai-response-table">
                    ...
                    <tr>
                        ...
                        <th style="width:16%">WorldKnowledge Topic</th>
                        ...
                        <?php foreach (array_keys($extraColumns) as $extraColumn): ?>
                            <th><?php echo dialecticAiResponseH($extraColumn); ?></th>
                        <?php endforeach; ?>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if (count($rows) > 0): ?>
                        <?php foreach ($rows as $row): ?>
                            ...
                            $promptModalContentById[$promptId] = dialecticAiResponseFormatPromptModalHtml($row["prompt"] ?? "");
                            $displayResponse = dialecticAiResponseExtractBestText($row["response"] ?? "", $row["prompt"] ?? "");
                            $worldknowledgeTopicDisplay = dialecticAiResponseFormatWorldKnowledge($row["prompt"] ?? "");
                            ?>
                            <tr>
                                <td><?php echo dialecticAiResponseH(dialecticAiResponseFormatLocalTs($row["localts"] ?? 0)); ?></td>
                                <td class="response-cell"><?php echo nl2br(dialecticAiResponseH($displayResponse)); ?></td>
                                <td><?php echo nl2br(dialecticAiResponseH($worldknowledgeTopicDisplay)); ?></td>
                                <td>
                                    <button class="view-contents-btn" data-prompt-id="<?php echo dialecticAiResponseH($promptId); ?>">&#x1F9FE; View Prompt</button>
                                </td>
                                <td class="http-cell"><?php echo dialecticAiResponseFormatUrlWithTimings($row["url"] ?? "", $row["response"] ?? ""); ?></td>
                                <td><?php echo dialecticAiResponseH($rowId); ?></td>
                                <?php foreach (array_keys($extraColumns) as $extraColumn): ?>
                                    <td><?php echo nl2br(dialecticAiResponseH($row[$extraColumn] ?? "")); ?></td>
                                <?php endforeach; ?>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <?php if (!empty($promptModalContentById)): ?>
                <div id="prompt-modal-store" style="display:none;">
                    <?php foreach ($promptModalContentById as $promptId => $promptHtml): ?>
                        <div id="<?php echo dialecticAiResponseH($promptId); ?>"><?php echo $promptHtml; ?></div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="pagination-row">
                <span class="info-message" style="padding:0">Page <?php echo intval($page); ?> / <?php echo intval($totalPages); ?> (<?php echo intval($totalRecords); ?> rows)</span>
                <?php if ($page > 1): ?>
                    <a class="btn-base" href="<?php echo dialecticAiResponseH(dialecticAiResponseBuildUrl($page - 1, $limit)); ?>">Previous</a>
                <?php endif; ?>
                <?php if ($page < $totalPages): ?>
                    <a class="btn-base" href="<?php echo dialecticAiResponseH(dialecticAiResponseBuildUrl($page + 1, $limit)); ?>">Next</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
