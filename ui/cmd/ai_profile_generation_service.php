...
function aiProfileLoadPromptTemplate($db): string
{
    ...
    return
        ...
        "Based on all this information, generate a concise character sheet for {DIALECTIC_NAME}.\n\n" .
        ...
        "<skills> Text. Skills, training, talents, combat abilities, and practical expertise.\n" .
...
function aiProfileStampAutofillQueued(array $currentNpcData, NpcMaster $npcMaster): void
{
    $extended = $npcMaster->getExtendedData($currentNpcData);
    $extended['auto_profile_last_error'] = 'queued_async';
    $extended['auto_profile_last_attempt_ts'] = time();
    $currentNpcData = $npcMaster->setExtendedData($currentNpcData, $extended);
    $npcMaster->updateByArray($currentNpcData);
}
...
function aiProfileGenerate(array $options): array
{
    ...
    if ($selectedEventsProvided) {
        ...
        if ($source === 'auto' && count($selectedEvents) < $eventLimit) {
            return [
                'done' => false,
                'error' => 'Not enough context events for automatic profile generation.',
                'error_type' => 'insufficient_events',
                'events' => count($selectedEvents),
                'required_events' => $eventLimit,
            ];
        }
    } else {
        $selectedEvents = aiProfileNormalizeSelectedEvents($selectedEvents);
    ...
    $head = [[
        'role' => 'system',
        'content' => "You are a writing assistant. Examine this character sheet and any dialogue or events provided from the fictional universe of Fallout."
    ]];
