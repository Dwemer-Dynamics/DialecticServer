...
{
 $activeId = dialecticGetGeneralSettingInt('GLOBAL_STT_CONNECTOR_ID', 0);
 if ($activeId > 0) {
 ...
 if ($createdId > 0) {
 dialecticSetGeneralSetting('GLOBAL_STT_CONNECTOR_ID', $createdId, dialecticGetSchemaDescription('GLOBAL_STT_CONNECTOR_ID'));
 }
 return $createdId;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_connector'])) {
 ...
 dialecticSetGeneralSetting('GLOBAL_STT_CONNECTOR_ID', $savedId, dialecticGetSchemaDescription('GLOBAL_STT_CONNECTOR_ID'));
...
.group-title { color: #f3d6a8; font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.05em; margin: 8px 0 0; }
...
.conn-card.active { outline: 2px solid rgb(255, 182, 65); background: linear-gradient(135deg, rgba(52,42,32,.95), rgba(44,34,24,.98)); box-shadow: 0 4px 12px rgba(255, 182, 65,.3); }
.conn-head { display: flex; justify-content: space-between; gap: 8px; align-items: flex-start; }
.conn-name { color: #e9efff; font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.05em; }
...
.field-block input:focus, .field-block select:focus, .field-block textarea:focus { outline: none; border-color: rgba(255, 182, 65,.45); box-shadow: 0 0 0 3px rgba(255, 182, 65,.09); }
...
.meta-group { display: none; border-top: 1px solid rgba(255, 182, 65,.12); margin-top: 8px; padding-top: 16px; }
.meta-group.active { display: block; }
.meta-group h3 { font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.2em; font-weight: normal; color: #f3d6a8; margin: 0 0 14px; }
...
@media (max-width: 980px) {
...
</style><main><div class="page-shell"><div class="page-header"><h1 class="api-title">STT Connector</h1><p class="page-subtitle">Speech-to-Text Setup Options.</p></div><?php if ($saved): ?><div class="notice">STT connector saved.</div><?php elseif ($notice !== ''): ?><div class="notice"><?php echo h($notice); ?></div><?php endif; ?><div class="layout"><div class="left-col"><div class="summary-note">
 ...
 </div><div class="list-wrap" id="stt_driver_list"><?php foreach ($groupedDriverOptions as $groupLabel => $groupDrivers): ?><?php if (empty($groupDrivers)) { continue; } ?><div class="group-title"><?php echo h($groupLabel); ?></div><?php foreach ($groupDrivers as $driverValue): ?><div class="conn-card<?php echo $currentDriver === $driverValue ? ' active' : ''; ?>" data-driver-card="<?php echo h($driverValue); ?>"><div class="conn-head"><div class="conn-name"><?php echo h($connector->getDisplayName($driverValue)); ?></div><div class="conn-badge"><?php echo h($connector->getProviderKeyFromDriver($driverValue) ?: 'SYSTEM'); ?></div></div><div class="conn-sub"><?php echo h($connector->getProviderTitle($driverValue)); ?></div></div><?php endforeach; ?><?php endforeach; ?></div></div><div class="right-col"><?php if (!$editingRow): ?><div class="settings-empty-note">No STT connector is configured yet.</div><?php else: ?><form method="post" action="<?php echo h(sttPageUrl()); ?>" id="stt_connector_form"><input type="hidden" name="id" value="<?php echo h($editingRow['id'] ?? ''); ?>"><input type="hidden" name="save_connector" value="1"><div class="btn-row"><button type="submit" class="btn-save">Save</button><button type="button" class="btn-primary" id="btn_test_connector_inline">Test</button><button type="button" class="btn-secondary" id="btn_google_free_stt_inline">Google Free STT</button></div><div class="orm-note">Testing saves the current connector first so the modal uses the latest settings.</div><div class="editor-grid"><div class="field-block"><label for="label">Name</label><input type="text" id="label" name="label" value="<?php echo h($editingRow['label'] ?? ''); ?>"><div class="field-help">This label is kept for migration and internal reference, even though only one STT connector is used globally.</div></div><div class="field-block"><label for="driver">Service</label><select id="driver" name="driver"><?php foreach ($groupedDriverOptions as $groupLabel => $groupDrivers): ?><?php if (empty($groupDrivers)) { continue; } ?><optgroup label="<?php echo h($groupLabel); ?>"><?php foreach ($groupDrivers as $driverValue): ?><option value="<?php echo h($driverValue); ?>" <?php echo $currentDriver === $driverValue ? 'selected' : ''; ?>><?php echo h($connector->getDisplayName($driverValue)); ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select><div class="field-help">Choose the speech-to-text backend DialecticServer should load globally.</div></div><div class="field-block" id="url_block" style="<?php echo $connector->driverSupportsEditableUrl($currentDriver) ? '' : 'display:none;'; ?>"><label for="url">URL</label><input type="url" id="url" name="url" value="<?php echo h($editingRow['url'] ?? $connector->getDefaultUrlForDriver($currentDriver)); ?>"><div class="field-help">Used for local or remote STT endpoints such as Local Whisper.</div></div><div class="field-block" id="api_badge_block" style="<?php echo $connector->driverUsesApiBadge($currentDriver) ? '' : 'display:none;'; ?>"><label for="api_badge_id">API Badge</label><?php
 ...
 ?><select id="api_badge_id" name="api_badge_id"><option value="">-- None --</option><?php foreach ($withKey as $apiRow): ?><?php
 ...
 ?><option value="<?php echo h($apiRowId); ?>" data-empty="0" <?php echo $selected; ?>><?php echo h($labelText); ?></option><?php endforeach; ?><?php if (!empty($noKey)): ?><option value="" disabled> Missing Key </option><?php foreach ($noKey as $apiRow): ?><?php
 $apiRowId = intval($apiRow['id'] ?? 0);
 $selected = strval($selectedApi) === strval($apiRowId) ? 'selected' : '';
 $labelText = ' ' . strval($apiRow['label'] ?? ('Key #' . $apiRowId)) . ' No key';
 ?><option value="<?php echo h($apiRowId); ?>" data-empty="1" <?php echo $selected; ?>><?php echo h($labelText); ?></option><?php endforeach; ?><?php endif; ?></select><div id="api_key_notice" class="api-key-notice"></div><div class="field-help">Cloud STT services require an API key from the API Keys page.</div></div></div><?php foreach ($driverOptions as $driverOption): ?><?php
 ...
 ?><div class="meta-group<?php echo $groupDriver === $currentDriver ? ' active' : ''; ?>" data-driver-fields="<?php echo h($groupDriver); ?>"><h3><?php echo h($connector->getProviderTitle($groupDriver)); ?> Settings</h3><?php if (empty($visibleFieldNames)): ?><div class="settings-empty-note">This STT provider does not have connector-level settings to configure here.</div><?php else: ?><div class="inline-two"><?php foreach ($groupSchema as $fieldName => $definition): ?><?php if (!sttShouldRenderField($fieldName, $definition)) { continue; } ?><?php
 ...
 ?><div class="field-block"><label for="<?php echo h($fieldKey); ?>"><?php echo h(sttFieldLabel($fieldName)); ?></label><?php if ($fieldType === 'boolean'): ?><input type="hidden" name="<?php echo h($fieldKey); ?>" value="false"><select id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>"><option value="true" <?php echo $fieldValue ? 'selected' : ''; ?>>Enabled</option><option value="false" <?php echo !$fieldValue ? 'selected' : ''; ?>>Disabled</option></select><?php elseif ($fieldType === 'integer' || $fieldType === 'int'): ?><input type="number" step="1" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php elseif ($fieldType === 'number'): ?><input type="number" step="0.01" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php elseif ($fieldType === 'longstring'): ?><textarea id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>"><?php echo h($fieldValue); ?></textarea><?php elseif ($fieldType === 'select'): ?><select id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>"><?php foreach (($definition['values'] ?? []) as $valueOption): ?><option value="<?php echo h($valueOption); ?>" <?php echo strval($fieldValue) === strval($valueOption) ? 'selected' : ''; ?>><?php echo h($valueOption); ?></option><?php endforeach; ?></select><?php elseif ($fieldType === 'selectmultiple'): ?><select id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>[]" multiple><?php $selectedValues = is_array($fieldValue) ? $fieldValue : []; ?><?php foreach (($definition['values'] ?? []) as $valueOption): ?><option value="<?php echo h($valueOption); ?>" <?php echo in_array($valueOption, $selectedValues, true) ? 'selected' : ''; ?>><?php echo h($valueOption); ?></option><?php endforeach; ?></select><?php else: ?><input type="text" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php endif; ?><?php if (!empty($definition['description'])): ?><div class="field-help"><?php echo $definition['description']; ?></div><?php endif; ?></div><?php endforeach; ?></div><?php endif; ?></div><?php endforeach; ?></form><?php endif; ?></div></div></div><div id="stt_test_modal"><div class="inner"><button type="button" class="btn-secondary" id="stt_test_close">Close</button><iframe id="stt_test_iframe" src="about:blank"></iframe></div></div></main><script>
(function(){
