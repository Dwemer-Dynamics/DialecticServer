...
function functionEditorGetParameterSchemaVariableValues()
{
 ...
 'dialectic_name' => trim(strval($GLOBALS['DIALECTIC_NAME'] ?? '')),
 ];
}
...
function functionEditorRestoreParameterSchemaVariablesInString($text)
{
 ...
 if ($key === 'dialectic_name') {
 $text = str_replace(['#DIALECTIC_NAME#', '$DIALECTIC_NAME', '{$DIALECTIC_NAME}', '{$GLOBALS["DIALECTIC_NAME"]}'], $value, $text);
 }
 }
...
function functionEditorReplaceActionTextVariablesInString($text)
{
 ...
 $text = preg_replace('/\b[Tt]he\s+NPC\b/u', '#DIALECTIC_NAME#', $text);
 $text = preg_replace('/\bNPC\b/u', '#DIALECTIC_NAME#', $text);
...
function functionEditorNormalizeSubmittedParameterSchema($rawValue, &$errorMessage)
{
 ...
 if (!function_exists('dialecticActionCatalogNormalizeParameterSchema')) {
 return $decoded;
 }

 return dialecticActionCatalogNormalizeParameterSchema($decoded);
}
...
{
 if (!function_exists('dialecticActionCatalogGetEditorFields')) {
 return [];
 }

 return dialecticActionCatalogGetEditorFields($row);
}
...
function functionEditorFormatConfigValue($field, $value)
{
 ...
 ? dialecticActionCatalogNormalizeEditorField($field)
 : $field;
 ...
 if (($field['format'] ?? '') === 'caps') {
 return functionEditorFormatCaps($value);
 }
...
function functionEditorFormatCaps($value)
{
 $value = intval($value);
 return ($value === 1) ? "1 cap" : ("{$value} caps");
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
 ...
 if ($messageType !== "err") {
 if (dialecticActionCatalogUpsertCustomConfig($codeName, $configValues)) {
 ...
 $message = "Unknown action code name.";
 $messageType = "err";
 } elseif (!function_exists("dialecticActionCatalogUpsertCustomParameters")) {
 ...
 $message = $errorMessage;
 $messageType = "err";
 } elseif (dialecticActionCatalogUpsertCustomParameters($codeName, $normalizedParameters)) {
 ...
 $message = $errorMessage;
 $messageType = "err";
 } elseif (function_exists("dialecticFindActionCatalogActionNameConflict")) {
 ...
 $messageType = "err";
 } elseif (dialecticActionCatalogUpsertCustomTextFields($codeName, [
 ...
 $messageType = "err";
 } elseif (!function_exists("dialecticActionCatalogDeleteCustomOverride")) {
 $message = "Action catalog override reset support is not available in this build.";
 $messageType = "err";
 } elseif (dialecticActionCatalogDeleteCustomOverride($codeName)) {
...
$catalogReady = function_exists("dialecticActionCatalogDbReady") && dialecticActionCatalogDbReady();

if ($catalogReady) {
 $whereParts = [];
 if ($search !== "") {
 $searchLiteral = dialecticActionCatalogSqlText("%" . $search . "%");
...
?><link rel="stylesheet" href="<?php echo $webRoot; ?>/ui/css/main.css"><style>
 ...
 box-shadow: 0 0 0 3px rgba(255, 182, 65, 0.12), inset 0 1px 1px rgba(0, 0, 0, 0.2);
 }
 .filter-summary {
 color: #d2b078;
 font-size: 0.92em;
...
</style><main><div id="toast" class="toast-notification"><span class="message"></span></div><div class="page-header"><h1 class="api-title">Action Editor</h1><p class="page-subtitle">Configure available actions exposed to AI prompting and execution</p></div><?php if (!$catalogReady): ?><div class="content-section"><h2>Action Catalog Unavailable</h2><p style="margin:0; color:#d0d6df; line-height:1.45;">
 Action catalog tables are not available yet. Run DialecticServer database updates, then reload this page.
 </p></div><?php else: ?><div class="content-grid"><div class="content-section"><div class="section-header"><h2>Action Summary</h2><button type="button" class="action-button secondary summary-action-button" id="viewActiveActionsButton">View Active Actions</button></div><div class="summary-grid"><div class="summary-card"><div class="summary-card-label">Total Actions</div><div class="summary-card-value"><span class="stat-pill"><?php echo h($countAll); ?></span></div></div><div class="summary-card"><div class="summary-card-label">Enabled</div><div class="summary-card-value"><span class="stat-pill enabled"><?php echo h($countEnabled); ?></span></div></div><div class="summary-card"><div class="summary-card-label">Disabled</div><div class="summary-card-value"><span class="stat-pill disabled"><?php echo h($countDisabled); ?></span></div></div></div></div><div class="content-section"><h2>How It Works</h2><p style="margin:0; color:#d0d6df; line-height:1.45;">
 ...
 versus <code>script_proxy</code>, and the pricing column can override selected cap costs per action
 without changing the shipped base catalog.
 </p></div><div id="activeActionsModal" class="action-modal" hidden><div class="action-modal-panel" role="dialog" aria-modal="true" aria-labelledby="activeActionsModalTitle"><div class="action-modal-header"><div><h3 class="action-modal-title" id="activeActionsModalTitle">Currently Active Actions</h3><p class="action-modal-subtitle">
 Enabled actions grouped by scope. Actions available in multiple scopes appear in each relevant section.
 </p></div><button type="button" class="action-modal-close" data-modal-close aria-label="Close active actions modal">&times;</button></div><div class="action-modal-body"><div class="active-scope-list-wrap"><?php foreach ($activeActionScopeGroups as $scopeGroup): ?><?php
 $scopeLabel = strval($scopeGroup["label"] ?? "");
 $scopeActions = is_array($scopeGroup["actions"] ?? null) ? $scopeGroup["actions"] : [];
 ?><section class="active-scope-row"><div class="active-scope-meta"><h4 class="active-scope-title"><?php echo h($scopeLabel); ?></h4><div class="active-scope-count"><?php echo h(count($scopeActions)); ?></div></div><div class="active-scope-actions"><?php if (count($scopeActions) === 0): ?><div class="active-scope-empty">No active actions in this scope.</div><?php else: ?><?php foreach ($scopeActions as $activeAction): ?><?php
 ...
 ?><div class="active-action-row"><div class="active-action-label"><strong><?php echo h($activeActionName); ?></strong><code><?php echo h($activeCodeName); ?></code></div><div class="active-action-description"><?php echo $activeDescription !== "" ? nl2br(h($activeDescription)) : '<em>No prompt provided.</em>'; ?></div></div><?php endforeach; ?><?php endif; ?></div></section><?php endforeach; ?></div></div></div></div><div class="full-width-section"><div class="action-container"><div class="filter-toolbar" id="entries"><div class="filter-toolbar-top"><div class="live-search-wrap"><input type="text" id="actionLiveSearch" value="<?php echo h($search); ?>" placeholder="Search"><div class="filter-summary"><span id="actionVisibleCount"><?php echo h(count($rows)); ?></span> of <?php echo h(count($rows)); ?> shown</div></div><div class="filter-actions"><button type="button" class="action-button secondary" id="actionFilterReset">Reset Filters</button></div></div><div class="filter-toolbar-bottom"><div class="filter-groups"><div class="filter-group"><div class="filter-group-label">State</div><div class="filter-chip-row"><input type="radio" name="live-state" id="live-state-all" value="all" <?php echo $state === "all" ? "checked" : ""; ?>><label for="live-state-all">All</label><input type="radio" name="live-state" id="live-state-enabled" value="enabled" <?php echo $state === "enabled" ? "checked" : ""; ?>><label for="live-state-enabled">Enabled</label><input type="radio" name="live-state" id="live-state-disabled" value="disabled" <?php echo $state === "disabled" ? "checked" : ""; ?>><label for="live-state-disabled">Disabled</label></div></div><div class="filter-group"><div class="filter-group-label">Scope</div><div class="filter-chip-row"><input type="radio" name="live-scope" id="live-scope-all" value="all" <?php echo $scope === "all" ? "checked" : ""; ?>><label for="live-scope-all">All</label><input type="radio" name="live-scope" id="live-scope-npc" value="npc" <?php echo $scope === "npc" ? "checked" : ""; ?>><label for="live-scope-npc">NPC</label><input type="radio" name="live-scope" id="live-scope-followers" value="followers" <?php echo $scope === "followers" ? "checked" : ""; ?>><label for="live-scope-followers">Followers</label><input type="radio" name="live-scope" id="live-scope-narrator" value="narrator" <?php echo $scope === "narrator" ? "checked" : ""; ?>><label for="live-scope-narrator">Narrator</label><input type="radio" name="live-scope" id="live-scope-dynamic" value="dynamic" <?php echo $scope === "dynamic" ? "checked" : ""; ?>><label for="live-scope-dynamic">Dynamic</label></div></div><div class="filter-group"><div class="filter-group-label">Dispatch</div><div class="filter-chip-row"><input type="radio" name="live-dispatch" id="live-dispatch-all" value="all" <?php echo $gameFilter === "all" ? "checked" : ""; ?>><label for="live-dispatch-all">All</label><input type="radio" name="live-dispatch" id="live-dispatch-game" value="game" <?php echo $gameFilter === "game" ? "checked" : ""; ?>><label for="live-dispatch-game">Game</label><input type="radio" name="live-dispatch" id="live-dispatch-server" value="server" <?php echo $gameFilter === "server" ? "checked" : ""; ?>><label for="live-dispatch-server">Server</label></div></div><div class="filter-group"><div class="filter-group-label">Source</div><div class="filter-chip-row"><input type="radio" name="live-source" id="live-source-all" value="all" <?php echo $customFilter === "all" ? "checked" : ""; ?>><label for="live-source-all">All</label><input type="radio" name="live-source" id="live-source-base" value="base" <?php echo $customFilter === "base" ? "checked" : ""; ?>><label for="live-source-base">Base</label><input type="radio" name="live-source" id="live-source-custom" value="custom" <?php echo $customFilter === "custom" ? "checked" : ""; ?>><label for="live-source-custom">Custom</label></div></div></div></div></div></div><div class="table-container"><table><thead><tr><th>Command</th><th>Info</th><th>Config</th><th>Parameters</th><th>Metadata</th><th>Toggle</th></tr></thead><tbody><?php if (count($rows) === 0): ?><tr><td colspan="6">No actions found.</td></tr><?php endif; ?><?php foreach ($rows as $row): ?><?php
 $codeName = strval($row["code_name"] ?? "");
 $enabled = dialecticActionCatalogToBool($row["is_activated"] ?? false);
 $isCustom = dialecticActionCatalogToBool($row["is_custom"] ?? false);
 $hasBase = dialecticActionCatalogToBool($row["has_base"] ?? false);
 $isNpc = dialecticActionCatalogToBool($row["available_to_npc"] ?? false);
 $isFollowers = dialecticActionCatalogToBool($row["available_to_followers"] ?? false);
 $isNarrator = dialecticActionCatalogToBool($row["available_to_narrator"] ?? false);
 $isGameFunction = dialecticActionCatalogToBool($row["game_function"] ?? false);
 ...
 $resolvedConfig = function_exists('dialecticActionCatalogGetResolvedCustomConfig')
 ? dialecticActionCatalogGetResolvedCustomConfig($codeName, $row)
 : $customConfig;
 ...
 ><td style="min-width: 220px;"><code class="command-code"><?php echo h($codeName); ?></code><div class="command-meta"><?php if ($isNpc): ?><span class="status-pill scope">NPC</span><?php endif; ?><?php if ($isFollowers): ?><span class="status-pill scope">Followers</span><?php endif; ?><?php if ($isNarrator): ?><span class="status-pill scope">Narrator</span><?php endif; ?><?php if (!$isNpc && !$isFollowers && !$isNarrator): ?><span class="status-pill scope">Dynamic</span><?php endif; ?><span class="status-pill <?php echo $isGameFunction ? "game" : "server"; ?>"><?php echo $isGameFunction ? "Game" : "Server"; ?></span><span class="status-pill <?php echo $isCustom ? "custom" : "base"; ?>"><?php echo $isCustom ? "Custom" : "Base"; ?></span><span class="status-pill <?php echo $enabled ? "enabled" : "disabled"; ?>"><?php echo $enabled ? "Enabled" : "Disabled"; ?></span></div></td><td style="max-width: 360px;"><div style="margin-bottom:8px;"><strong><?php echo h($actionNameValue); ?></strong></div><?php echo nl2br(h($descriptionValue)); ?><div class="inline-action-editor" style="margin-top: 12px;"><form method="post" action="<?php echo h(functionEditorBuildUrl($currentFilterParams, $isEmbed, "entries")); ?>"><input type="hidden" name="action" value="update_action_text_fields"><input type="hidden" name="code_name" value="<?php echo h($codeName); ?>"><div class="config-field" style="margin-bottom: 12px;"><label for="<?php echo h('text-name-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>">Name</label><div class="editor-controls"><input
 type="text"
 id="<?php echo h('text-name-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>"
 name="action_name"
 value="<?php echo h($actionNameValue); ?>"
 ></div></div><div class="config-field" style="margin-bottom: 12px;"><label for="<?php echo h('text-description-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>">Prompt</label><div class="editor-controls"><textarea
 id="<?php echo h('text-description-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>"
 name="description"
 rows="4"
 ><?php echo h($descriptionValue); ?></textarea></div></div><div class="config-field"><label for="<?php echo h('text-return-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>">Return Message</label><div class="editor-controls"><textarea
 id="<?php echo h('text-return-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>"
 name="return_message"
 rows="3"
 ><?php echo h($returnMessageValue); ?></textarea></div><div class="helper-text">
 Edit the visible action name, the prompt text, and the default return text from this row.
 </div></div><div class="editor-controls"><button type="submit" class="btn-save">Save Info</button></div></form></div></td><td class="pricing-cell"><?php if (count($configFields) > 0): ?><div class="inline-action-editor"><form method="post" action="<?php echo h(functionEditorBuildUrl($currentFilterParams, $isEmbed, "entries")); ?>"><input type="hidden" name="action" value="update_action_config"><input type="hidden" name="code_name" value="<?php echo h($codeName); ?>"><?php foreach ($configFields as $configField): ?><?php
 ...
 $fieldValue = $resolvedConfig[$fieldKey] ?? (function_exists('dialecticActionCatalogGetEditorFieldDefaultValue')
 ? dialecticActionCatalogGetEditorFieldDefaultValue($configField, $row)
 : '');
 ?><div class="config-field" style="margin-bottom: 14px;"><label for="<?php echo h($fieldId); ?>"><?php echo h($configField['label'] ?? $fieldKey); ?></label><div class="editor-controls"><?php if ($fieldType === 'boolean'): ?><input type="hidden" name="config[<?php echo h($fieldKey); ?>]" value="0"><input
 type="checkbox"
 id="<?php echo h($fieldId); ?>"
 name="config[<?php echo h($fieldKey); ?>]"
 value="1"
 <?php echo functionEditorToBool($fieldValue) ? 'checked' : ''; ?>
 ><?php elseif ($fieldType === 'textarea'): ?><textarea
 id="<?php echo h($fieldId); ?>"
 name="config[<?php echo h($fieldKey); ?>]"
 rows="3"
 placeholder="<?php echo h($configField['placeholder'] ?? ''); ?>"
 ><?php echo h($fieldValue); ?></textarea><?php elseif ($fieldType === 'select'): ?><select
 id="<?php echo h($fieldId); ?>"
 name="config[<?php echo h($fieldKey); ?>]"
 ><?php foreach (($configField['options'] ?? []) as $option): ?><?php $optionValue = strval($option['value'] ?? ''); ?><option value="<?php echo h($optionValue); ?>" <?php echo $optionValue === strval($fieldValue) ? 'selected' : ''; ?>><?php echo h($option['label'] ?? $optionValue); ?></option><?php endforeach; ?></select><?php else: ?><input
 type="<?php echo $fieldType === 'integer' || $fieldType === 'number' ? 'number' : 'text'; ?>"
 id="<?php echo h($fieldId); ?>"
 name="config[<?php echo h($fieldKey); ?>]"
 <?php if (is_numeric($configField['minimum'] ?? null)): ?>min="<?php echo h($configField['minimum']); ?>"<?php endif; ?><?php if (is_numeric($configField['maximum'] ?? null)): ?>max="<?php echo h($configField['maximum']); ?>"<?php endif; ?><?php if (is_numeric($configField['step'] ?? null)): ?>step="<?php echo h($configField['step']); ?>"<?php elseif ($fieldType === 'integer'): ?>step="1"<?php endif; ?>
 placeholder="<?php echo h($configField['placeholder'] ?? ''); ?>"
 value="<?php echo h($fieldValue); ?>"
 ><?php endif; ?></div></div><?php endforeach; ?><div class="editor-controls"><button type="submit" class="btn-save">Save Config</button></div></form></div><?php else: ?><span class="pricing-empty">No editable config</span><?php endif; ?></td><td style="min-width: 420px;"><?php echo $parametersRendered; ?><details class="inline-action-editor" style="margin-top: 12px;"><summary class="parameter-schema-toggle">Advanced: Edit Parameter Schema</summary><form method="post" action="<?php echo h(functionEditorBuildUrl($currentFilterParams, $isEmbed, "entries")); ?>" style="margin-top: 10px;"><input type="hidden" name="action" value="update_action_parameters"><input type="hidden" name="code_name" value="<?php echo h($codeName); ?>"><div class="config-field"><label for="<?php echo h('parameters-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>">Parameter Schema JSON</label><div class="editor-controls"><textarea
 id="<?php echo h('parameters-' . preg_replace('/[^a-zA-Z0-9_-]/', '-', $codeName)); ?>"
 name="parameters_json"
 rows="12"
 style="font-family: Consolas, monospace;"
 placeholder="{&quot;type&quot;:&quot;object&quot;,&quot;properties&quot;:{},&quot;required&quot;:[]}"
 ><?php echo h($parametersEditorValue); ?></textarea></div><div class="helper-text">
 ...
 </div></div><div class="editor-controls"><button type="submit" class="btn-save">Save Parameters</button></div></form></details></td><td style="min-width: 320px;"><details><summary class="parameter-schema-toggle">Metadata JSON</summary><pre class="json-preview"><?php echo h($metadataPreview); ?></pre></details><?php if (!in_array(trim($scriptProxyPreview), ["", "[]", "{}"], true)): ?><details style="margin-top:10px;"><summary class="return-preview" style="cursor:pointer;">ScriptProxy Program</summary><pre class="json-preview"><?php echo h($scriptProxyPreview); ?></pre></details><?php endif; ?></td><td><form method="post" action="<?php echo h(functionEditorBuildUrl($currentFilterParams, $isEmbed, "entries")); ?>"><?php if ($isEmbed): ?><input type="hidden" name="embed" value="1"><?php endif; ?><input type="hidden" name="action" value="toggle_action"><input type="hidden" name="code_name" value="<?php echo h($codeName); ?>"><input type="hidden" name="target_enabled" value="<?php echo h($targetEnabled); ?>"><button type="submit" class="<?php echo $enabled ? "btn-danger" : "btn-save"; ?>"><?php echo $enabled ? "Disable" : "Enable"; ?></button></form><?php if ($isCustom && $hasBase): ?><form method="post" action="<?php echo h(functionEditorBuildUrl($currentFilterParams, $isEmbed, "entries")); ?>" style="margin-top: 8px;" onsubmit="return confirm('Reset this action to its base definition? This will delete the custom override row for <?php echo h($codeName); ?>.');"><?php if ($isEmbed): ?><input type="hidden" name="embed" value="1"><?php endif; ?><input type="hidden" name="action" value="reset_action_override"><input type="hidden" name="code_name" value="<?php echo h($codeName); ?>"><button type="submit" class="action-button secondary">
 Reset Override
 </button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table><div class="empty-filter-state" id="actionFilterEmptyState">No actions match the current filters.</div></div></div></div><?php endif; ?></main><script>
