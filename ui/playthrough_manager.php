...
$TITLE = " DIALECTIC - Playthrough Saves";
$debugPaneLink = false;
if ($ptmFragment) {
...
if (!$metaReady) {
 echo '<div class="message">Playthrough Saves are unavailable. Run the DIALECTIC database update, then reload this page.</div>';
}
...
?><style>
...
</style><?php if ($isEmbed): ?><style> main { padding-top: 20px; } </style><?php endif; ?><main><div id="toast" class="toast-notification"><span class="message"></span></div><div id="switch-overlay" role="alert" aria-live="assertive" aria-modal="true"><div class="loading-modal"><div class="loading-title" id="loading-title">Restoring Playthrough Save</div><div class="lds-ring"><div></div><div></div><div></div><div></div></div><div class="loading-sub">This can take a few minutes. Please keep this tab open.</div></div></div><div class="page-header"><?php if ($ptmFragment): ?><h2>Playthrough Saves and cleanup</h2><?php else: ?><h1>Playthrough Saves</h1><?php endif; ?><div style="font-size: 0.95em; color: #ccc; margin-bottom: 15px;">Save and restore DIALECTIC data. Use each Playthrough Save with its matching Fallout save.</div><details class="storage-help"><summary>How saves work</summary><div style="background: rgba(0,0,0,0.3); padding: 15px; border-radius: 8px; border: 1px solid #444; margin-top: 15px;"><div style="font-size: 0.9em; color: #e0e0e0; line-height: 1.6;"><strong>Current progress</strong> = the DIALECTIC data you are using now.<br><strong>Playthrough Saves</strong> = saved copies of your DIALECTIC data.<br><strong>Restore</strong> = updates the active Playthrough Save with your current progress, then loads the selected save.<br><strong>Automatic Rollback Saves</strong> = Copies made when you load an older game save. Choose how many game days behind below.<br></div></div></details></div><div class="content-section full-width-section" style="background: linear-gradient(135deg, #1a3a2a 0%, #2a2a2a 100%); border: 2px solid #4ade80;"><div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;"><div style="font-size: 2.5em;"></div><div style="flex: 1;"><h2 style="margin: 0;">Current progress</h2><div style="font-size: 0.9em; color: #9fb1c9; margin-top: 4px;">
 ...
 Active Playthrough Save: <?php echo h($activeProfileName !== '' ? $activeProfileName : '(unknown)'); ?></div><div style="border-left: 2px solid #444; padding-left: 12px; margin-left: 6px;"><strong style="color:#f8f9fa;">Player:</strong><span id="live-player"><?php echo h($livePlayerName); ?></span></div><div><strong style="color:#f8f9fa;">Game:</strong><span id="live-game"><?php echo h($liveGameName); ?></span></div><div><strong style="color:#f8f9fa;">Events:</strong><span id="live-eventlog"><?php echo intval($liveEventlogCount); ?></span></div><div><strong style="color:#f8f9fa;">World knowledge:</strong><span id="live-worldknowledge"><?php echo intval($liveWorldKnowledgeCount); ?></span></div><div><strong style="color:#f8f9fa;">Last in-game date:</strong><span id="live-last"><?php echo h($liveFalloutDate !== '' ? $liveFalloutDate : 'n/a'); ?></span></div></div></div><?php if (!empty($timelineItems)) { ?><div class="timeline" id="pt-timeline"><div class="timeline-title" id="pt-title"></div><div class="timeline-track"></div><div class="timeline-notches" id="pt-timeline-notches"></div><div class="timeline-nodes" id="pt-timeline-nodes"></div><div class="timeline-legend"><span id="pt-min"></span><span id="pt-max"></span></div><div class="timeline-tooltip" id="pt-tooltip"></div></div><?php } ?></div><?php if (!empty($message)) { echo '<div class="content-section">'.$message.'</div>'; } ?><div class="content-grid"><div class="content-section"><h2> New Playthrough Save</h2><div style="font-size: 0.9em; color: #9fb1c9; margin-bottom: 12px;">
 Save a copy of your current DIALECTIC data. Restore it alongside the matching Fallout save.
 </div><form method="post" class="create-form"><input type="hidden" name="action" value="create"><label for="name">Save name</label><br><input type="text" id="name" name="name" required style="width: 100%; margin: 6px 0;" placeholder="e.g., Before Quest X"><label for="notes">Notes (optional)</label><br><input type="text" id="notes" name="notes" style="width: 100%; margin: 6px 0;" placeholder="e.g., Level 25, just finished main quest"><div class="button-group"><button type="submit" class="button" style="background-color: rgb(1 53 166 / 90%); color: #fff;"> New Playthrough Save</button></div></form></div><div class="content-section"><h2> Playthrough Saves</h2><button type="button" class="ptx-import">Import save</button><div style="font-size: 0.9em; color: #9fb1c9; margin-bottom: 12px;">
 These saved copies are separate from your current DIALECTIC data.<br>
 Restore a copy to replace your current DIALECTIC data, then load the matching Fallout save.
 </div><?php if (empty($profiles)) { ?><div style="text-align:center; color:#ccc; padding: 12px;">No Playthrough Saves yet. Use the save form to create one.</div><?php } else { ?><div class="backup-list" style="max-height: 420px; overflow-y:auto; padding: 0; margin: 0; border: 1px solid #333333; border-radius: 8px; background-color: #1a1a1a;"><?php foreach ($profiles as $p) {
 $nm = strtolower((string)($p['name'] ?? ''));
 $nt = strtolower((string)($p['notes'] ?? ''));
 $isTimeline = (strpos($nm,'timeline') !== false) || (strpos($nt,'timeline') !== false);
 ?><div class="backup-item<?php echo $isTimeline ? ' timelinebreak' : ''; ?>" style="padding: 12px; border-bottom: 1px solid #333333; <?php if ((int)$p['is_active'] === 1) { echo 'background: rgba(74, 222, 128, 0.1); border-left: 4px solid #4ade80;'; } ?>"><div style="display:flex; justify-content:space-between; gap: 10px;"><div style="flex:1; min-width:0;"><div style="font-weight:bold; font-size: 14px; word-break: break-all;"><?php if ((int)$p['is_active'] === 1) { ?><span style="background:#4ade80; color:#000; padding:2px 8px; border-radius:4px; font-size:11px; font-weight:bold; margin-right:6px;"> ACTIVE</span><?php } ?><?php echo h($p['name']); ?><?php
 ...
 ?></div><div style="font-size: 12px; color:#ccc; display:flex; gap:10px; flex-wrap:wrap;"><span><?php echo h($p['created_at']); ?></span><span><?php echo h(formatFileSize((int)$p['size_bytes'])); ?></span><span> Player: <?php echo h($p['player_name'] ?? ''); ?></span><span> Game: <?php echo h($p['game'] ?? 'Fallout'); ?></span><span> Events: <?php echo intval($p['eventlog_count'] ?? 0); ?></span><span> World knowledge: <?php echo intval($p['worldknowledge_count'] ?? 0); ?></span><?php
 $lg = isset($p['last_gamets']) ? intval($p['last_gamets']) : 0;
 $skDate = $lg > 0 ? convert_gamets2fallout_long_date($lg) : '';
 ?><span> last in-game: <?php echo h($skDate); ?></span></div><?php if (!empty($p['notes'])) { ?><div style="font-size: 12px; color:#9fb1c9; margin-top: 4px; word-break: break-all;"><?php echo h($p['notes']); ?></div><?php } ?></div><div style="display:flex; gap:6px; align-items:flex-start;"><button type="button" class="ptx-download" data-profile-id="<?= (int)$p['id'] ?>">Download</button><?php if ((int)$p['is_active'] !== 1) { ?><form method="post" class="switch-form"><input type="hidden" name="action" value="switch"><input type="hidden" name="profile_id" value="<?php echo (int)$p['id']; ?>"><button type="submit" class="button" style="background-color: rgb(1 53 166 / 90%); color:#fff; padding:6px 10px;">Restore</button></form><?php } else { ?><button class="button" style="background-color: #333; color:#999; padding:6px 10px; cursor: not-allowed;" disabled>Already Active</button><?php } ?><?php $isDefault = (strtolower((string)$p['name']) === 'default'); ?><?php if (!$isDefault) { ?><form method="post" onsubmit="return confirm('Permanently delete this Playthrough Save? This cannot be undone.');"><input type="hidden" name="action" value="delete"><input type="hidden" name="profile_id" value="<?php echo (int)$p['id']; ?>"><button type="submit" class="button" style="background-color: rgba(166, 53, 63, 0.9); color:#fff; padding:6px 10px;" <?php echo ((int)$p['is_active']===1? 'disabled':''); ?>>Delete</button></form><?php } ?></div></div></div><?php } ?></div><?php } ?></div></div><?php include __DIR__ . '/tmpl/playthrough_transfer_controls.php'; include __DIR__ . '/tmpl/playthrough_save_controls.php'; ?></main><?php
if (!$ptmFragment) {
...
(function(){
 ...
 if (overlayTitle) overlayTitle.textContent = 'Creating Playthrough Save';
 showOverlay();
 }
 }, true);
