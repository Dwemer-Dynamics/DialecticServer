...
function dialecticQuickstartMiniMeDefaultUrl(): string {
    return 'http://127.0.0.1:8082/';
}

function dialecticQuickstartProbeUrl(string $rawUrl): array {
    $result = [
        'ok' => false,
        'http_code' => 0,
        'latency_ms' => 0,
    ...
    $request =
        "GET " . $path . " HTTP/1.1\r\n" .
        "Host: " . $host . "\r\n" .
        "User-Agent: DialecticQuickstartProbe/1.0\r\n" .
        "Accept: */*\r\n" .
        "Connection: close\r\n\r\n";
...
function dialecticQuickstartEnsureActiveSttConnectorId(STTConnector $connector): int {
...
function dialecticQuickstartNormalizeTtsDriver(TTSConnector $connector, $driver): string {
...
function dialecticQuickstartEnsureTtsConnectorForDriver(TTSConnector $connector, string $selectedDriver): ?array {
    $selectedDriver = dialecticQuickstartNormalizeTtsDriver($connector, $selectedDriver);

    $existingRow = dialecticResolvePreferredTtsConnectorRow($selectedDriver);
    if ($existingRow) {
        return $existingRow;
    }
...
function dialecticQuickstartApplyTtsSelection(TTSConnector $connector, $selectedDriver): int {
    $selectedDriver = dialecticQuickstartNormalizeTtsDriver($connector, $selectedDriver);
    $previousPreferredRow = dialecticResolvePreferredTtsConnectorRow();
    $previousPreferredId = intval($previousPreferredRow['id'] ?? 0);

    $selectedRow = dialecticQuickstartEnsureTtsConnectorForDriver($connector, $selectedDriver);
...
function dialecticQuickstartGetLlmConnectorLabelById($db, int $id): string {
    if ($id <= 0 || !$db) {
        return '';
    }
    try {
...
function dialecticQuickstartGetGeneralLlmConnectorSummary($db): array {
    $items = [
    ...
    foreach ($items as $settingId => $displayName) {
        $connectorId = dialecticGetGeneralSettingInt($settingId, 0);
        $label = dialecticQuickstartGetLlmConnectorLabelById($db, $connectorId);
        if ($label === '') {
            continue;
        }
        $summary[] = [
...
if (isset($_GET['minime_probe']) && strval($_GET['minime_probe']) === '1') {
    ...
    $rawUrl = trim(strval($_GET['url'] ?? dialecticQuickstartMiniMeDefaultUrl()));
    $result = [
        'ok' => false,
        'url' => $rawUrl,
        'http_code' => 0,
    ...
    $probe = dialecticQuickstartProbeUrl($rawUrl);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qs_action'])) {
    ...
    if ($action === 'profile_quicksave_metadata') {
        ...
        $worldknowledge  = $truthy($_POST['worldknowledge_infinium'] ?? null);
        $player2Force = $truthy($_POST['player2_force_all_llm'] ?? null);
        if ($worldknowledge !== null) {
            dialecticSetGeneralSetting('WORLDKNOWLEDGE_INFINIUM', $worldknowledge ? 'true' : 'false');
        }
    ...
    if ($action === 'save_quickstart') {
        ...
        try {
            ...
            // PLAYER_NAME is detected from the game and saved to core_player automatically.

            $ttsConnector = new TTSConnector();
            $selectedTtsDriver = dialecticQuickstartNormalizeTtsDriver($ttsConnector, $_POST['TTSFUNCTION'] ?? ($GLOBALS["TTSFUNCTION"] ?? 'none'));
            $savedTtsId = dialecticQuickstartApplyTtsSelection($ttsConnector, $selectedTtsDriver);

            $sttConnector = new STTConnector();
            $activeSttId = dialecticQuickstartEnsureActiveSttConnectorId($sttConnector);
            ...
            if ($activeSttId > 0 && $existingStt) {
                ...
                dialecticSetGeneralSetting('GLOBAL_STT_CONNECTOR_ID', $activeSttId, dialecticGetSchemaDescription('GLOBAL_STT_CONNECTOR_ID'));
                $savedSttId = $activeSttId;
            } else {
                $savedSttId = $sttConnector->create($sttPayload);
                if ($savedSttId > 0) {
                    dialecticSetGeneralSetting('GLOBAL_STT_CONNECTOR_ID', $savedSttId, dialecticGetSchemaDescription('GLOBAL_STT_CONNECTOR_ID'));
                }
            }
...
$quickstartActiveSttId = dialecticQuickstartEnsureActiveSttConnectorId($quickstartSttConnector);
...
$quickstartActiveTtsRow = dialecticResolvePreferredTtsConnectorRow();
$quickstartActiveTtsDriver = dialecticQuickstartNormalizeTtsDriver($quickstartTtsConnector, $quickstartActiveTtsRow['driver'] ?? ($GLOBALS["TTSFUNCTION"] ?? ''));
...
$generalLlmConnectorSummary = dialecticQuickstartGetGeneralLlmConnectorSummary($db);
...
echo '<section class="qs-section" id="qs_minime_section">
        ...
        <div class="form-group qs-field">
            ...
            <input id="qs_minime_probe_url" type="hidden" value="' . htmlspecialchars(dialecticQuickstartMiniMeDefaultUrl()) . '">
...
foreach ($quickstartConf as $pname => $parms) {
    ...
    if ($parms["type"] == "string") {
        ...
        if ($pname == "TTSFUNCTION") {
            ...
            $parms["description"] = "Select the TTS service you wish to use. Recommended: OmniVoice, PocketTTS or Chatterbox. <br>You can install OmniVoice, PocketTTS, Chatterbox, XTTS and configure Inworld in DIALECTIC. For provider-specific settings and advanced endpoint editing, use the <a href='" . $webRoot . "/ui/core/tts_connectors.php' target='_blank'>TTS Connectors</a> page.";
        } else if ($pname == "STTFUNCTION") {
...
echo '<style>
    @font-face { font-family: "Gothic821"; src: url("css/font/Gothic821CondensedRegular.otf") format("opentype"); font-weight: normal; font-style: normal; }
    /* Page shell */
    .qs-page {
        padding-top: 80px;
        padding-bottom: 40px;
    ...
    /* Headings styled like WorldKnowledge */
    .qs-title {
        margin: 0;
