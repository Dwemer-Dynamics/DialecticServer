...
function worldknowledge_render_context_rule_form(array $rule, bool $isNew = false): void
{
    ...
    $fields = [
        ...
        'location' => ['Location or Region', 'Current location or matching region.'],
        'worldspace' => ['Worldspace', 'Current Fallout worldspace.'],
        ...
        <div class="context-rule-grid">
            ...
            <?php foreach ($fields as $field => [$label, $description]): ?>
                <div class="context-rule-field">
                    ...
                    <input type="text" name="condition_<?php echo $field; ?>" value="<?php echo htmlspecialchars(worldknowledge_context_rule_condition_text($conditions, $field), ENT_QUOTES, 'UTF-8'); ?>">
                </div>
            <?php endforeach; ?>
        </div>
...
<div id="rules-tab" class="tab-content">
    <div class="full-width-section">
        <div class="context-rule-intro">
            <h2>&#127919; World Knowledge Context Rules</h2>
            <p>All populated conditions must match. Comma-separated values within one condition are alternatives. Leave all conditions blank for an always-on rule.</p>
            <p>Normal topic search and Force Location World Knowledge continue to work alongside these deterministic rules.</p>
        </div>

        <?php
        worldknowledge_render_context_rule_form([
            'priority' => 100,
            'selector_type' => 'topic',
            'max_articles' => 1,
            'conditions' => [],
        ...
        $contextRulesResult = pg_query(
            ...
            "SELECT id, label, enabled, priority, selector_type, selector_value, conditions, max_articles
               FROM {$schema}.worldknowledge_context_rule
              ORDER BY priority, id"
        );
        if ($contextRulesResult) {
            $contextRuleCount = 0;
            while ($contextRule = pg_fetch_assoc($contextRulesResult)) {
                worldknowledge_render_context_rule_form($contextRule, false);
                $contextRuleCount++;
            }
