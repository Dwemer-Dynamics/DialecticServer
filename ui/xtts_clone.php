...
if (!function_exists('dialecticTtsStudioTabToDriver')) {
    function dialecticTtsStudioTabToDriver(string $tab): string
    {
...
if (!function_exists('dialecticTtsStudioSpeakerCacheKey')) {
    function dialecticTtsStudioSpeakerCacheKey(string $driver, string $scope = ''): string
    {
...
if (!function_exists('dialecticTtsStudioSanitizeLanguage')) {
...
if (!function_exists('dialecticTtsStudioResolveConnectorRow')) {
    function dialecticTtsStudioResolveConnectorRow(string $driver): ?array
    {
...
if (!function_exists('dialecticTtsStudioResolveConnectorMetadata')) {
    function dialecticTtsStudioResolveConnectorMetadata(string $driver): array
    {
        $ttsConnector = new TTSConnector();
        $driver = $ttsConnector->normalizeDriverValue($driver);
        if ($driver === '') {
        ...
        $row = dialecticTtsStudioResolveConnectorRow($driver);
        if (!is_array($row)) {
            return [];
        }
...
if (!function_exists('dialecticTtsStudioResolveEndpointForDriver')) {
    function dialecticTtsStudioResolveEndpointForDriver(string $driver): string
    {
        $ttsConnector = new TTSConnector();
        $driver = $ttsConnector->normalizeDriverValue($driver);
        if ($driver === '') {
...
if (!function_exists('dialecticTtsStudioAudioCppBaseEndpoint')) {
    function dialecticTtsStudioAudioCppBaseEndpoint(string $endpoint): string
    {
        $endpoint = normalize_endpoint_url(trim($endpoint));
...
if (!function_exists('dialecticTtsStudioApplyConnectorGlobals')) {
    function dialecticTtsStudioApplyConnectorGlobals(string $driver): void
    {
        $ttsConnector = new TTSConnector();
        $driver = $ttsConnector->normalizeDriverValue($driver);
        if ($driver === '') {
...
if (!function_exists('dialecticTtsStudioFetchSpeakersList')) {
    function dialecticTtsStudioFetchSpeakersList(string $driver, string $language = ''): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver($driver);
        if ($endpoint === '') {
            return [];
        ...
        if ($driver === 'omnivoice') {
            $language = dialecticTtsStudioSanitizeLanguage($language);
            if ($language !== '') {
...
if (!function_exists('dialecticTtsStudioGetCachedSpeakersList')) {
    function dialecticTtsStudioGetCachedSpeakersList(string $driver, string $language = ''): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver($driver);
        if (dialecticTtsStudioIsAudioCppPocketTts($driver, $endpoint) && function_exists('getLocalVoices')) {
            return getLocalVoices();
        ...
        $cacheKey = dialecticTtsStudioSpeakerCacheKey($driver, $driver === 'omnivoice' ? $language : '');
        $speakers = $_SESSION[$cacheKey] ?? [];
        return is_array($speakers) ? $speakers : [];
    }
}

if (!function_exists('dialecticTtsStudioStoreSpeakersList')) {
    function dialecticTtsStudioStoreSpeakersList(string $driver, array $speakers, string $language = ''): void
    {
        $_SESSION[dialecticTtsStudioSpeakerCacheKey($driver, $driver === 'omnivoice' ? $language : '')] = normalizeSpeakersList($speakers);
    }
}

if (!function_exists('dialecticTtsStudioFetchOmniVoiceLibraries')) {
    function dialecticTtsStudioFetchOmniVoiceLibraries(): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver('omnivoice');
        if ($endpoint === '') {
            return [];
        }

        $probe = dialecticTtsStudioProbeJson($endpoint . '/voice_libraries');
...
if (!function_exists('dialecticTtsStudioResolveOmniVoiceLanguage')) {
    function dialecticTtsStudioResolveOmniVoiceLanguage($candidate = ''): string
    {
        $candidate = dialecticTtsStudioSanitizeLanguage($candidate);
        if ($candidate !== '') {
            return $candidate;
        }

        $metadata = dialecticTtsStudioResolveConnectorMetadata('omnivoice');
        $metadataLanguage = dialecticTtsStudioSanitizeLanguage($metadata['language'] ?? '');
        if ($metadataLanguage !== '') {
            return $metadataLanguage;
        }

        foreach (dialecticTtsStudioFetchOmniVoiceLibraries() as $library) {
            if (!is_array($library) || empty($library['active'])) {
                continue;
            }
            $libraryId = dialecticTtsStudioSanitizeLanguage($library['id'] ?? '');
            if ($libraryId !== '') {
                return $libraryId;
            }
        }
...
if (!function_exists('dialecticTtsStudioFetchOmniVoiceVoiceItems')) {
    function dialecticTtsStudioFetchOmniVoiceVoiceItems(string $language): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver('omnivoice');
        $language = dialecticTtsStudioResolveOmniVoiceLanguage($language);
        if ($endpoint === '') {
            return [];
        }

        $probe = dialecticTtsStudioProbeJson($endpoint . '/speakers_list_extended?language=' . rawurlencode($language));
        if ($probe['response'] === false || intval($probe['http_code']) < 200 || intval($probe['http_code']) >= 300) {
            return [];
        }
...
if (!function_exists('dialecticTtsStudioVoiceUploadPostFields')) {
    function dialecticTtsStudioVoiceUploadPostFields(string $driver, string $voicePath, string $fileType, string $fileName, string $language = ''): array
    {
        $postFields = [
        ...
        if ($driver === 'omnivoice') {
            ...
            $postFields['language'] = dialecticTtsStudioResolveOmniVoiceLanguage($language);
...
if (!function_exists('dialecticTtsStudioDeleteVoice')) {
    function dialecticTtsStudioDeleteVoice(string $driver, string $voice, string $language = ''): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver($driver);
        if ($endpoint === '') {
        ...
        if ($driver === 'omnivoice') {
            $url .= '?language=' . rawurlencode(dialecticTtsStudioResolveOmniVoiceLanguage($language));
        }
...
if (!function_exists('dialecticTtsStudioVoiceUploadResult')) {
    function dialecticTtsStudioVoiceUploadResult(string $driver, int $httpCode, string $response): array
    {
        if ($driver !== 'omnivoice') {
...
if (!function_exists('dialecticTtsStudioVoiceSamplePath')) {
    function dialecticTtsStudioVoiceSamplePath(string $voice): string
    {
        $voice = trim($voice);
...
if (!function_exists('dialecticTtsStudioSyncVoiceSampleForTest')) {
    function dialecticTtsStudioSyncVoiceSampleForTest(string $driver, string $voice, string $logFile = '', string $language = ''): array
    {
        $endpoint = dialecticTtsStudioResolveEndpointForDriver($driver);
        if ($endpoint === '') {
        ...
        $voiceSamplePath = dialecticTtsStudioVoiceSamplePath($voice);
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, dialecticTtsStudioVoiceUploadPostFields($driver, $voiceSamplePath, $fileType, $fileName, $language));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        $uploadResult = dialecticTtsStudioVoiceUploadResult($driver, $httpCode, strval($response));
        if ($curlError !== '') {
        ...
        if ($uploadResult['success']) {
            dialecticTtsStudioStoreSpeakersList($driver, dialecticTtsStudioFetchSpeakersList($driver, $language), $language);
...
if (!function_exists('dialecticTtsStudioConfigureCompatibleTestGlobals')) {
    function dialecticTtsStudioConfigureCompatibleTestGlobals(string $driver, string $ttsFunction, string $voice, string $language = ''): void
    {
        dialecticTtsStudioApplyConnectorGlobals($driver);
...
if (!function_exists('dialecticTtsStudioRunReturnLinesTest')) {
    function dialecticTtsStudioRunReturnLinesTest(string $testText, string $logFile): array
    {
...
if (isset($_GET['action']) && $_GET['action'] === 'test_xtts' && isset($_GET['voice'])) {
    ...
    dialecticTtsStudioApplyConnectorGlobals('xtts-fastapi');
    ...
    try {
        ...
        if ($file === '') {
            ...
            $syncResult = dialecticTtsStudioSyncVoiceSampleForTest('xtts-fastapi', $voice, $logFile);
            if ($syncResult['success']) {
                dialecticTtsStudioConfigureCompatibleTestGlobals('xtts-fastapi', 'xtts-fastapi', $voice);
                $retryResult = dialecticTtsStudioRunReturnLinesTest($testText, $logFile);
...
if (isset($_GET['action']) && $_GET['action'] === 'test_chatterbox' && isset($_GET['voice'])) {
    ...
    dialecticTtsStudioApplyConnectorGlobals('chatterbox');
    ...
    try {
        ...
        if ($file === '') {
            ...
            $syncResult = dialecticTtsStudioSyncVoiceSampleForTest('chatterbox', $voice, $logFile);
            if ($syncResult['success']) {
                dialecticTtsStudioConfigureCompatibleTestGlobals('chatterbox', 'chatterbox', $voice);
                $retryResult = dialecticTtsStudioRunReturnLinesTest($testText, $logFile);
                $file = $retryResult['file'];
                if ($file === '') {
...
if (isset($_GET['action']) && $_GET['action'] === 'test_pockettts' && isset($_GET['voice'])) {
    ...
    dialecticTtsStudioApplyConnectorGlobals('pockettts');
    ...
    try {
        ...
        if ($file === '') {
            ...
            $syncResult = dialecticTtsStudioSyncVoiceSampleForTest('pockettts', $voice, $logFile);
            if ($syncResult['success']) {
                dialecticTtsStudioConfigureCompatibleTestGlobals('pockettts', 'pockettts', $voice);
                $retryResult = dialecticTtsStudioRunReturnLinesTest($testText, $logFile);
                $file = $retryResult['file'];
                if ($file === '') {
...
if (isset($_GET['action']) && $_GET['action'] === 'test_omnivoice' && isset($_GET['voice'])) {
    ...
    $language = dialecticTtsStudioResolveOmniVoiceLanguage($_GET['language'] ?? '');
    ...
    dialecticTtsStudioConfigureCompatibleTestGlobals('omnivoice', 'omnivoice', $voice, $language);
    $testText = "Patrolling the Mojave almost makes you wish for a Nuclear Winter.";
    $testResult = dialecticTtsStudioRunReturnLinesTest($testText, $logFile);
    ...
    if ($file === '') {
        ...
        $syncResult = dialecticTtsStudioSyncVoiceSampleForTest('omnivoice', $voice, $logFile, $language);
        if ($syncResult['success']) {
            dialecticTtsStudioConfigureCompatibleTestGlobals('omnivoice', 'omnivoice', $voice, $language);
            $retryResult = dialecticTtsStudioRunReturnLinesTest($testText, $logFile);
            $file = $retryResult['file'];
...
if (isset($_GET['action']) && ($_GET['action'] === 'test_cartesia' || $_GET['action'] === 'test_inworld') && isset($_GET['voice'])) {
    ...
    function getClonedVoicesForTest($provider) {
        ...
        if ($provider === 'cartesia') {
            dialecticTtsStudioApplyConnectorGlobals('cartesia');
            require_once(__DIR__ . '/../tts/tts-cartesia.php');
            return getCartesiaCachedVoicesMap();
        }
        if ($provider === 'inworld') {
            dialecticTtsStudioApplyConnectorGlobals('inworld');
            require_once(__DIR__ . '/../tts/tts-inworld.php');
            return getInworldCachedVoicesMap();
        }
        $db = $GLOBALS["db"];
    ...
    try {
        ...
        if (in_array($provider, ['xtts', 'chatterbox', 'pockettts', 'omnivoice'], true)) {
            ...
            $language = $provider === 'omnivoice'
                ? dialecticTtsStudioResolveOmniVoiceLanguage($_GET['language'] ?? '')
                : '';

            $driver = dialecticTtsStudioTabToDriver($provider);
            $endpoint = dialecticTtsStudioResolveEndpointForDriver($driver);
            if ($endpoint === '') {
            ...
            if (dialecticTtsStudioIsAudioCppPocketTts($driver, $endpoint)) {
                dialecticTtsStudioStoreSpeakersList($driver, dialecticTtsStudioFetchSpeakersList($driver));
            ...
            $url = $endpoint . '/upload_sample';
            $postFields = dialecticTtsStudioVoiceUploadPostFields($driver, $voicePath, $fileType, $fileName, $language);
            ...
            if (curl_errno($ch)) {
                ...
                $uploadResult = dialecticTtsStudioVoiceUploadResult($driver, intval($httpCode), strval($curlResponse));
...
$activeTtsStudioDriver = dialecticTtsStudioTabToDriver($activeTab);
if ($activeTtsStudioDriver !== '') {
    $activeLanguageScope = $activeTtsStudioDriver === 'omnivoice' ? $activeOmniVoiceLanguage : '';
    $cachedSpeakers = dialecticTtsStudioGetCachedSpeakersList($activeTtsStudioDriver, $activeLanguageScope);
    if (empty($cachedSpeakers)) {
        dialecticTtsStudioStoreSpeakersList(
            $activeTtsStudioDriver,
            dialecticTtsStudioFetchSpeakersList($activeTtsStudioDriver, $activeLanguageScope),
            $activeLanguageScope
        );
    }
}
...
{
    $row = dialecticTtsStudioResolveConnectorRow('cartesia');
...
{
    $row = dialecticTtsStudioResolveConnectorRow('inworld');
    $metadata = dialecticTtsStudioResolveConnectorMetadata('inworld');
    $hasConnector = is_array($row);
...
function dialecticTtsStudioProbeJson(string $url, string $method = 'GET', ?array $payload = null): array
{
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
function dialecticTtsStudioDetectEndpointProvider(string $endpoint): array
{
    $endpoint = normalize_endpoint_url(trim($endpoint));
    if ($endpoint === '') {
        return [
    ...
    $chatterboxProbe = dialecticTtsStudioProbeJson($endpoint . '/speakers_list_extended');
    ...
    $xttsProbe = dialecticTtsStudioProbeJson($endpoint . '/languages');
...
function dialecticTtsStudioProbeEndpointStatus(string $driver, string $endpoint): array
{
    $endpoint = normalize_endpoint_url(trim($endpoint));
    if ($endpoint === '') {
        return [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'unsync_cartesia_single' && isset($_POST['voice'])) {
        ...
        $voice = resolveLocalVoiceName($_POST['voice']);
        if ($voice !== '') {
            deleteCachedCartesiaVoiceId($voice);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'unsync_inworld_single' && isset($_POST['voice'])) {
        ...
        $voice = resolveLocalVoiceName($_POST['voice']);
        if ($voice !== '') {
            deleteCachedInworldVoiceId($voice);
    ...
    if (isset($_POST['action'], $_POST['voice']) &&
        ...
        $driver = dialecticTtsStudioTabToDriver($redirectTab);
        $voice = trim(strval($_POST['voice']));
        $language = $driver === 'omnivoice'
            ? dialecticTtsStudioResolveOmniVoiceLanguage($_POST['language'] ?? ($_GET['language'] ?? ''))
            : '';
        $deleteResult = dialecticTtsStudioDeleteVoice($driver, $voice, $language);

        if ($deleteResult['success']) {
            dialecticTtsStudioStoreSpeakersList(
                $driver,
                dialecticTtsStudioFetchSpeakersList($driver, $language),
                $language
            );
    ...
    if (isset($_POST['action']) && isset($_POST['voice']) &&
        ...
        if ($endpoint !== '' && file_exists($voiceSamplePath)) {
            ...
            $url = $endpoint . '/upload_sample';
            $postFields = dialecticTtsStudioVoiceUploadPostFields($driver, $voiceSamplePath, $fileType, $fileName, $language);
            ...
            if (curl_errno($ch)) {
                ...
                $uploadResult = dialecticTtsStudioVoiceUploadResult($driver, intval($httpCode), strval($response));
    ...
    if (isset($_POST["get_speakers"])) {
        $refreshDriver = dialecticTtsStudioTabToDriver($activeTab);
        if ($refreshDriver !== '') {
            $refreshLanguage = $refreshDriver === 'omnivoice' ? $activeOmniVoiceLanguage : '';
            dialecticTtsStudioStoreSpeakersList(
                $refreshDriver,
                dialecticTtsStudioFetchSpeakersList($refreshDriver, $refreshLanguage),
                $refreshLanguage
            );
        }
    }
    if (isset($_POST["submit"])) {
        $submitDriver = dialecticTtsStudioTabToDriver($activeTab);
        $submitEndpoint = dialecticTtsStudioResolveEndpointForDriver($submitDriver);
        ...
        foreach (($submitEndpoint !== '' ? $filesToProcess : []) as $fileName) {
            ...
            $postFields = dialecticTtsStudioVoiceUploadPostFields($submitDriver, $destinationPath, $fileType, $fileName, $submitLanguage);
            ...
            if (curl_errno($ch)) {
                ...
                $uploadResult = dialecticTtsStudioVoiceUploadResult($submitDriver, intval($httpCode), strval($response));
                if ($uploadResult['success']) {
                    $uploadedCount++;
                } else {
        ...
        $uploadAllDriver = dialecticTtsStudioTabToDriver($activeTab);
        $uploadAllEndpoint = dialecticTtsStudioResolveEndpointForDriver($uploadAllDriver);
        ...
        if (dialecticTtsStudioIsAudioCppPocketTts($uploadAllDriver, $uploadAllEndpoint)) {
        ...
        foreach (($uploadAllEndpoint !== '' ? $files : []) as $filePath) {
            ...
            if ($fileType !== 'audio/wav' && $fileType !== 'audio/x-wav') {
                ...
                $postFields = dialecticTtsStudioVoiceUploadPostFields($uploadAllDriver, $filePath, $fileType, $fileName, $uploadAllLanguage);
                ...
                if (curl_errno($ch)) {
                    ...
                    $uploadResult = dialecticTtsStudioVoiceUploadResult($uploadAllDriver, intval($httpCode), strval($response));
                    if ($uploadResult['success']) {
                        $numUploaded++;
                    } else {
        ...
        dialecticTtsStudioStoreSpeakersList(
            $uploadAllDriver,
            dialecticTtsStudioFetchSpeakersList($uploadAllDriver, $uploadAllLanguage),
            $uploadAllLanguage
        );
        $verb = dialecticTtsStudioIsAudioCppPocketTts($uploadAllDriver, $uploadAllEndpoint)
...
<main>
    ...
    <div class="tab-content <?php echo $activeTab === 'xtts' ? 'active' : ''; ?>" data-tab-type="xtts">
        <div class="content-section full-width-section">
            ...
            <div style="margin-top: 20px; padding: 15px; background: rgba(74, 138, 182, 0.1); border: 1px solid #4a8ab6; border-radius: 6px;">
                ...
                <ul style="margin: 0; padding-left: 20px;">
                    ...
                    <li>Filename: lowercase with underscores (e.g., "doc_mitchell.wav")</li>
        ...
        <div class="content-section full-width-section">
            ...
            $localVoices = getLocalVoices();
            $xttsVoices = [];
            foreach (dialecticTtsStudioGetCachedSpeakersList('xtts-fastapi') as $speaker) {
        ...
        <div class="content-section full-width-section">
            ...
            <form action="<?php echo $webRoot; ?>/ui/xtts_clone.php?tab=xtts" method="post" onsubmit="showLoadingMessage('Syncing voice cache to XTTS server, this can take a couple minutes...');">
                ...
                <p>For cloud setup instructions, see our <a href="https://dwemerdynamics.com/dialectic/remote-hosting-guide.html" style="color: yellow;" target="_blank" rel="noopener noreferrer">Cloud XTTS Guide</a>.</p>
    ...
    <div class="tab-content <?php echo $activeTab === 'chatterbox' ? 'active' : ''; ?>" data-tab-type="chatterbox">
        ...
        <div class="content-section full-width-section">
            ...
            $serverVoices = [];
            foreach (dialecticTtsStudioGetCachedSpeakersList('chatterbox') as $speaker) {
                $displayName = basename($speaker, '.wav');
    ...
    <div class="tab-content <?php echo $activeTab === 'pockettts' ? 'active' : ''; ?>" data-tab-type="pockettts">
        ...
        <div class="content-section full-width-section">
            ...
            $localVoices = getLocalVoices();
            $xttsVoices = [];
            foreach (dialecticTtsStudioGetCachedSpeakersList('pockettts') as $speaker) {
                    $displayName = basename($speaker, '.wav');
                    $xttsVoices[$displayName] = true;
            }
            $pocketTtsEndpoint = dialecticTtsStudioResolveEndpointForDriver('pockettts');
            $pocketTtsCanDeleteRemote = !dialecticTtsStudioIsAudioCppPocketTts('pockettts', $pocketTtsEndpoint);
            ?>
    ...
    <div class="tab-content <?php echo $activeTab === 'omnivoice' ? 'active' : ''; ?>" data-tab-type="omnivoice">
        <?php
        $omnivoiceLibraries = dialecticTtsStudioFetchOmniVoiceLibraries();
        $activeOmniVoiceLanguage = dialecticTtsStudioResolveOmniVoiceLanguage($activeOmniVoiceLanguage);
        ...
        $omnivoiceItems = dialecticTtsStudioFetchOmniVoiceVoiceItems($activeOmniVoiceLanguage);
    ...
    <div class="tab-content <?php echo $activeTab === 'cartesia' ? 'active' : ''; ?>">
        ...
        <div class="content-section full-width-section">
            ...
            <p>For detailed information, see our <a href="https://dwemerdynamics.com/dialectic/tts.html#Cartesia" style="color: yellow;" target="_blank" rel="noopener noreferrer">Cartesia TTS Guide</a>.</p>
    ...
    <div class="tab-content <?php echo $activeTab === 'inworld' ? 'active' : ''; ?>">
        ...
        <div class="content-section full-width-section">
            ...
            <p>For detailed information, see our <a href="https://dwemerdynamics.com/dialectic/tts.html#Inworld" style="color: yellow;" target="_blank" rel="noopener noreferrer">Inworld TTS Guide</a>.</p>
            ...
            <div class="voice-status-grid">
                <?php foreach ($inworldDisplayVoices as $voice): ?>
                    ...
                    $inworldManaged = !empty($inworldMetadata['managed'])
                        && trim(strval($inworldMetadata['voice_id'] ?? '')) === strval($voiceStatus['cached_voice_id'] ?? '');
                    ?>
