...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_player'])) {
 try {
 // Player name is sourced from in-game telemetry and saved automatically.
 // Save player info
 ...
 $player->set('tts_filter_preset', dialecticVoiceFilterNormalizePreset($_POST['tts_filter_preset']));
 }
...
$playerTtsFilterPreset = strval($allPlayerData['tts_filter_preset'] ?? '');
...
?><link rel="stylesheet" href="<?php echo $webRoot; ?>/ui/css/main.css"><?php echo dialecticVoiceFilterAssetTags($webRoot); ?><style>
 ...
 padding: 14px;
 border-radius: 8px;
 border: 1px solid rgba(255, 182, 65, 0.18);
...
.stat-bar.action-points {
 ...
 statusIndicator.classList.toggle('status-disabled', !isEnabled);
  ...
  function playerTtsToast(message, isError=false, duration=3200) {
  ...
  async function previewPlayerTts() {
  const btn = document.getElementById('player_tts_preview_btn');
  const audio = document.getElementById('player_tts_preview_audio');
  if (!btn || !audio) return;
  const oldLabel = btn.textContent;
  btn.disabled = true;
  btn.textContent = 'Generating...';
  try {
  const response = await fetch('<?php echo $webRoot; ?>/ui/api/player_tts_preview.php', { cache: 'no-store' });
  let data = null;
  try { data = await response.json(); } catch (_e) {}
  if (!response.ok || !data || data.status !== 'success' || !data.url) {
  throw new Error((data && data.message) ? data.message : ('HTTP ' + response.status));
  }
  audio.src = data.url;
  await audio.play();
  playerTtsToast('Player TTS preview playing.', false, 3000);
  } catch (error) {
  playerTtsToast('Player TTS preview failed: ' + (error.message || 'Unknown error'), true, 5000);
  } finally {
  btn.disabled = false;
  btn.textContent = oldLabel;
  }
  }
  window.previewPlayerTts = previewPlayerTts;
  </script><?php if ($saveSuccess || $saveMessage): ?><script>
 setTimeout(function(){ 
 try{ 
 ...
 </label></div><span class="hint">If enabled, all NPCs know this bio. If disabled, only The Narrator knows it.</span></div><div class="content-section player-tts-section"><h2 class="section-title-with-status"><span class="section-title-text">Player TTS</span><span
 ...
 title="<?php echo $playerTtsConnectorId !== '' ? 'Player TTS is enabled' : 'Player TTS is disabled'; ?>"
  ><span class="status-dot" aria-hidden="true"></span><span id="player_tts_status_text"><?php echo $playerTtsConnectorId !== '' ? 'Enabled' : 'Disabled'; ?></span></span></h2><label for="tts_connector_id">TTS Connector</label><select id="tts_connector_id" name="tts_connector_id"><option value="">Disabled</option><?php foreach ($ttsConnectorRows as $row): ?><?php $rowId = strval($row['id'] ?? ''); ?><option value="<?php echo htmlspecialchars($rowId); ?>" <?php echo ($playerTtsConnectorId === $rowId) ? 'selected' : ''; ?>><?php echo htmlspecialchars(strval($row['label'] ?? ('Connector #' . $rowId))); ?></option><?php endforeach; ?></select><span class="hint">Select the connector used when Player TTS generates spoken responses.</span><label for="tts_voice_override">VoiceID</label><input type="text" id="tts_voice_override" name="tts_voice_override" value="<?php echo htmlspecialchars($playerTtsVoiceId); ?>" placeholder="TheNarrator"><span class="hint">Dedicated voice identifier used for Player TTS.</span><?php echo dialecticRenderVoiceFilterField([
 'id' => 'tts_filter_preset',
 'value' => $playerTtsFilterPreset,
 'scope' => 'player',
 'connector_source' => '#tts_connector_id',
 'voice_source' => '#tts_voice_override',
 'hint' => 'Fixed audio filter applied to the player voice. The play button next to it uses the connector, VoiceID, and filter currently selected above, including unsaved changes.',
]); ?><div class="speech-style-tools"><button id="player_tts_preview_btn" type="button" class="btn-ai-generate" onclick="previewPlayerTts()">Preview Player Voice</button><audio id="player_tts_preview_audio" preload="none"></audio></div><span class="hint">Plays your last <strong>saved</strong> Player TTS settings end to end. To audition unsaved connector, VoiceID, or Voice Filter choices, use the play button beside Voice Filter.</span><div id="player_tts_elevenlabs_panel" class="player-provider-panel" style="display:none;"><h3>ElevenLabs Player Overrides</h3><span class="hint">Only used when the selected Player TTS connector is ElevenLabs. Leave a field blank to inherit the connector default.</span><div class="player-provider-grid"><div><label for="tts_elevenlabs_model_id">Model ID</label><input type="text" id="tts_elevenlabs_model_id" name="tts_elevenlabs_model_id" value="<?php echo htmlspecialchars($playerTtsElevenModelId); ?>" placeholder="eleven_v3"><span class="hint">Examples: <code>eleven_multilingual_v2</code>, <code>eleven_v3</code>.</span></div><div><label for="tts_elevenlabs_speed">Speed</label><input type="number" step="0.05" id="tts_elevenlabs_speed" name="tts_elevenlabs_speed" value="<?php echo htmlspecialchars($playerTtsElevenSpeed); ?>" placeholder="1.0"><span class="hint">Player-only speed override for ElevenLabs.</span></div><div><label for="tts_elevenlabs_stability">Stability</label><input type="number" step="0.05" id="tts_elevenlabs_stability" name="tts_elevenlabs_stability" value="<?php echo htmlspecialchars($playerTtsElevenStability); ?>" placeholder="0.75"></div><div><label for="tts_elevenlabs_similarity_boost">Similarity Boost</label><input type="number" step="0.05" id="tts_elevenlabs_similarity_boost" name="tts_elevenlabs_similarity_boost" value="<?php echo htmlspecialchars($playerTtsElevenSimilarityBoost); ?>" placeholder="0.75"></div><div><label for="tts_elevenlabs_style">Style</label><input type="number" step="0.05" id="tts_elevenlabs_style" name="tts_elevenlabs_style" value="<?php echo htmlspecialchars($playerTtsElevenStyle); ?>" placeholder="0.0"></div><div><label for="tts_elevenlabs_use_speaker_boost">Speaker Boost</label><select id="tts_elevenlabs_use_speaker_boost" name="tts_elevenlabs_use_speaker_boost"><option value="" <?php echo $playerTtsElevenUseSpeakerBoost === '' ? 'selected' : ''; ?>>Use Connector Default</option><option value="true" <?php echo $playerTtsElevenUseSpeakerBoost === 'true' ? 'selected' : ''; ?>>Enabled</option><option value="false" <?php echo $playerTtsElevenUseSpeakerBoost === 'false' ? 'selected' : ''; ?>>Disabled</option></select><span class="hint">Eleven v3 ignores Speaker Boost.</span></div><div style="grid-column: 1 / -1;"><label for="tts_elevenlabs_v3_audio_tags">V3 Enhancers</label><textarea id="tts_elevenlabs_v3_audio_tags" name="tts_elevenlabs_v3_audio_tags" placeholder="[whispers] [curious]"><?php echo htmlspecialchars($playerTtsElevenV3AudioTags); ?></textarea><span class="hint">Prepended to the Player TTS input when the effective model is <code>eleven_v3</code>. Use ElevenLabs-style audio tags here.</span></div></div></div><label for="speech_style">Player Speech Style</label><textarea id="speech_style" name="speech_style" placeholder="Describe how your character speaks and communicates..."><?php echo htmlspecialchars($speechStyle); ?></textarea><span class="hint">Used for Auto Chat mode and player-style generation prompts.</span><div class="speech-style-tools"><label for="speech_style_guidance">AI Generation</label><textarea id="speech_style_guidance" placeholder="Optional: mention traits or tone to prioritize when generating your speech style paragraph."></textarea><button id="generate_speech_style_btn" type="button" class="btn-ai-generate" onclick="generatePlayerSpeechStyle()">AI Generate From Last 200 Inputs</button></div><span class="hint">Reads up to the last 200 player input events and generates a one-paragraph speech style prompt.</span></div><div class="content-section"><h2> Player Diary</h2><input type="hidden" name="diary_enabled" value="0"><input type="hidden" name="auto_diary_enabled" value="0"><input type="hidden" name="auto_diary_wait_enabled" value="0"><label class="toggle-row"><div class="toggle-switch"><input
 type="checkbox"
 id="diary_enabled"
 name="diary_enabled"
 value="1"
 <?php echo $playerDiaryEnabled ? 'checked' : ''; ?>
 ><span class="toggle-slider"></span></div><span class="toggle-label">Enable <?php echo htmlspecialchars($playerName); ?>'s Diary</span></label><span class="hint">Allows <?php echo htmlspecialchars($playerName); ?> to write diary entries. This can be triggered by the Prisma Actions menu or Auto Diary.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="auto_diary_enabled" name="auto_diary_enabled" value="1" <?php echo $playerAutoDiaryEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Player Auto Diary</span></label><span class="hint">Automatically writes <?php echo htmlspecialchars($playerName); ?>'s diary when sleeping. Requires Player Diary to be enabled.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="auto_diary_wait_enabled" name="auto_diary_wait_enabled" value="1" <?php echo $playerAutoDiaryWaitEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Player Auto Diary Wait</span></label><span class="hint">Also writes <?php echo htmlspecialchars($playerName); ?>'s diary when waiting. Requires Player Diary and Player Auto Diary to be enabled.</span></div></div></form><!-- Read-only Game Data Section --><div class="full-width-section"><h2> Player Statistics</h2></div><div class="content-grid two-col"><!-- Inventory Card --><?php if (!empty($inventory)): ?><div class="content-section"><h2>Inventory (<?php echo count($inventory); ?> items)</h2><div class="inventory-container"><div class="inventory-list"><?php 
 // Sort inventory by name
 usort($inventory, function($a, $b) {
 return strcmp($a['name'] ?? '', $b['name'] ?? '');
 });
 foreach ($inventory as $item): 
 ?><div class="inventory-item"><span class="inventory-item-name"><?php echo htmlspecialchars($item['name'] ?? 'Unknown Item'); ?></span><span class="inventory-item-count"><?php echo intval($item['count'] ?? 1); ?></span></div><?php endforeach; ?></div></div></div><?php else: ?><div class="content-section"><h2>Inventory</h2><div class="no-data">No inventory data available. Play the game to sync your inventory.</div></div><?php endif; ?><!-- Equipment Card --><?php
 $equipmentSlots = [
 ['label' => 'Face', 'keys' => ['face', 'head', 'helmet']],
 ['label' => 'Head', 'keys' => ['hair', 'head']],
 ['label' => 'Body', 'keys' => ['body', 'upper_body', 'armor']],
 ['label' => 'Boots', 'keys' => ['boots']],
 ['label' => 'Gloves', 'keys' => ['gloves']],
 ['label' => 'Amulet', 'keys' => ['amulet']],
 ['label' => 'Ring', 'keys' => ['ring']],
 ['label' => 'Cape', 'keys' => ['cape']],
 ['label' => 'Backpack', 'keys' => ['backpack']],
 ['label' => 'Left Hand', 'keys' => ['left_hand']],
 ['label' => 'Right Hand', 'keys' => ['right_hand']],
 ];
 $equipmentFirstValue = function(array $equipment, array $keys) {
 foreach ($keys as $key) {
 if (isset($equipment[$key]) && !empty($equipment[$key])) {
 ...
 $hasEquipment = false;
 foreach ($equipmentSlots as $slotInfo) {
 $itemName = $equipmentFirstValue($equipment, $slotInfo['keys']);
 if ($itemName) {
 $hasEquipment = true;
 break;
 }
 }
 ?><div class="content-section"><h2>Equipment</h2><?php if (!empty($equipment)): ?><?php if ($hasEquipment): ?><div class="equipment-grid"><?php foreach ($equipmentSlots as $slotInfo):
 $label = $slotInfo['label'];
 $itemName = $equipmentFirstValue($equipment, $slotInfo['keys']);
 ?><div class="equipment-slot"><div class="equipment-slot-name"><?php echo $label; ?></div><?php if ($itemName): ?><div class="equipment-item-name"><?php echo htmlspecialchars($itemName); ?></div><?php else: ?><div class="equipment-empty">Empty</div><?php endif; ?></div><?php endforeach; ?></div><?php else: ?><div class="no-data"><p><strong>No equipment currently equipped.</strong></p><p>If you have items equipped in-game but they are not showing here:</p><ul><li>Make sure you are in-game (not in a menu)</li><li>Talk to any NPC to trigger a sync</li><li>Or wait a few seconds for auto-sync</li><li>Then refresh this page</li></ul></div><?php endif; ?><?php else: ?><div class="no-data">No equipment data available. Play the game to sync your equipment.</div><?php endif; ?></div><!-- Stats Card --><?php if (!empty($stats)): ?><div class="content-section"><h2>Character Stats</h2><div class="stats-grid"><div class="stat-card"><div class="stat-card-title">Level</div><div class="stat-card-value"><?php echo intval($stats['level'] ?? 1); ?></div></div><div class="stat-card"><div class="stat-card-title">Health</div><div class="stat-card-value"><?php 
 $hp = floatval($stats['health'] ?? 0);
 $hpMax = floatval($stats['health_max'] ?? 1);
 echo round($hp) . ' / ' . round($hpMax);
 ?></div><div class="stat-bar-container"><div class="stat-bar health" style="width: <?php echo min(100, ($hp / max(1, $hpMax)) * 100); ?>%"></div></div></div><div class="stat-card"><div class="stat-card-title">Action Points</div><div class="stat-card-value"><?php
 $ap = floatval($stats['action_points'] ?? 0);
 $apMax = floatval($stats['action_points_max'] ?? 1);
 echo round($ap) . ' / ' . round($apMax);
 ?></div><div class="stat-bar-container"><div class="stat-bar action-points" style="width: <?php echo min(100, ($ap / max(1, $apMax)) * 100); ?>%"></div></div></div><div class="stat-card"><div class="stat-card-title">Karma</div><div class="stat-card-value"><?php
 echo intval($stats['karma'] ?? 0);
 ?></div></div></div></div><?php endif; ?></div><!-- Skills Section --><?php if (!empty($skills)): ?><div class="content-section full-width-section"><h2> Skills</h2><div class="skills-grid"><?php
