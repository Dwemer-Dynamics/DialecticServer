...
// Determine web root similar to worldknowledge_upload
...
.llm-left .llm-title { font-family: 'Gothic821', serif; letter-spacing: 0.6px; word-spacing: 10px; }
.conn-li { 
    border:1px solid #3a3a3a; 
...
.profile-test-title { font-size:18px; font-weight:800; color:rgb(255, 182, 65); }
...
.profile-test-progress > div { height:100%; width:0%; background:linear-gradient(90deg, rgb(255, 182, 65), #ffd089); transition:width .2s ease; }
...
.provider-title { display:flex; align-items:center; gap:10px; color:#e0e0e0; font-size:14px; line-height:1.25; }
...
.setting-icon { width:20px; text-align:center; color: rgb(255, 182, 65); }
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["inline_update_profile"])) {
    ...
    try {
        ...
        $allowed = [
            ...
            'diary_connector_id','tts_connector_id'
        ];
...
if (!function_exists('dialecticNormalizeImportedProfileLabel')) {
    function dialecticNormalizeImportedProfileLabel($rawLabel, $fileName = '')
    {
...
if (!function_exists('dialecticUniqueProfileLabel')) {
    function dialecticUniqueProfileLabel($label)
    {
...
if (!function_exists('dialecticNullIfBlank')) {
    function dialecticNullIfBlank($value)
    {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["import_profile"])) {
    ...
    try {
        ...
        $profileLabel = dialecticUniqueProfileLabel(dialecticNormalizeImportedProfileLabel($profileData['label'] ?? '', $importFileName));
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["create_import_rule"])) {
    ...
    $data = [
        ...
        'match_name' => dialecticNullIfBlank($_POST['match_name'] ?? ''),
...
if (isset($_GET["create_blank"])) {
    try {
        $defaultMeta = json_encode(CoreProfile::defaultMetadata(), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
...
$npcCountRows = $GLOBALS["db"]->fetchAll("SELECT profile_id, COUNT(*) AS c FROM core_npc_master WHERE trim(lower(npc_name)) <> 'the narrator' GROUP BY profile_id");
$profileIdToNpcCount = [];
...
const saveAllBtn = document.getElementById('btn_save_all');
            ...
            try {
                if (!empty($editItem["metadata"])) {
                    ...
                    if (is_array($metaData)) {
                        ...
                        foreach (['TTSFUNCTION'] as $runtimeKey) {
                            if (array_key_exists($runtimeKey, $metaData)) {
                                $currentProfileOverrides[$runtimeKey] = $metaData[$runtimeKey];
                            }
                        }
                    }
                }
    ...
    // Persist embedded LLM editors before saving profile
    async function saveAllConnectorEditors(){
        ...
        // Inline connector editors are no longer used; embedded LLM editors are the source of truth.
    ...
    function renderKVList(obj, keys, labels){
        ...
        for (let i=0;i<keys.length;i++){
            ...
            html += `<div style="color:rgb(255, 182, 65); font-weight:bold;">${lab}</div><div style="overflow-wrap:anywhere;">${val}</div>`;
        }
        html += '</div>';
        return html;
    }
                ...
                <strong>About Profile Rules:</strong>
                    ...
                    <li><strong>Regex examples:</strong>
                        <ul style="margin: 4px 0 0 16px; padding: 0;">
                            <li>
                                <strong>Name exact</strong>: <code>^veronica$</code>
                                <div style="color:#9fb1c9; font-size:12px; margin-top:2px;">matches: veronica &nbsp;|&nbsp; does not match: boone</div>
                            </li>
                            <li>
                                <strong>Name starts with</strong>: <code>^doc</code>
                                <div style="color:#9fb1c9; font-size:12px; margin-top:2px;">matches: doc_mitchell &nbsp;|&nbsp; does not match: veronica</div>
                            </li>
                            <li>
                                <strong>Name contains</strong>: <code>boone</code>
                                <div style="color:#9fb1c9; font-size:12px; margin-top:2px;">matches: craig_boone &nbsp;|&nbsp; does not match: dialectic</div>
                            </li>
                            <li>
                                <strong>Names one of</strong>: <code>^(veronica|dialectic)$</code>
                                <div style="color:#9fb1c9; font-size:12px; margin-top:2px;">matches: veronica, dialectic &nbsp;|&nbsp; does not match: boone</div>
                            </li>
                            <li>
                            ...
                            <li>
                                ...
                                <div style="color:#9fb1c9; font-size:12px; margin-top:2px;">matches when any active NPC faction name satisfies the expression</div>
                                    ...
                                    <li><strong>Mods:</strong> Comma-separated list (e.g., "FalloutNV.esm,Dialectic.esp")</li>
                    <li><strong>Action:</strong> JSON object with NPC fields to set (e.g., <code>{"voiceid": "FemaleAdult01", "refid": "000E32A9"}</code>)</li>
    ...
    <div class="modal-container" style="max-width: 700px;">
        ...
        <div class="modal-body" style="padding: 16px;">
            ...
            <div style="margin-bottom: 16px;">
                <label for="import_file" style="display: block; font-weight: 700; color: rgb(255, 182, 65); margin-bottom: 8px;">
            ...
            <div style="margin-bottom: 16px; padding: 12px; background: #141414; border: 1px solid #3a3a3a; border-radius: 8px;">
                <div style="font-weight: 700; color: rgb(255, 182, 65); margin-bottom: 8px;">Import Assignment Options</div>
            ...
            <div id="import_preview" style="display: none; margin-bottom: 16px;">
                <div style="font-weight: 700; color: rgb(255, 182, 65); margin-bottom: 8px;">Preview:</div>
...
#import_profile_modal .modal-title { margin:0; font-weight:700; color: rgb(255, 182, 65); font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.6em; }
...
#import_rules_modal .modal-title { margin:0; font-weight:700; color: rgb(255, 182, 65); font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.6em; }
...
.rule-card.editing { border-color: rgb(255, 182, 65); }
.rule-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
.rule-title { font-weight: 700; color: rgb(255, 182, 65); font-size: 1.1em; }
...
.rule-input:focus { border-color: rgb(255, 182, 65); outline: none; }
.rule-input.json { font-family: monospace; min-height: 60px; }
.rule-checkbox { accent-color: rgb(255, 182, 65); transform: scale(1.4); cursor: pointer; }
        ...
        saveRule: async function(id) {
            ...
            (function(){
                ...
                try {
                    ...
                    alert('Action must be valid JSON. Example: {"voiceid":"FemaleAdult01"}');
                    throw e;
                }
            })();
