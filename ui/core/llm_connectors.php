...
function dialectic_llm_apply_connector_metadata_post_overrides(array $metadata, array $post): array {
...
function find_player2_api_badge_id() {
 ...
 "WHERE lower(label) IN ('player2','dialectic') " .
...
// Determine web root and bring in site chrome like worldknowledge_upload
...
</style><main class="d-flex flex-column"><?php
$noticeMsg = '';
...
if (isset($_GET["partial"]) && $_GET["partial"] === "editor") {
 ...
 .service-icon.active { outline:2px solid rgb(255, 182, 65); opacity:1; }
 ...
 </script><form method="post" onsubmit='if (window.isInIframe) { window.handleEmbeddedSave(); return false; } return consolidation();' style='<?= $editItem!=null?"":"display:none"?>'><?php if ($editItem): ?><input type="hidden" name="id" value="<?= $editItem["id"] ?>"><?php endif; ?><input type="hidden" name="partial" value="editor"><div class="two-col-llm"><div><div class="top-actions" style="display:flex; gap:8px; align-items:center; margin-bottom:8px;"><?php if ($editItem): ?><button type="submit" name="save" class="btn-save">Save</button><button type="button" id="btn_test_connector" class="btn-primary" style="display:none;">Test</button><div class="orm-note" id="test_note" style="margin-top:6px; display:none;">Please save any changes before testing to ensure the latest settings are used.</div><?php else: ?><button type="submit" name="create" class="btn-save">Create</button><?php endif; ?></div><label for='label'>Name</label><br><input type="text" name="label" value="<?= htmlspecialchars($editItem["label"] ?? "") ?>"><br><label id="service_label">Service</label><div class="service-picker"><div class="service-icons"><img src="<?= $webRoot; ?>/ui/images/core/icons/openrouter.jpg" alt="OpenRouter" class="service-icon" data-service="openrouter" /><img src="<?= $webRoot; ?>/ui/images/core/icons/openai.jpg" alt="OpenAI" class="service-icon" data-service="openai" /><img src="<?= $webRoot; ?>/ui/images/core/icons/google.jpg" alt="Google" class="service-icon" data-service="google" /><img src="<?= $webRoot; ?>/ui/images/core/icons/groq.jpg" alt="Groq" class="service-icon" data-service="groq" /><img src="<?= $webRoot; ?>/ui/images/core/icons/nanogpt.jpg" alt="NanoGPT" class="service-icon" data-service="nanogpt" /><img src="<?= $webRoot; ?>/ui/images/core/icons/player2.jpg" alt="Player2" class="service-icon" data-service="player2" /><img src="<?= $webRoot; ?>/ui/images/core/icons/custom.jpg" alt="Custom" class="service-icon" data-service="custom" /></div></div><input type="hidden" id="service_input" name="service" value="<?= htmlspecialchars($editItem["service"] ?? "") ?>"><div id="service_signup_link" class="orm-note" style="font-size:12px; margin:-6px 0 8px 0; display:none;"><a id="signup_link" href="#" target="_blank" rel="noopener noreferrer" style="color:#ffb862; text-decoration:underline;">Sign up here</a> to get your API key for this service.
 </div><div id="service_tos_warning" class="orm-note" style="font-size:12px; margin:-4px 0 8px 0; display:none; color:#ffd2a6;">
 ...
 </div><div id="custom_note" class="orm-muted" style="font-size:12px; display:none; margin:-6px 0 8px 0;">
 Custom allows you to build your own connector setting using one of our API drivers to use non-supported services with DIALECTIC. For advanced users only
 </div><div id="url_row"><label for='url'>URL</label><br><div style="display:flex; gap:8px; align-items:center;"><input type="text" name="url" value="<?= htmlspecialchars($editItem["url"] ?? "") ?>" style="flex:1 1 auto;"><button type="button" id="btn_wsl_ip" class="btn-primary" title="Use WSL IP" style="display:none;" data-ip="<?= htmlspecialchars($WSL_IP) ?>">WSL IP</button><button type="button" id="btn_host_ip" class="btn-primary" title="Use HOST IP" style="display:none;" data-ip="<?= htmlspecialchars($HOST_IP) ?>">PC IP</button></div></div><div id="model_row"><label for='model'>Model</label><br><input type="text" name="model" value="<?= htmlspecialchars($editItem["model"] ?? "") ?>"><br></div><div id="provider_row"><label for='provider'>Provider</label><br><input type="text" name="provider" placeholder="(Optional) leave empty to use recommended provider" value="<?= htmlspecialchars($editItem["provider"] ?? "") ?>"><br></div><div id="driver_row" style="display:none;"><label for='driver'>Driver</label><br><input type="text" id="driver_input" name="driver" value="<?= htmlspecialchars($editItem["driver"] ?? "") ?>" style="display:none"><br><select id="driver_select" style="display:none"><option value="openrouterjson">OpenRouter JSON</option><option value="openaijson">OpenAI JSON</option><option value="google_openaijson">Google OpenAI JSON</option><option value="groqjson">Groq JSON</option><option value="player2json">Player2 JSON</option></select></div><div id="api_key_row"><?php
 ...
 ?></div><div id="reasoning_row"><label class="label-with-toggle"><span class='tip-label' data-tip='Fixes reasoning only models so they do not output <think> and <answer> tags. May be slower then regular models.'>Reasoning Model Fix</span><input type="hidden" name="reasoning_model" value="0"><input type="checkbox" name="reasoning_model" value="1" <?= isset($editItem["reasoning_model"]) && $editItem["reasoning_model"] == 1 ? "checked" : "" ?>></label></div><div id="json_toggles" style="margin-top:8px;"><label class="label-with-toggle"><span class='tip-label' data-tip='Force responses to be strict JSON. NonJSON output may be rejected or autoretried.'>Enforce JSON</span><input type="hidden" name="enforce_json" value="0"><input type="checkbox" name="enforce_json" value="1" <?= isset($editItem["enforce_json"]) && $editItem["enforce_json"] == 1 ? "checked" : "" ?>></label><div style="height:6px;"></div><label class="label-with-toggle"><span class='tip-label' data-tip='Guide/validate the JSON structure with a schema. Best used with Enforce JSON.'>JSON Schema</span><input type="hidden" name="json_schema" value="0"><input type="checkbox" name="json_schema" value="1" <?= isset($editItem["json_schema"]) && $editItem["json_schema"] == 1 ? "checked" : "" ?>></label><div style="height:6px;"></div><label class="label-with-toggle"><span class='tip-label' data-tip='Send a starter JSON object to steer field names/shape in the response.'>Prefill JSON</span><input type="hidden" name="prefill_json" value="0"><input type="checkbox" name="prefill_json" value="1" <?= isset($editItem["prefill_json"]) && $editItem["prefill_json"] == 1 ? "checked" : "" ?>></label></div><div id="remove_action_prompt" style="margin-top:12px;"><label class="label-with-toggle"><span class='tip-label' data-tip='Disable SSE streaming for this connector and wait for the full JSON reply before parsing. Useful for local LM Studio or other OpenAI-compatible servers that stream slowly or emit long reasoning chunks first.'>Disable Streaming</span><input type="hidden" name="disable_streaming" value="0"><input type="checkbox" name="disable_streaming" value="1" <?php 
 $metadata = [];
 ...
 ?>></label><div style="height:6px;"></div><label class="label-with-toggle"><span class='tip-label' data-tip='Option to disable the action enforcement prompt. Some models like gemini-3-flash tend to use actions a lot.'>Remove Action Prompt</span><input type="hidden" name="remove_action_prompt" value="0"><input type="checkbox" name="remove_action_prompt" value="1" <?php 
 $metadata = [];
 if (isset($editItem["metadata"]) && !empty($editItem["metadata"])) {
 $metadata = is_string($editItem["metadata"]) ? json_decode($editItem["metadata"], true) : $editItem["metadata"];
 if (!is_array($metadata)) $metadata = [];
 ...
 echo "</div>";
 echo "</div>";
 ?></div></div></form><script src='https://cdnjs.cloudflare.com/ajax/libs/ace/1.23.4/ace.js'></script><script src='https://cdnjs.cloudflare.com/ajax/libs/js-yaml/4.1.0/js-yaml.min.js'></script><script>
 (function(){
 ...
 const apiBadgeLabelMatch = { openrouter: ['openrouter'], openai: ['openai'], google: ['google'], groq: ['groq'], nanogpt: ['nano-gpt','nanogpt'], player2: ['player2','dialectic'] };
 ...
 modal.innerHTML = `
 <div style="width:90%; max-width:1200px; height:80vh; background:#2a2a2a; border:1px solid #4a4a4a; border-radius:10px; box-shadow:0 10px 30px rgba(0,0,0,0.6); position:relative; overflow:hidden;"><button id=\"llmtest_close\" class=\"btn-secondary\" style=\"position:absolute; top:8px; right:10px; z-index:3;\">Close</button><div id=\"llmtest_loading\" style=\"position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.4); z-index:2;\"><div style=\"width:48px; height:48px; border:4px solid rgba(255,255,255,0.25); border-top-color:#ffb862; border-radius:50%; animation: llmspin 1s linear infinite;\"></div></div><iframe id=\"llmtest_iframe\" src=\"about:blank\" style=\"width:100%; height:100%; border:0; background:transparent; position:relative; z-index:1;\"></iframe></div><style>@keyframes llmspin{to{transform:rotate(360deg)}}</style>`;
 ...
 </script><?php if ($noticeMsg !== ''): ?><script>
 (function(){
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["create"])) {
 ...
 dialectic_llm_apply_connector_metadata_post_overrides($metadata, $payload),
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && (isset($_POST["save"]) || isset($_POST["update"])) ) {
 ...
 dialectic_llm_apply_connector_metadata_post_overrides($metadata, $_POST),
 JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES
 );
...
?><div class="page-header"><h1 class="api-title">LLM Connectors</h1><p class="page-subtitle">Configure Language Model connectors for AI dialogue generation</p></div><div id="toast" class="toast-notification" style="position:static; margin: 8px auto 12px; display:block; opacity:0; transform:none; max-width:960px; width: calc(100% - 20px);"><span class="message"></span></div><div class="llm-layout"><div class="llm-left position-sticky"><div class="sidebar-action-grid"><form method="get" action="llm_connectors.php"><input type="hidden" name="create_blank" value="1"><button type="submit" class="btn-save">New</button></form><form method="post" action="llm_connectors.php" enctype="multipart/form-data" id="llm_import_form"><input type="hidden" name="import" value="1"><input type="file" name="import_file[]" id="llm_import_file" accept=".csv" multiple style="display:none"><button type="button" class="btn-primary" id="llm_import_btn">Import</button></form></div><div id="llm_list" class="conn-list"></div><script>
 ...
 html += `
 <div class="conn-li${active}" data-id="${String(r.id)}"><div class="head"><div class="title">${escapeHtml(r.label||('Connector #'+r.id))}</div><div class="badge">${escapeHtml(r.driver||'')}</div></div><div class="sub">${escapeHtml(r.model||'')}</div><div class="actions"><form method="get" action="llm_connectors.php" onsubmit="return confirm('Are you sure you want to delete this connector?');" style="display:inline"><input type="hidden" name="delete" value="${String(r.id)}"><button type="submit" class="btn-danger">Delete</button></form><form method="get" action="llm_connectors.php" style="display:inline"><input type="hidden" name="clone" value="${String(r.id)}"><button type="submit" class="btn-primary">Clone</button></form></div></div>`;
 });
 ...
 </script></div><div class="llm-right"><div class="form-container wide-centered"><style>
...
.conn-li .head { display:flex; justify-content:space-between; gap:8px; align-items:flex-start; }
.conn-li .title { font-weight:600; color:#e9efff; font-size:14px; line-height:1.2; overflow-wrap:anywhere; }
...
</style><?php if (!$editItem): ?><div class="connector-placeholder" style="border:1px solid #3a3a3a; background: linear-gradient(180deg, rgba(42, 42, 42, 0.95), rgba(34, 34, 34, 0.98)); color:#9fb1c9; border-radius:10px; padding:18px; margin-bottom:10px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.1);"><div style="font-weight:600; color:#e9efff; margin-bottom:6px;">No connector selected</div><div>Select a connector from the list on the left to view and edit its settings.</div></div><?php endif; ?><script>
// Ensure consolidation() exists even without metadata editor
if (typeof window.consolidation !== 'function') {
 window.consolidation = function(){ return true; };
}
</script><form method="post" onsubmit='return consolidation()' style='<?= $editItem!=null?"":"display:none"?>'><?php if ($editItem): ?><input type="hidden" name="id" value="<?= $editItem["id"] ?>"><?php endif; ?><div class="two-col-llm"><div><div class="top-actions" style="display:flex; gap:8px; align-items:center; margin-bottom:8px;"><?php if ($editItem): ?><button type="submit" name="save" class="btn-save">Save</button><button type="button" id="btn_test_connector_main" class="btn-primary">Test</button><button type="submit" formmethod="get" formaction="llm_connectors.php" name="export" value="<?= htmlspecialchars($editItem['id'] ?? '') ?>" class="btn-primary">Export</button><div class="orm-note" style="margin-top:6px;">Please save any changes before testing to ensure the latest settings are used.</div><?php else: ?><button type="submit" name="create" class="btn-save">Create</button><?php endif; ?></div><label for='label'>Name</label><br><input type="text" name="label" value="<?= htmlspecialchars($editItem["label"] ?? "") ?>"><br><label id="service_label">Service</label><div class="service-picker"><div class="service-icons"><img src="<?= $webRoot; ?>/ui/images/core/icons/openrouter.jpg" alt="OpenRouter" class="service-icon" data-service="openrouter" /><img src="<?= $webRoot; ?>/ui/images/core/icons/openai.jpg" alt="OpenAI" class="service-icon" data-service="openai" /><img src="<?= $webRoot; ?>/ui/images/core/icons/google.jpg" alt="Google" class="service-icon" data-service="google" /><img src="<?= $webRoot; ?>/ui/images/core/icons/groq.jpg" alt="Groq" class="service-icon" data-service="groq" /><img src="<?= $webRoot; ?>/ui/images/core/icons/nanogpt.jpg" alt="NanoGPT" class="service-icon" data-service="nanogpt" /><img src="<?= $webRoot; ?>/ui/images/core/icons/player2.jpg" alt="Player2" class="service-icon" data-service="player2" /><img src="<?= $webRoot; ?>/ui/images/core/icons/custom.jpg" alt="Custom" class="service-icon" data-service="custom" /></div></div><input type="hidden" id="service_input" name="service" value="<?= htmlspecialchars($editItem["service"] ?? "") ?>"><div id="service_signup_link" class="orm-note" style="font-size:12px; margin:-6px 0 8px 0; display:none;"><a id="signup_link" href="#" target="_blank" rel="noopener noreferrer" style="color:#ffb862; text-decoration:underline;">Sign up here</a> to get your API key for this service.
 </div><div id="service_tos_warning" class="orm-note" style="font-size:12px; margin:-4px 0 8px 0; display:none; color:#ffd2a6;">
 Please be warned that OpenAI, Anthropic and Google have started to enforce stricter terms of service regarding NSFW actitives. <a href="https://openrouter.ai/terms#_6_-prohibited-conduct_" target="_blank" rel="noopener noreferrer" style="color:#ffb862; text-decoration:underline;">More info here</a>.
 </div><div id="custom_note" class="orm-muted" style="font-size:12px; display:none; margin:-6px 0 8px 0;">
 Custom allows you to build your own connector setting using one of our API drivers to use non-supported services with DIALECTIC. Depending on the service you may not need to fill out all fields. For advanced users only
 </div><div id="url_row"><label for='url'>URL</label><br><div style="display:flex; gap:8px; align-items:center;"><input type="text" name="url" value="<?= htmlspecialchars($editItem["url"] ?? "") ?>" style="flex:1 1 auto;"><button type="button" id="btn_wsl_ip" class="btn-primary" title="Use WSL IP" style="display:none;" data-ip="<?= htmlspecialchars($WSL_IP) ?>">WSL IP</button><button type="button" id="btn_host_ip" class="btn-primary" title="Use HOST IP" style="display:none;" data-ip="<?= htmlspecialchars($HOST_IP) ?>">PC IP</button></div></div><div id="model_row"><label for='model'>Model</label><br><input type="text" name="model" value="<?= htmlspecialchars($editItem["model"] ?? "") ?>"><br></div><div id="provider_row"><label for='provider'>Provider</label><br><input type="text" name="provider" placeholder="(Optional) leave empty to use recommended provider" value="<?= htmlspecialchars($editItem["provider"] ?? "") ?>"><br></div><div id="driver_row" style="display:none;"><label for='driver'>Driver</label><br><input type="text" id="driver_input" name="driver" value="<?= htmlspecialchars($editItem["driver"] ?? "") ?>" style="display:none"><br><select id="driver_select"><option value="openrouterjson">OpenRouter JSON</option><option value="openaijson">OpenAI JSON</option><option value="google_openaijson">Google OpenAI JSON</option><option value="groqjson">Groq JSON</option><option value="player2json">Player2 JSON</option></select></div><div id="api_key_row"><?php
 ...
 echo "</select>";
 echo "<div id='api_key_notice' class='api-key-notice'></div>";
 ?></div><div id="reasoning_row"><label class="label-with-toggle"><span class='tip-label' data-tip='Fixes reasoning only models so they do not output <think> and <answer> tags. May be slower then regular models.'>Reasoning Model Fix</span><input type="hidden" name="reasoning_model" value="0"><input type="checkbox" name="reasoning_model" value="1" <?= isset($editItem["reasoning_model"]) && $editItem["reasoning_model"] == 1 ? "checked" : "" ?>></label></div><div id="json_toggles" style="margin-top:8px;"><label class="label-with-toggle"><span class='tip-label' data-tip='Force responses to be strict JSON. NonJSON output may be rejected or autoretried.'>Enforce JSON</span><input type="hidden" name="enforce_json" value="0"><input type="checkbox" name="enforce_json" value="1" <?= isset($editItem["enforce_json"]) && $editItem["enforce_json"] == 1 ? "checked" : "" ?>></label><div style="height:6px;"></div><label class="label-with-toggle"><span class='tip-label' data-tip='Guide/validate the JSON structure with a schema. Best used with Enforce JSON.'>JSON Schema</span><input type="hidden" name="json_schema" value="0"><input type="checkbox" name="json_schema" value="1" <?= isset($editItem["json_schema"]) && $editItem["json_schema"] == 1 ? "checked" : "" ?>></label><div style="height:6px;"></div><label class="label-with-toggle"><span class='tip-label' data-tip='Send a starter JSON object to steer field names/shape in the response.'>Prefill JSON</span><input type="hidden" name="prefill_json" value="0"><input type="checkbox" name="prefill_json" value="1" <?= isset($editItem["prefill_json"]) && $editItem["prefill_json"] == 1 ? "checked" : "" ?>></label></div><div id="remove_action_prompt_main" style="margin-top:12px;"><label class="label-with-toggle"><span class='tip-label' data-tip='Disable SSE streaming for this connector and wait for the full JSON reply before parsing. Useful for local LM Studio or other OpenAI-compatible servers that stream slowly or emit long reasoning chunks first.'>Disable Streaming</span><input type="hidden" name="disable_streaming" value="0"><input type="checkbox" name="disable_streaming" value="1" <?php 
 $metadataMain = [];
 if (isset($editItem["metadata"]) && !empty($editItem["metadata"])) {
...
(function(){
 ...
 modal.style.cssText = 'position:fixed; inset:0; display:none; align-items:center; justify-content:center; background:rgba(0,0,0,0.65); z-index:10000;';
 modal.innerHTML = `
 <div style="width:90%; max-width:1200px; height:80vh; background:#111; border:1px solid rgba(138,155,182,0.4); border-radius:10px; box-shadow:0 10px 30px rgba(0,0,0,0.6); position:relative; overflow:hidden;"><button id=\"llmtest_close\" class=\"btn-secondary\" style=\"position:absolute; top:8px; right:10px; z-index:3;\">Close</button><div id=\"llmtest_loading\" style=\"position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.4); z-index:2;\"><div style=\"width:48px; height:48px; border:4px solid rgba(255,255,255,0.25); border-top-color:#ffb862; border-radius:50%; animation: llmspin 1s linear infinite;\"></div></div><iframe id=\"llmtest_iframe\" src=\"about:blank\" style=\"width:100%; height:100%; border:0; background:#0e1624; position:relative; z-index:1;\"></iframe></div><style>@keyframes llmspin{to{transform:rotate(360deg)}}</style>`;
 document.body.appendChild(modal);
 function openModal(url){ const iframe = document.getElementById('llmtest_iframe'); const loader = document.getElementById('llmtest_loading'); if (loader) loader.style.display = 'flex'; iframe.onload = function(){ if (loader) loader.style.display = 'none'; }; iframe.src = url; modal.style.display = 'flex'; }
 function closeModal(){ modal.style.display = 'none'; try { document.getElementById('llmtest_iframe').src='about:blank'; } catch(_){} }
 document.addEventListener('click', function(e){ if (e.target && e.target.id==='llmtest_close') closeModal(); });
...
(function(){
...
</script><!-- list/grid moved to left pane --><?php
 ...
 ?><script src='https://cdnjs.cloudflare.com/ajax/libs/ace/1.23.4/ace.js'></script><script src='https://cdnjs.cloudflare.com/ajax/libs/js-yaml/4.1.0/js-yaml.min.js'></script><script>
(function(){
 if (!window.ace || !window.jsyaml) return;
 var containers = document.querySelectorAll('.extra_parameters_editor_container');
 var editors = [];
