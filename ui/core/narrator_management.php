...
function narratorManagementBuildNarratorActionStats($rows)
{
 ...
 if (is_array($rows)) {
 foreach ($rows as $row) {
 if (function_exists('dialecticActionCatalogToBool') && dialecticActionCatalogToBool($row['is_activated'] ?? false)) {
 $enabled++;
 }
 }
 }
...
if (
 ...
 $row = function_exists('dialecticGetActionCatalogRow') ? dialecticGetActionCatalogRow($codeName) : null;
 if (!is_array($row) || !dialecticActionCatalogToBool($row['available_to_narrator'] ?? false)) {
 ...
 $updatedRow = function_exists('dialecticGetActionCatalogRow') ? dialecticGetActionCatalogRow($codeName) : null;
 ...
 'code_name' => $codeName,
 'enabled' => is_array($updatedRow) ? dialecticActionCatalogToBool($updatedRow['is_activated'] ?? false) : $targetEnabled,
 'stats' => $stats,
 ]);
 exit;
 }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_narrator'])) {
 ...
 $escapedRoleplayName = $GLOBALS["db"]->escape($roleplayName);
 $matchingNpc = $GLOBALS["db"]->fetchOne(
...
$narrator->set('hide_from_context', isset($_POST['hide_from_context']) && $_POST['hide_from_context'] === '1' ? '1' : '0');
 ...
 // Only accept values from the server-owned preset catalog; leave the stored value alone if it is unavailable.
 if (isset($_POST['tts_filter_preset']) && dialecticVoiceFilterCatalogAvailable()) {
 $narrator->set('tts_filter_preset', dialecticVoiceFilterNormalizePreset($_POST['tts_filter_preset']));
 }
 if (isset($_POST['core'])) {
 ...
 if (isset($_POST['worldknowledge'])) {
 $narrator->set('worldknowledge', $_POST['worldknowledge']);
 }
 if (isset($_POST['prompt_head'])) {
 $narrator->set('prompt_head', $_POST['prompt_head']);
 }

 $narrationGeneralSettings = [
 'NARRATOR_TALKS' => isset($_POST['enabled']) && $_POST['enabled'] === '1',
 'NARRATOR_WELCOME' => isset($_POST['welcome_enabled']) && $_POST['welcome_enabled'] === '1',
 'HIDE_NARRATOR_DIALOGUE' => isset($_POST['hide_from_context']) && $_POST['hide_from_context'] === '1',
 'RANDOM_NARATION' => isset($_POST['random_enabled']) && $_POST['random_enabled'] === '1',
 'RANDOM_NARATION_CHANCE' => isset($_POST['random_chance']) ? max(1, min(100, intval($_POST['random_chance']))) : 15,
 'RANDOM_NARRATION_COOLDOWN' => isset($_POST['random_cooldown']) ? max(0, min(10, intval($_POST['random_cooldown']))) : 2,
 'INLINE_NARRATION_MODE' => $inlineNarrationMode,
 'REMOVE_ASTERISKS_FROM_PLAYER_INPUT' => isset($_POST['remove_asterisks_from_player_input']) && $_POST['remove_asterisks_from_player_input'] === '1',
 'REMOVE_ASTERISKS_FROM_NPC_OUTPUT' => isset($_POST['remove_asterisks_from_npc_output']) && $_POST['remove_asterisks_from_npc_output'] === '1',
 'REMOVE_PLAYER_AUTOCHAT_ASTERISKS' => isset($_POST['remove_player_autochat_asterisks']) && $_POST['remove_player_autochat_asterisks'] === '1',
 'PRESERVE_ASTERISKS_IN_CONTEXT' => isset($_POST['preserve_asterisks_in_context']) && $_POST['preserve_asterisks_in_context'] === '1',
 ];
 foreach ($narrationGeneralSettings as $settingId => $settingValue) {
 dialecticSetGeneralSetting($settingId, $settingValue, dialecticGetSchemaDescription($settingId));
 }
...
$narratorTtsFilterPreset = $narrator->get('tts_filter_preset') ?? '';
...
$worldknowledge = $narrator->get('worldknowledge') ?? 'knowall';
...
?><link rel="stylesheet" href="<?php echo $webRoot; ?>/ui/css/main.css"><?php echo dialecticVoiceFilterAssetTags($webRoot); ?><style>
 ...
 /* Checkbox group - keep for dynamic profile fields */
 .checkbox-group {
 display: flex;
 align-items: center;
 margin-bottom: 12px;
 ...
 border-color: rgba(255, 182, 65, 0.28);
 ...
 border-bottom: 2px solid rgba(255, 182, 65, 0.5);
 ...
 box-shadow: inset 0 0 10px rgba(255, 182, 65, 0.1);
 }
 ...
 color: rgb(255, 182, 65);
 border: 1px solid rgb(255, 182, 65);
 }
 ...
 padding: 20px;
 background: linear-gradient(180deg, rgba(26, 26, 26, 0.95), rgba(20, 20, 20, 0.98));
 border-bottom: 1px solid rgba(255, 182, 65, 0.3);
 ...
 padding: 15px 20px;
 background: linear-gradient(180deg, rgba(26, 26, 26, 0.95), rgba(20, 20, 20, 0.98));
 border-top: 1px solid rgba(255, 182, 65, 0.3);
 text-align: right;
...
</style><?php endif; ?><main><div class="page-container"><div id="toast" class="toast-notification <?php echo $saveSuccess ? '' : 'error'; ?>"><span class="message"><?php echo htmlspecialchars($saveMessage); ?></span></div><?php if ($saveSuccess || $saveMessage): ?><script>
 setTimeout(function(){ 
 try{ 
 ...
 Narrator Management
 </h1><p>Configure narrator behavior and settings</p></div><form method="post" action=""><button type="submit" class="btn-save" name="save_narrator" value="1">Save Narration Settings</button><div class="content-grid"><!-- Core Settings Section --><div class="content-section"><h2>Core Settings</h2><label for="roleplay_name">Narrator Name</label><input type="text" id="roleplay_name" name="roleplay_name" maxlength="64" value="<?php echo htmlspecialchars($roleplayName); ?>" placeholder="The Narrator"><span class="hint">Changes how the narrator is identified in prompts, LLM context, subtitles, and history displays. Internal routing, storage, actions, and TTS continue to use The Narrator.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="enabled" name="enabled" value="1" <?php echo $enabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Enable Narrator</span></label><span class="hint">Enable or disable the narrator system entirely.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="hide_from_context" name="hide_from_context" value="1" <?php echo $hideFromContext ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Hide Narrator from NPC Context</span></label><span class="hint">Hide Narrator-spoken dialogue lines from NPC context.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="diary_enabled" name="diary_enabled" value="1" <?php echo $diaryEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Narrator Diary</span></label><span class="hint">Allow The Narrator to write manual diary entries from narrator-targeted diary actions.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="auto_diary_enabled" name="auto_diary_enabled" value="1" <?php echo $autoDiaryEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Narrator Auto Diary</span></label><span class="hint">Allow The Narrator to join sleep and wait auto-diary generation. This does not affect manual narrator diary actions.</span></div><!-- Narration Section --><div class="content-section"><h2>Inline Narration</h2><label for="inline_narration_mode">Inline Narration Mode</label><select id="inline_narration_mode" name="inline_narration_mode"><option value="disabled" <?php echo $inlineNarrationMode === 'disabled' ? 'selected' : ''; ?>>Disabled</option><option value="narrator" <?php echo $inlineNarrationMode === 'narrator' ? 'selected' : ''; ?>>Narrator</option><option value="npc" <?php echo $inlineNarrationMode === 'npc' ? 'selected' : ''; ?>>NPC</option><option value="text_only" <?php echo $inlineNarrationMode === 'text_only' ? 'selected' : ''; ?>>Text Only</option></select><span class="hint">Controls leading *narration* blocks. Narrator uses The Narrator voice, NPC speaks the full line, Text Only displays the narration but speaks only the dialogue, and Disabled turns off special routing.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="remove_asterisks_from_player_input" name="remove_asterisks_from_player_input" value="1" <?php echo $removeAsterisksFromPlayerInput ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Remove Player Input Asterisks From TTS</span></label><span class="hint">Filters *asterisked* player actions/emphasis from player TTS only. The in-game player subtitle echo keeps the visible *text* either way. Turn this off if you want the player voice to speak asterisked text too.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="remove_asterisks_from_npc_output" name="remove_asterisks_from_npc_output" value="1" <?php echo $removeAsterisksFromNpcOutput ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Remove NPC Output Asterisks</span></label><span class="hint">Filters *asterisked* NPC narration/emotes from NPC speech and subtitles. Text Only always keeps leading narration visible. Turn this off if you want NPCs in other modes to keep or speak their own asterisked text.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="remove_player_autochat_asterisks" name="remove_player_autochat_asterisks" value="1" <?php echo $removePlayerAutochatAsterisks ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Remove Player Autochat Astreisk</span></label><span class="hint">Keeps AUTOCHAT and `**` player respeech spoken-only by stripping leading narration and *asterisked* narration from the rewritten player line. Turn this off if you want rewritten player text to keep one short leading *narration* block.</span><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="preserve_asterisks_in_context" name="preserve_asterisks_in_context" value="1" <?php echo $preserveAsterisksInContext ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Keep NPC Narration Description in Context History</span></label><span class="hint">Keep *narration* descriptions intact in the eventlog context history. NPCs will be able to see these descriptions in their prompts..</span></div><!-- Welcome Message Section --><div class="content-section"><h2>Welcome Message</h2><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="welcome_enabled" name="welcome_enabled" value="1" <?php echo $welcomeEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Enable Welcome Message on Load</span></label><span class="hint">The Narrator will give you a quick recap of what happened previously after you have loaded a save game.</span><label for="welcome_cooldown">Welcome Message Cooldown (minutes)</label><input type="number" id="welcome_cooldown" name="welcome_cooldown" value="<?php echo htmlspecialchars((string)$welcomeCooldown); ?>" min="1" max="1440"><span class="hint">Minimum time in minutes between welcome messages. Range: 1-1440 (24 hours), Default: 10 minutes</span></div><!-- Random Narration Section --><div class="content-section"><h2>Random Narration</h2><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="random_enabled" name="random_enabled" value="1" <?php echo $randomEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Enable Random Narration</span></label><span class="hint">Enable random Narrator interjections. The Narrator will occasionally add visual scene descriptions during conversations.</span><label for="random_chance">Random Narration Chance (%)</label><input type="number" id="random_chance" name="random_chance" value="<?php echo htmlspecialchars((string)$randomChance); ?>" min="1" max="100"><span class="hint">Probability (1-100) that the Narrator will interject with a scene description. Default: 15%</span><label for="random_cooldown">Random Narration Cooldown</label><input type="number" id="random_cooldown" name="random_cooldown" value="<?php echo htmlspecialchars((string)$randomCooldown); ?>" min="0" max="10"><span class="hint">Minimum number of conversation rounds between Narrator interjections. Prevents narration spam. Range: 0-10, Default: 2</span></div><div class="content-section"><h2>Bored Events</h2><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="bored_enabled" name="bored_enabled" value="1" <?php echo $boredEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Allow Narrator Bored Events</span></label><span class="hint">When enabled, bored events can sometimes route through The Narrator instead of the selected NPC.</span><label for="bored_chance">Narrator Bored Event Chance (%)</label><input type="number" id="bored_chance" name="bored_chance" value="<?php echo htmlspecialchars((string)$boredChance); ?>" min="1" max="100"><span class="hint">Probability (1-100) that an eligible bored event will be rerouted to The Narrator. Default: 25%</span></div><!-- Quest Comments Section --><div class="content-section"><h2>Quest Comments</h2><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="quest_comment_enabled" name="quest_comment_enabled" value="1" <?php echo $questCommentEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Enable Quest Comments</span></label><span class="hint">Narrator will comment on quest objective updates.</span><label for="quest_comment_chance">Quest Comment Chance (%)</label><input type="number" id="quest_comment_chance" name="quest_comment_chance" value="<?php echo htmlspecialchars((string)$questCommentChance); ?>" min="1" max="100"><span class="hint">Probability (1-100) that Narrator will comment on quest updates. Default: 10%</span><label for="quest_comment_cooldown">Quest Comment Cooldown (minutes)</label><input type="number" id="quest_comment_cooldown" name="quest_comment_cooldown" value="<?php echo htmlspecialchars((string)$questCommentCooldown); ?>" min="1" max="60"><span class="hint">Minimum time in minutes between quest comments. Prevents spam. Range: 1-60 minutes, Default: 3 minutes</span></div></div><!-- Profile & Voice Section --><div class="content-grid"><div class="content-section"><h2>Profile & Voice</h2><label for="profile_id">Profile</label><select id="profile_id" name="profile_id"><?php foreach ($allProfiles as $profile): ?><option value="<?php echo htmlspecialchars((string)$profile['id']); ?>" <?php echo ($profileId == $profile['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($profile['label'] ?? 'Profile ' . $profile['id']); ?></option><?php endforeach; ?></select><span class="hint">LLM connector profile for The Narrator.</span><label for="voiceid">Voice ID</label><input type="text" id="voiceid" name="voiceid" value="<?php echo htmlspecialchars($voiceid); ?>" placeholder="TheNarrator"><span class="hint">TTS voice identifier for The Narrator.</span><?php echo dialecticRenderVoiceFilterField([
 'id' => 'tts_filter_preset',
 'value' => $narratorTtsFilterPreset,
 'scope' => 'narrator',
 'profile_source' => '#profile_id',
 'voice_source' => '#voiceid',
 'hint' => 'Fixed audio filter applied to The Narrator voice. Use the play button to hear the profile, Voice ID, and filter currently selected above, including unsaved changes.',
]); ?><label for="worldknowledge">Knowledge Tags</label><input type="text" id="worldknowledge" name="worldknowledge" placeholder="Comma-separated knowledge tags (e.g., knowall, knowsome, knownone)" value="<?php echo htmlspecialchars($worldknowledge); ?>"><span class="hint">Comma-separated knowledge tags used by knowledge systems for lookup restrictions.</span></div><div class="content-section"><h2>Selected Profile Connectors</h2><?php
 // Helper function to get connector label
 $getConnectorLabel = function($id) use ($llmById) {
 return htmlspecialchars($llmById[$id] ?? '');
 };
 ?><div id="profile_llm_summary" style="display:grid; grid-template-columns: auto 1fr; gap:8px; color:#cfd9ea; font-size: 13px; line-height: 1.6;"><div style="color:rgb(255, 182, 65); font-weight:600;"> TTS:</div><div><?= htmlspecialchars($ttsById[$currentProfileData['tts_connector_id'] ?? null] ?? '') ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Standard:</div><div><?= $getConnectorLabel($currentProfileData['llm_primary_id'] ?? null) ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Fast:</div><div><?= $getConnectorLabel($currentProfileData['llm_secondary_id'] ?? null) ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Power:</div><div><?= $getConnectorLabel($currentProfileData['llm_tertiary_id'] ?? null) ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Experimental:</div><div><?= $getConnectorLabel($currentProfileData['llm_quaternary_id'] ?? null) ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Diary:</div><div><?= $getConnectorLabel($currentProfileData['diary_connector_id'] ?? null) ?></div><div style="color:rgb(255, 182, 65); font-weight:600;"> Formatter:</div><div><?= $getConnectorLabel($currentProfileData['llm_formatter_id'] ?? null) ?></div></div><span class="hint" style="margin-top: 8px;">These connectors are configured in the selected profile and will be used for The Narrator's AI responses.</span></div></div><script>
 (function(){
 ...
 [' Standard:', labelOf(pc ? pc.llm_primary_id : null)],
 [' Fast:', labelOf(pc ? pc.llm_secondary_id : null)],
 ...
 let html = '';
 rows.forEach(([k, v]) => {
 html += '<div style="color:rgb(255, 182, 65); font-weight:600;">' + k + '</div>';
 ...
 </script><!-- Prompt Head Override Section --><div class="content-section full-width-section"><h2>Prompt Head Override</h2><label for="prompt_head">Custom Prompt Head</label><textarea id="prompt_head" name="prompt_head" rows="5" placeholder="High-level system instructions injected before the core..."><?php echo htmlspecialchars($promptHead); ?></textarea><span class="hint">System preamble inserted before other sections. This overrides the profile and global prompt head when The Narrator is active. Leave empty to use profile/global defaults.</span></div><!-- Character Fields Section --><div class="content-section full-width-section"><h2>Character Description</h2><!-- Dynamic Profile Section (inline) --><div class="dynamic-profile-card"><h3> Dynamic Profile Updates</h3><label class="toggle-row"><div class="toggle-switch"><input type="checkbox" id="dynamic_profile" name="dynamic_profile" value="1" <?php echo $dynamicProfileEnabled ? 'checked' : ''; ?>><span class="toggle-slider"></span></div><span class="toggle-label">Enable Dynamic Profile</span></label><span class="hint">Allow systems to evolve the narrator profile based on gameplay events. Configured here and processed automatically by the server.</span><?php dps_render_controls($narrator->getAll(), true); ?><label class="field-selection-label">Fields to update</label><span class="hint">Select which fields should be dynamically updated:</span><div class="field-chips"><label class="field-chip"><input type="checkbox" name="dynamic_profile_fields[]" value="personality" <?php echo in_array('personality', $dynamicProfileFields) ? 'checked' : ''; ?>><span class="chip-text">Personality</span></label><label class="field-chip"><input type="checkbox" name="dynamic_profile_fields[]" value="speechstyle" <?php echo in_array('speechstyle', $dynamicProfileFields) ? 'checked' : ''; ?>><span class="chip-text">Speech Style</span></label><label class="field-chip"><input type="checkbox" name="dynamic_profile_fields[]" value="goals" <?php echo in_array('goals', $dynamicProfileFields) ? 'checked' : ''; ?>><span class="chip-text">Goals</span></label></div><span class="hint">Recommended: Select only 1-3 fields. Updates use DYNAMIC_PROMPT_* prompts from Global Settings.</span></div><label for="core">Core Summary</label><textarea id="core" name="core" rows="3" placeholder="Quick summary of The Narrator's persona..."><?php echo htmlspecialchars($core); ?></textarea><span class="hint">Brief summary of The Narrator's role and personality.</span><label for="background">Background</label><textarea id="background" name="background" rows="4" placeholder="Background description..."><?php echo htmlspecialchars($background); ?></textarea><span class="hint">Detailed background and history of The Narrator.</span><label for="personality">Personality</label><textarea id="personality" name="personality" rows="3" placeholder="Personality traits..."><?php echo htmlspecialchars($personality); ?></textarea><span class="hint">Behavioral traits and personality characteristics.</span><label for="speechstyle">Speech Style</label><textarea id="speechstyle" name="speechstyle" rows="2" placeholder="How The Narrator speaks..."><?php echo htmlspecialchars($speechstyle); ?></textarea><span class="hint">How The Narrator communicates and speaks.</span><label for="goals">Goals</label><textarea id="goals" name="goals" rows="3" placeholder="Current objectives..."><?php echo htmlspecialchars($goals); ?></textarea><span class="hint">Current goals and objectives for The Narrator.</span><details class="advanced-prompts-wrap"><summary class="advanced-prompts-summary"><span class="advanced-prompts-summary-text"><span class="advanced-prompts-summary-icon"></span><span>Narration Actions</span></span><span class="narrator-actions-summary-count" id="narratorActionsSummaryCount"><?php echo intval($narratorActionStats['enabled'] ?? 0); ?> enabled / <?php echo intval($narratorActionStats['total'] ?? 0); ?> total
 </span></summary><div class="advanced-prompts-panel"><?php if (!empty($narratorActionRows)): ?><div class="advanced-prompts-table-wrap"><table class="advanced-prompts-table narrator-actions-table"><thead><tr><th>Action</th><th>Description</th><th>Status</th><th>Toggle</th></tr></thead><tbody><?php foreach ($narratorActionRows as $actionRow): ?><?php
 ...
 $actionEnabled = dialecticActionCatalogToBool($actionRow['is_activated'] ?? false);
 $nextActionState = $actionEnabled ? '0' : '1';
 ?><tr id="narrator-action-row-<?php echo htmlspecialchars($actionCodeName, ENT_QUOTES, 'UTF-8'); ?>"><td><div class="narrator-actions-name"><strong><?php echo htmlspecialchars($actionName, ENT_QUOTES, 'UTF-8'); ?></strong><code><?php echo htmlspecialchars($actionCodeName, ENT_QUOTES, 'UTF-8'); ?></code></div></td><td class="narrator-actions-description"><?php echo htmlspecialchars($actionDescription, ENT_QUOTES, 'UTF-8'); ?></td><td class="narrator-actions-state"><?php if ($actionEnabled): ?><span class="advanced-status-badge custom">Enabled</span><?php else: ?><span class="advanced-status-badge default">Disabled</span><?php endif; ?></td><td class="narrator-actions-toggle-cell"><button
 type="button"
 ...
 ><?php echo $actionEnabled ? 'Disable' : 'Enable'; ?></button></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><div class="narrator-actions-empty">No narrator-scoped actions were found.</div><?php endif; ?></div></details><details class="advanced-prompts-wrap"><summary class="advanced-prompts-summary"><span class="advanced-prompts-summary-text"><span class="advanced-prompts-summary-icon"></span><span>Advanced Prompts (Prompts Manager)</span></span></summary><div class="advanced-prompts-panel"><?php if (!empty($advancedPrompts)): ?><div class="advanced-prompts-table-wrap"><table class="advanced-prompts-table"><thead><tr><th>Prompt Key</th><th>Description</th><th>Status</th><th>Preview</th><th>Actions</th></tr></thead><tbody><?php foreach ($advancedPrompts as $promptRow):
 ...
 ?><tr id="advanced-prompt-row-<?php echo htmlspecialchars($promptRow['prompt_key']); ?>"><td class="advanced-prompt-key-cell"><code><?php echo $promptKey; ?></code></td><td class="advanced-prompt-description-cell"><?php echo $description; ?></td><td><?php if ($isCustomPrompt): ?><span class="advanced-status-badge custom">Custom</span><?php else: ?><span class="advanced-status-badge default">Default</span><?php endif; ?></td><td class="advanced-prompt-content-cell"><div class="advanced-prompt-preview <?php echo $isCustomPrompt ? 'custom' : ''; ?>"><?php echo htmlspecialchars($preview); ?></div></td><td class="advanced-prompts-actions-cell"><button type="button" class="advanced-prompts-btn advanced-prompts-btn-edit" onclick="openNarratorPromptModal('<?php echo $promptKey; ?>')">Edit</button><?php if ($isCustomPrompt): ?><button type="button" class="advanced-prompts-btn advanced-prompts-btn-clear" onclick="clearNarratorPrompt('<?php echo $promptKey; ?>')">Clear</button><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><div class="advanced-prompts-empty">No advanced narrator prompts were found.</div><?php endif; ?></div></details></div><div style="display:flex; justify-content:flex-start; margin-top:20px;"><button type="submit" class="btn-save" name="save_narrator" value="1">Save Narration Settings</button></div></form><div id="narratorAdvancedPromptModal" class="advanced-prompts-modal"><div class="advanced-prompts-modal-content"><div class="advanced-prompts-modal-header"><span class="advanced-prompts-modal-close" onclick="closeNarratorPromptModal()">&times;</span><h3>Edit Prompt: <span id="narratorAdvancedPromptModalKey"></span></h3></div><div class="advanced-prompts-modal-body"><div class="advanced-prompts-modal-group"><label>Description</label><p id="narratorAdvancedPromptModalDescription" style="color:#b0b0b0; margin:0;"></p></div><div class="advanced-prompts-modal-group"><label>Default Prompt (Read-Only)</label><div id="narratorAdvancedPromptModalDefault" class="advanced-prompts-readonly"></div></div><div class="advanced-prompts-modal-group"><label>Custom Prompt (Optional - Leave empty to use default)</label><textarea id="narratorAdvancedPromptModalCustom" class="advanced-prompts-textarea" placeholder="Enter your custom prompt here, or leave empty to use the default prompt..."></textarea></div></div><div class="advanced-prompts-modal-footer"><button type="button" class="advanced-prompts-modal-btn advanced-prompts-modal-btn-cancel" onclick="closeNarratorPromptModal()">Cancel</button><button type="button" class="advanced-prompts-modal-btn advanced-prompts-modal-btn-save" onclick="saveNarratorPrompt()">Save Custom Prompt</button></div></div></div></div></main><script>
...
const narratorActions = <?php
 ...
 'enabled' => dialecticActionCatalogToBool($actionRow['is_activated'] ?? false),
 ];
 }
