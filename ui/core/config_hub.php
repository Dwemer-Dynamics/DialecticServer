...
function dialecticConfigHubActiveClass($tabId)
{
    global $initialTab;
    return $initialTab === $tabId ? ' active' : '';
}

function dialecticConfigHubIframeAttrs($tabId, $src, $loading = 'lazy')
{
    global $initialTab;
    $escapedSrc = htmlspecialchars($src, ENT_QUOTES, 'UTF-8');
    if ($initialTab === $tabId) {
        return 'loading="eager" src="' . $escapedSrc . '"';
    }

    return 'loading="' . htmlspecialchars($loading, ENT_QUOTES, 'UTF-8') . '" src="about:blank" data-src="' . $escapedSrc . '"';
}
?>
...
.tab-button.active { background: linear-gradient(180deg, rgba(42, 42, 42, 0.95), rgba(34, 34, 34, 0.98)); border-color: rgba(255, 182, 65, 0.5); border-bottom: 2px solid rgba(42, 42, 42, 0.95); color: rgb(255, 182, 65); box-shadow: 0 4px 8px rgba(255, 182, 65, 0.2); }
...
<main class="d-flex flex-column">
    ...
    <div class="content-area flex-grow-1 d-flex overflow-hidden" style="min-height: 0;">
        <div id="npc" class="tab-content<?php echo dialecticConfigHubActiveClass('npc'); ?>">
            <div class="embed-wrap">
                <iframe class="embed" <?php echo dialecticConfigHubIframeAttrs('npc', $webRoot . '/ui/core/npc_master.php?embed=1'); ?>></iframe>
            </div>
        </div>
        ...
        <div id="worldknowledge" class="tab-content">
            <div class="embed-wrap">
                <iframe class="embed" loading="lazy" src="about:blank" data-src="<?php echo $webRoot; ?>/ui/worldknowledge_upload.php?embed=1"></iframe>
            </div>
        </div>
        <div id="npcbio" class="tab-content<?php echo dialecticConfigHubActiveClass('npcbio'); ?>">
            <div class="embed-wrap">
                <iframe class="embed" <?php echo dialecticConfigHubIframeAttrs('npcbio', $webRoot . '/ui/npc_upload.php?embed=1'); ?>></iframe>
            </div>
        </div>
