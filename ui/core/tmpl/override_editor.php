...
function ovr_icon_for(string $key): string {
 ...
 if (strpos($u, 'WORLDKNOWLEDGE') === 0) return '';
 if (strpos($u, 'QUEST_') === 0) return '';
 if (strpos($u, 'LANG_') === 0) return '';
 if ($u === 'DIALECTIC_ANIMATIONS') return '';
...
.btn-add-<?= $prefix ?>ovr {
 ...
 border-radius: 6px;
 border: 1px solid <?= $isProfileMode ? 'rgb(255, 182, 65)' : '#4a4a4a' ?>;
...
.<?= $prefix ?>ovr-setting-option:hover {
 ...
 <?php if ($isProfileMode): ?>border-color: rgb(255, 182, 65);<?php endif; ?>
}
...
<?php endif; ?></style><div class="<?= $prefix ?>ovr-container"><?php if ($isNpcMode): ?><div class="<?= $prefix ?>ovr-header"><div class="<?= $prefix ?>ovr-title">Current Overrides</div></div><?php endif; ?><div id="<?= $prefix ?>ovr-list" class="<?= $prefix ?>ovr-list"></div><button type="button" id="btn-add-<?= $prefix ?>ovr" class="btn-add-<?= $prefix ?>ovr">
 + Add <?= $isProfileMode ? 'Global Setting ' : '' ?>Override
 </button><?php if ($isNpcMode): ?><div class="advanced-json-toggle" id="advanced-json-toggle">
 Advanced: Raw JSON Editor (click to expand)
 </div><div class="advanced-json-content" id="advanced-json-content"><div id="extended_data_raw_editor" style="min-height: 300px;"></div></div><?php endif; ?></div><!-- Modal --><div id="<?= $prefix ?>ovr-modal" class="<?= $prefix ?>ovr-modal-backdrop"><div class="<?= $prefix ?>ovr-modal"><div class="<?= $prefix ?>ovr-modal-header"><div class="<?= $prefix ?>ovr-modal-title" id="<?= $prefix ?>ovr-modal-title">Add Override</div><button type="button" class="btn-<?= $prefix ?>ovr" id="<?= $prefix ?>ovr-modal-close">Close</button></div><div class="<?= $prefix ?>ovr-modal-body"><div id="<?= $prefix ?>ovr-select-stage"><?php if ($isNpcMode): ?><input type="text" id="<?= $prefix ?>ovr-search" class="<?= $prefix ?>ovr-search" placeholder="Filter settings..." style="margin-bottom:12px;"><?php endif; ?><div style="color:#9fb1c9; font-size:13px; margin-bottom:8px;">
 Select a <?= $isProfileMode ? 'global ' : '' ?>setting to override:
 </div><div id="<?= $prefix ?>ovr-settings-list" class="<?= $prefix ?>ovr-settings-list"></div></div><div id="<?= $prefix ?>ovr-value-stage" style="display: none;"><div id="<?= $prefix ?>ovr-value-form"></div></div></div><div class="<?= $prefix ?>ovr-modal-footer"><button type="button" class="btn-<?= $prefix ?>ovr" id="<?= $prefix ?>ovr-modal-back" style="display:none;">Back</button><button type="button" class="btn-<?= $prefix ?>ovr" id="<?= $prefix ?>ovr-modal-cancel">Cancel</button><button type="button" class="btn-add-<?= $prefix ?>ovr" id="<?= $prefix ?>ovr-modal-save" style="width:auto; display:none;">Save Override</button></div></div></div><script>
(function(){
 ...
 if (u.startsWith('WORLDKNOWLEDGE')) return '';
 if (u.startsWith('QUEST_')) return '';
 if (u.includes('LANG_')) return '';
 if (u === 'DIALECTIC_ANIMATIONS') return '';
 ...
 'WORLDKNOWLEDGE_AMOUNT': 'WORLDKNOWLEDGE ARTICLES AMOUNT',
 ...
 : String(value);
 return `
 <div class="${PREFIX}ovr-item" data-key="${escapeHtml(key)}"><div class="${PREFIX}ovr-icon">${getIcon(key)}</div><div class="${PREFIX}ovr-info"><div class="${PREFIX}ovr-key">${escapeHtml(getLabel(key))}</div><div class="${PREFIX}ovr-value">${escapeHtml(displayValue)}</div></div><div class="${PREFIX}ovr-actions"><button type="button" class="btn-${PREFIX}ovr btn-edit-${PREFIX}ovr" data-key="${escapeHtml(key)}">Edit</button><button type="button" class="btn-${PREFIX}ovr danger btn-remove-${PREFIX}ovr" data-key="${escapeHtml(key)}"></button></div></div>
 `;
 }).join('');
 ...
 hasResults = true;
 html += `<div style="margin-top:${html?'16px':'0'}; margin-bottom:8px; color:rgb(255, 182, 65); font-weight:700; font-size:14px;">${category}</div>`;
 
 filteredKeys.forEach(key => {
 const schema = SCHEMA[key];
 const desc = (schema.description || '').substring(0, 120);
 html += `
 <div class="${PREFIX}ovr-setting-option" data-key="${escapeHtml(key)}"><div class="${PREFIX}ovr-setting-name">${getIcon(key)} ${escapeHtml(getLabel(key))}</div><div class="${PREFIX}ovr-setting-desc">${escapeHtml(desc)}${desc.length >= 120 ? '...' : ''}</div></div>
 `;
 });
 }
 ...
 inputHtml = `
 <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" id="${PREFIX}ovr-value-input" ${checked} style="transform: scale(1.5);"><span style="color: #e9efff;">Enabled</span></label>
 `;
 } else if (type === 'select' && values.length > 0) {
 inputHtml = `
 <select id="${PREFIX}ovr-value-input" class="${PREFIX}ovr-input" required><option value="">-- Select --</option>
 ...
 form.innerHTML = `
 <div class="${PREFIX}ovr-input-group"><label class="${PREFIX}ovr-input-label">${getIcon(key)} ${escapeHtml(getLabel(key))}</label><div style="color:#9fb1c9; font-size:12px; margin-bottom:8px;">${escapeHtml(desc)}</div>
 ${inputHtml}
 ${validationHint}
 </div>
 `;
