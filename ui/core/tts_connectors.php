...
.meta-group h3 { font-family: 'Gothic821', serif; word-spacing: 6px; font-size: 1.2em; font-weight: normal; }
...
@media (max-width: 980px) {
 ...
 ?><div class="conn-card<?php echo $rowActive; ?>" data-edit-id="<?php echo h($rowId); ?>"><div class="conn-head"><div class="conn-name"><?php echo h($row['label'] ?? ('Connector #' . $rowId)); ?></div><div class="conn-badge"><?php echo h($ttsConnector->getDisplayName($rowDriver)); ?></div></div><div class="conn-sub"><?php echo h($row['url'] ?? ''); ?></div><div class="conn-usage"><?php echo h($rowUseCount); ?> assignment<?php echo $rowUseCount === 1 ? '' : 's'; ?></div></div><?php endforeach; ?></div></div><div class="right-col"><?php if (!$editItem): ?><div class="placeholder">
 Select a connector from the left to edit it. New installs will already have the currently selected TTS provider migrated into this table.
 </div><?php else: ?><form method="post" action="<?php echo h(ttsPageUrl()); ?>" id="tts_connector_form"><input type="hidden" name="id" value="<?php echo h($editItem['id']); ?>"><div class="btn-row"><button type="submit" class="btn-save" name="save_connector" value="1">Save</button><button type="button" class="btn-primary" id="btn_test_connector">Test</button><button type="submit" class="btn-primary" formmethod="get" formaction="<?php echo h(ttsPageUrl()); ?>" name="export" value="<?php echo h($editItem['id']); ?>">Export</button><a class="btn-primary" href="<?php echo h(ttsPageUrl(['clone' => $editItem['id']])); ?>">Clone</a><a class="btn-danger" href="<?php echo h(ttsPageUrl(['delete' => $editItem['id']])); ?>" onclick="return confirm('Delete this TTS connector?');">Delete</a></div><div class="orm-note">Please save any changes before testing to ensure the latest settings are used.</div><div class="editor-grid"><div class="field-block"><label for="label">Name</label><input type="text" id="label" name="label" value="<?php echo h($editItem['label'] ?? ''); ?>"><div class="field-help">This label appears in profile and player connector pickers.</div></div><div class="field-block"><label for="driver">Service</label><select id="driver" name="driver"><?php foreach ($groupedDriverOptions as $groupLabel => $groupDrivers): ?><?php if (empty($groupDrivers)) { continue; } ?><optgroup label="<?php echo h($groupLabel); ?>"><?php foreach ($groupDrivers as $driverValue): ?><option value="<?php echo h($driverValue); ?>" <?php echo $currentDriver === $driverValue ? 'selected' : ''; ?>><?php echo h($ttsConnector->getDisplayName($driverValue)); ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select><div class="field-help">The provider driver this connector loads at runtime.</div></div><?php $showUrlField = ttsShouldShowUrlField($ttsConnector, $currentDriver); ?><div class="field-block" id="url_block" style="<?php echo $showUrlField ? '' : 'display:none;'; ?>"><label for="url">URL</label><input type="url" id="url" name="url" value="<?php echo h($editItem['url'] ?? ''); ?>"><div class="field-help">Used for providers that expose a local or remote HTTP endpoint.</div></div><div class="field-block" id="api_badge_block" style="<?php echo $ttsConnector->driverUsesApiBadge($currentDriver) ? '' : 'display:none;'; ?>"><label for="api_badge_id">API Badge</label><?php
 ...
 ?><option value="<?php echo h($apiRowId); ?>" data-empty="1" <?php echo $selected; ?>><?php echo h($labelText); ?></option><?php endforeach; ?><?php endif; ?></select><div id="api_key_notice" class="api-key-notice"></div><div class="field-help">Cloud TTS providers require an API key.</div></div></div><?php $sharedMetadataSchema = $ttsConnector->getConnectorMetadataFieldSchema(); ?><div class="meta-group active"><h3 style="margin:0 0 14px;color:#f3d6a8;">NPC Fallbacks</h3><div class="inline-two"><?php foreach ($sharedMetadataSchema as $fieldName => $definition): ?><?php
 $fieldType = $definition['type'] ?? 'string';
 $fieldValue = $currentMetadata[$fieldName] ?? '';
 $fieldKey = 'meta__shared__' . $fieldName;
 ?><div class="field-block"><label for="<?php echo h($fieldKey); ?>"><?php echo h(ttsFormatFieldLabel($fieldName)); ?></label><?php if ($fieldType === 'boolean'): ?><input type="hidden" name="<?php echo h($fieldKey); ?>" value="false"><select id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>"><option value="true" <?php echo $fieldValue ? 'selected' : ''; ?>>Enabled</option><option value="false" <?php echo !$fieldValue ? 'selected' : ''; ?>>Disabled</option></select><?php elseif ($fieldType === 'integer'): ?><input type="number" step="1" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php elseif ($fieldType === 'number'): ?><input type="number" step="0.01" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php elseif ($fieldType === 'longstring'): ?><textarea id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>"><?php echo h($fieldValue); ?></textarea><?php else: ?><input type="text" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>"><?php endif; ?><?php if (!empty($definition['description'])): ?><div class="field-help"><?php echo $definition['description']; ?></div><?php endif; ?></div><?php endforeach; ?></div></div><?php foreach ($driverOptions as $driverOption): ?><?php
 ...
 ?><div class="meta-group<?php echo $groupDriver === $currentDriver ? ' active' : ''; ?>" data-driver-fields="<?php echo h($groupDriver); ?>"><h3 style="margin:0 0 14px;color:#f3d6a8;"><?php echo h($ttsConnector->getProviderTitle($groupDriver)); ?> Settings</h3><?php if (empty($visibleFieldNames)): ?><div class="settings-empty-note">This TTS provider does not have any connector-level settings to configure here.</div><?php else: ?><div class="inline-two"><?php foreach ($groupSchema as $fieldName => $definition): ?><?php if (!ttsShouldRenderField($fieldName, $definition, $ttsConnector, $groupDriver)) { continue; } ?><?php
 $fieldType = $definition['type'] ?? 'string';
 ...
 <option value="<?php echo h($valueOption); ?>" <?php echo strval($fieldValue) === strval($valueOption) ? 'selected' : ''; ?>><?php echo h($valueOption); ?></option>
 <?php endforeach; ?>
 </select>
 ...
 <?php foreach (($definition['values'] ?? []) as $valueOption): ?>
 <option value="<?php echo h($valueOption); ?>" <?php echo in_array($valueOption, $selectedValues, true) ? 'selected' : ''; ?>><?php echo h($valueOption); ?></option>
 <?php endforeach; ?>
 </select>
 <?php else: ?>
 <input type="text" id="<?php echo h($fieldKey); ?>" name="<?php echo h($fieldKey); ?>" value="<?php echo h($fieldValue); ?>">
 ...
 <?php endforeach; ?></div><?php endif; ?></div><?php endforeach; ?></form><?php endif; ?></div></div></div><div id="tts_test_modal"><div class="inner"><button type="button" class="btn-secondary" id="tts_test_close">Close</button><iframe id="tts_test_iframe" src="about:blank"></iframe></div></div></main><script>
(function(){
 ...
 function openTtsTest(connectorId) {
 if (!connectorId || !modal || !iframe) {
 return;
 }
