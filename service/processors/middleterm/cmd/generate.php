...
foreach (array_reverse($contextDataFull) as $entry) {
    if ($entry["content"]) {
        $history .= "===\nMemory entry, date " . convert_gamets2fallout_date($entry["gamets_truncated"]).PHP_EOL;
        $history .= trim($entry["content"]) . PHP_EOL.PHP_EOL;
        $lastgamets=$entry["gamets_truncated"];
    }
}

/*$head[] = ['role' => 'system', 'content' => "You're an AI assistant. Examine this memory logbook from a story in the Fallout universe."];
...
$request = "Main character in this logbook is {$GLOBALS["DIALECTIC_NAME"]}.
...
// Replace {DIALECTIC_NAME} placeholder with actual character name
$request = str_replace('{DIALECTIC_NAME}', $GLOBALS['DIALECTIC_NAME'], $requestPrompt);
