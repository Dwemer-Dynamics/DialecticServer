...
class openrouterjson
{
    ...
    private $_lastStreamErrorCode;
    private $_lastStreamErrorType;
    ...
    public function __construct()
    {
        ...
        $this->_lastStreamErrorCode=null;
        $this->_lastStreamErrorType="";
        $this->_lastStreamErrorMessage="";
        require_once(__DIR__."/__jpd.php");
    }

    private function captureStreamError($line) {
        $this->_lastStreamErrorCode=null;
        $this->_lastStreamErrorType="";
        $this->_lastStreamErrorMessage="";

        $payload = trim((string)$line);
        ...
        $error = $decoded["error"];
        if (isset($error["code"])) {
            $this->_lastStreamErrorCode = intval($error["code"]);
        }
        if (isset($error["message"])) {
            $this->_lastStreamErrorMessage = (string)$error["message"];
        }
        if (isset($error["metadata"]) && is_array($error["metadata"]) && isset($error["metadata"]["error_type"])) {
            $this->_lastStreamErrorType = (string)$error["metadata"]["error_type"];
        }
    }

    public function getLastStreamErrorCode() {
        return $this->_lastStreamErrorCode;
    }

    public function getLastStreamErrorType() {
        return $this->_lastStreamErrorType;
    }

    public function getLastStreamErrorMessage() {
        return $this->_lastStreamErrorMessage;
    }

    private function extractMangledMessageFromCurrentBuffer() {
        ...
        $message = (string)$finalData["message"];
        if (trim($message) === "") {
            return "";
        }
        ...
        $mangledBuffer = str_replace($this->_extractedbuffer, "", $message);
        $this->_extractedbuffer=$message;
    ...
    private function isWebSearchInMessage($s_msg="") {
        $b_res = false;
        if (strlen($s_msg) > 7) {
            $i_pos = stripos($s_msg, "Fallout search");
            if ($i_pos === false) 
                $i_pos = stripos($s_msg, "Search Fallout");
            if ($i_pos === false) 
                $i_pos = stripos($s_msg, "Find knowledge in Fallout");
            $b_res = (!($i_pos === false));
        }
        return $b_res;
    }
    ...
    public function open($contextData, $customParms)
    {
        ...
        ****/
        ...
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('OPENROUTERJSON');
        }
        ...
        if (isset($GLOBALS["DIALECTIC_SPEECHSTYLE"]) && (!empty($GLOBALS["DIALECTIC_SPEECHSTYLE"]))) {
            ...
            if ($n>=($n_ctxsize-1) && $element["role"]!="tool") {
                ...
                if ($element["role"]=="system") {
                    ...
                    if (isset($element["tool_calls"])) {
                        $pb["system"].="{$GLOBALS["DIALECTIC_NAME"]} issued ACTION {$element["tool_calls"][0]["function"]["name"]}";
                        $lastAction="{$GLOBALS["DIALECTIC_NAME"]} issued ACTION {$element["tool_calls"][0]["function"]["name"]} {$element["tool_calls"][0]["function"]["arguments"]}";
                        ...
                        $actionTargetValue = dialecticExtractActionArgumentTargetValue($localArguments);
                        if (isset($GLOBALS["F_RETURNMESSAGES"][$localFuncCodeName])) {
                            $lastAction=dialecticFormatReturnMessageTemplate($localFuncCodeName, $localArguments);
                        }
                        $contextDataCopy[]=[
                                "role"=>"assistant",
                                "content"=>"{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\", \"listener\": \"{$dialogueTarget["target"]}\", \"mood\": \"\",\"action\": \"$lastActionName\",\"target\": \"".$actionTargetValue."\", \"message\": \"\"}"
                            ];
                            
                        $gameRequestCopy=$GLOBALS["gameRequest"];    
                        $gameRequestCopy[3]="{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\", \"listener\": \"{$dialogueTarget["target"]}\", \"mood\": \"\",\"action\": \"$lastActionName\", \"target\": \"".$actionTargetValue."\", \"message\": \"\"}";
                        ...
                        if (is_array($alreadyJs)) {
                            ...
                            if (true) {
                                ...
                                $contextData[$n]=[
                                    "role"=>"assistant",
                                    "content"=>"{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\", \"listener\": \"{$dialogueTarget["target"]}\", \"mood\": \"\", \"action\": \"Talk\",\"target\": \"\", \"message\":\"".trim($dialogueTarget["cleanedString"])."\"}"
        ...
        if ((isset($GLOBALS["CONNECTOR"][$this->name]["PREFILL_JSON"])) && ($GLOBALS["CONNECTOR"][$this->name]["PREFILL_JSON"])) {
            $GLOBALS["PATCH"]["PREAPPEND"]="{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\",";
        ...
        if (!$assistantAppearedInhistory) { // is this still needed?
            ...
            if (isset($GLOBALS["DIALECTIC_NO_EXAMPLES"]) && $GLOBALS["DIALECTIC_NO_EXAMPLES"]) {
                $contextExamples=[];
            } else {
                // EXAMPLES
                $contextExamples[]= [
                ...
                $contextExamples[]= [
                    "role"=>"assistant",
                    "content"=>"{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\",\"listener\": \"{$GLOBALS["PLAYER_NAME"]}\", \"mood\": \"default\", \"action\": \"Talk\",\"target\": \"\", \"message\": \"What are you looking at?\"}"
        ...
        foreach (dialecticGetEnabledConnectorExtraParameters($GLOBALS["CONNECTOR"][$this->name] ?? []) as $k => $v) {
                $data[$k]=$v;
        }
        ...
        if ($this->_websearch) { // online search request 
            ...
            if (stripos($search_text, "Fallout") === false) 
                $s_prefix = "Fallout lore ";
            else
                $s_prefix = "";
            ...
            $data["messages"] = [
                ['role' => 'system', 
                 'content' => "" // "Role-play in Fallout universe. "
                 ."You are an expert with extensive knowledge about Fallout lore focusing on puzzle solutions, quests, places and people." 
            ...
            $data["plugins"] = [
                ['id' => 'web', 
                 'search_prompt' => "Search the web to find relevant information related to Fallout universe. "
    ...
    public function process()
    {
        ...
        if (strpos($line, '"error"') !== false) {
            $this->captureStreamError($line);
            Logger::error("Error response from LLM: $line");
            $salvagedBuffer = $this->extractMangledMessageFromCurrentBuffer();
            if (trim($salvagedBuffer) !== "") {
                Logger::warn("openrouterjson: stream error after usable partial JSON; closing stream and returning salvaged speech.");
                $this->_forcedClose=true;
                return $salvagedBuffer;
            }
            return -1;
        }
    ...
    public function processActions()
    {
        ...
        if ($this->_functionName) {
            ...
            if (is_array($parameterArr)) {
                ...
                $commandStr = dialecticEncodeActionLine($GLOBALS["DIALECTIC_NAME"], $functionCodeName, $parameterArr, $parameter) . "\n";
    ...
    public function fast_request($contextData, $customParms,$callName='')
    {
        ...
        require_once(__DIR__.DIRECTORY_SEPARATOR."..".DIRECTORY_SEPARATOR."functions".DIRECTORY_SEPARATOR."json_response.php");
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('OPENROUTERJSON_FAST');
        }
