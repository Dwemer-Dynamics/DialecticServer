...
class openaijson
{
    ...
    private function init_connector($customParms) {
        ...
        $extraParameters = dialecticGetEnabledConnectorExtraParameters($GLOBALS["CONNECTOR"][$this->name] ?? []);
    ...
    public function open($contextData, $customParms)
    {
        ...
        ****/
        ...
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('OPENAIJSON');
        }
        ...
        $b_speech_style = (
            (isset($GLOBALS["DIALECTIC_SPEECHSTYLE"]) && (!empty($GLOBALS["DIALECTIC_SPEECHSTYLE"]))) || 
            (stripos($GLOBALS["DIALECTIC_PERS"],"#SpeechStyle")!==false)
        );
        if ($b_speech_style) {
            ...
            if (($n >= $n_last_context) && ($element["role"] != "tool")) {
                ...
                if ($lastrole=="assistant" && $lastrole!=$element["role"] && $element["role"]!="tool" && !empty($assistantRoleBuffer)) {
                    $contextDataCopy[]=[
                        ...
                        // no json version:
                        //"content" => "{$GLOBALS["DIALECTIC_NAME"]}: ".trim($assistantRoleBuffer)
                    ];
                    $lastTargetBuffer="";
                    $assistantRoleBuffer="";
    ...
    public function fast_request($contextData, $customParms,$callName='')
    {
        ...
        require_once(__DIR__.DIRECTORY_SEPARATOR."..".DIRECTORY_SEPARATOR."functions".DIRECTORY_SEPARATOR."json_response.php");
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('OPENAIJSON_FAST');
        }
