...
foreach ($normalizedContext as $n=>$s_msg) {
    if ($n==(sizeof($normalizedContext)-1)) {   // Last prompt line
        ...
        $narratorContextLabel = preg_quote(dialecticGetNarratorRoleplayName(), '/');
...
$context.=strtr($GLOBALS["DIALECTIC_PERS"],["#PLAYER_NAME#"=>$GLOBALS["PLAYER_NAME"]]);
        ...
        if ($s_msg["role"]=="user") {
            ...
            $GLOBALS["DEBUG_DATA"][]=$s_msg["content"];
             ...
             if (isset($s_msg["tool_calls"])) {
                    $pb["system"].="{$GLOBALS["DIALECTIC_NAME"]} issued ACTION {$s_msg["tool_calls"][0]["function"]["name"]}";
                    $lastAction="{$GLOBALS["DIALECTIC_NAME"]} issued ACTION {$s_msg["tool_calls"][0]["function"]["name"]} {$s_msg["tool_calls"][0]["function"]["arguments"]}";
                    ...
                    /*$contextHistory.="### Response:{\"character\": \"{$GLOBALS["DIALECTIC_NAME"]}\", \"listener\": \"{$dialogueTarget["target"]}\", \"mood\": \"\", \"action\": \"\", 
            ...
            $contextHistory.=dialecticBuildNarratorContextLine(strtr($lastAction,["#RESULT#"=>$s_msg["content"]]));
