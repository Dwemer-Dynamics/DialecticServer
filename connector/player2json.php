...
class player2json
{
    ...
    private function init_connector() {
        ...
        $this->_game_key = trim((string)($GLOBALS["CONNECTOR"][$this->name]["player2_game_key"] ?? $GLOBALS["CONNECTOR"][$this->name]["API_KEY"] ?? "DIALECTIC"));
        if ($this->_game_key === "") {
            $this->_game_key = "DIALECTIC";
        }
    ...
    public function open($contextData, $customParms)
    {
        ...
        ****/
        ...
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('PLAYER2JSON');
        }
        ...
        if (strpos($GLOBALS["DIALECTIC_PERS"],"#SpeechStyle")!==false) {
            ...
            if ($n>=(sizeof($contextDataOrig)-1) && $element["role"]!="tool") {
                ...
                if ($element["role"]=="system") {
                    ...
                    if (isset($element["tool_calls"])) {
                        ...
                        $lastAction="{$GLOBALS["DIALECTIC_NAME"]} issued ACTION {$element["tool_calls"][0]["function"]["name"]} {$element["tool_calls"][0]["function"]["arguments"]}, #RESULT#";
    ...
    public function fast_request($contextData, $customParms,$callName='')
    {
        ...
        require_once(__DIR__.DIRECTORY_SEPARATOR."..".DIRECTORY_SEPARATOR."functions".DIRECTORY_SEPARATOR."json_response.php");
        if (function_exists('dialecticEnsureNarratorJsonResponseState')) {
            dialecticEnsureNarratorJsonResponseState('PLAYER2JSON_FAST');
        }
