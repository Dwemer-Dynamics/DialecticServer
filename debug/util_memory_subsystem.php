...
function dialecticResolveSummaryConnectorRuntime(): ?array
{
...
function resyncMemorySummaries($db, $forceAll = false, $onlyFix = false)
{
    ...
    $db->execQuery("UPDATE memory_summary SET companions=NULL WHERE btrim(COALESCE(companions, ''), '|, ') = ''");
    ...
    foreach ($missingCompanions as $row) {
        $peopleRows = $db->fetchAll("SELECT people FROM eventlog WHERE gamets > {$row["gamets_truncated"]} - $pfi AND gamets <= {$row["gamets_truncated"]} + $pfi");
        $npcs       = [];
        ...
        foreach ($npcs as $name => $occurrences) {
            ...
            if ($cleanName === '' || strtolower($cleanName) === 'unknown' || strtolower($cleanName) === 'the narrator' || $cleanName === '--' || $cleanName === '-') {
                continue;
            }
            if (stripos((string)$row["packed_message"], $cleanName) === false) {
                continue;
            }
            $npcInMemory[] = $cleanName;
        }

        if (!empty($npcInMemory)) {
            $peopleFmt = $db->escape("|" . implode("|", array_values(array_unique($npcInMemory))) . "|");
    ...
    foreach ($missingCompanions2 as $row) {
        $peopleRow = $db->fetchOne("SELECT STRING_AGG(DISTINCT person, '|') AS people FROM public.memory_v CROSS JOIN LATERAL unnest(array[speaker, listener]) AS participant(person) WHERE gamets > {$row["gamets_truncated"]} - $pfi AND gamets <= {$row["gamets_truncated"]} + $pfi");
        $npcs      = [];
        ...
        $npcInMemory = [];
        foreach ($npcs as $name => $occurrences) {
            if (stripos((string)$row["packed_message"], $name) === false) {
                continue;
            }
            $npcInMemory[] = $name;
        }
...
function syncIndividualMemorySummaries($db, $connectionHandler = null, $maxBatchesPerNpc = 3)
{
    ...
    if (!$connectionHandler) {
        $summaryConnectorState = dialecticResolveSummaryConnectorRuntime();
    ...
    if (!$individualPromptTemplate) {
        $individualPromptTemplate =
            "You are writing an individual memory bank summary for NPC {NPC_NAME} in Fallout roleplay.\n".
    ...
    foreach ($enabledNpcs as $npcRow) {
        ...
        while (sizeof($pendingRows) >= $threshold && $batchCounter < $maxBatchesPerNpc) {
            ...
            foreach ($batch as $entry) {
                ...
                $history .= "===\nMemory entry, date " . convert_gamets2fallout_date($entryGamets) . PHP_EOL;
...
commands:
    ...
    query_worldknowledge	Query for a worldknowledge entry.
    ...
    sync_worldknowledge	Sync worldknowledge <> Vector embeddings. Needs TEXT2VEC active
...
");
    ...
    if ($argv[1] == "get") {
    ...
    } elseif ($argv[1] == "query_worldknowledge") {
        echo "Query memory for '{$argv[2]}'" . PHP_EOL;
        ...
        if ($GLOBALS["FEATURES"]["MEMORY_EMBEDDING"]["USE_TEXT2VEC"]) {
            ...
            error_log("DataSearchWorldKnowledgeByVector Expanded keywords: <$currentWorldKnowledgeTopic> <$locationCtx> <$contextKeywords>");

            $res = DataSearchWorldKnowledgeByVector($argv[2], $currentWorldKnowledgeTopic, $locationCtx, $contextKeywords);

        } else {
            die("FTS worldknowledge search still not supported in this script");
        }
    ...
    } elseif ($argv[1] == "sync_worldknowledge") {
        if ($GLOBALS["FEATURES"]["MEMORY_EMBEDDING"]["USE_TEXT2VEC"]) {
            echo "Creating vectors for memories" . PHP_EOL;

            $db      = new sql();
            $results = $db->fetchAll("select topic,topic_desc from worldknowledge where vector384 is null");
            $counter = 0;
            foreach ($results as $row) {

                $TEST_TEXT = $row["topic_desc"];
                storeMemoryWorldKnowledge($TEST_TEXT, $TEST_TEXT, $row["topic"]); // JUST UPDATE embedding in memory_summary
        ...
        if ($entries_to_process_count == 0) {
            ...
            if (!$summaryConnectorState) {
                ...
                if ($row["classifier"] == "diary") {
                    ...
                    $prompt   = [];
                    $prompt[] = ['role' => 'system',
                        'content' => "This is a playthrough in Fallout.\n" . $memoryPromptProcessed
                    ];
